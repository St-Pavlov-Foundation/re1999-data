...
function slot0.startLoading(slot0, slot1, slot2)
	...
	logNormal("BootResMgr.startLoading loadedCb = " .. slot6 .. slot7 .. tostring(slot2))

	slot0._loadedCb = slot1
	slot0._cbObj = slot2

	for slot6, slot7 in pairs(slot0._allRes.prefabs) do
		slot0._loadedCount = slot0._loadedCount + 1

		loadAbAsset(slot6, false, slot0._onPrefabLoaded, slot0)
	end

	for slot6, slot7 in pairs(slot0._allRes.imgs) do
		slot0._loadedCount = slot0._loadedCount + 1

		loadAbAsset(slot7.path, false, slot0._onImgLoaded, slot0)
	end
end
