...
function slot0._getLangVersions(slot0)
	...
	for slot9 = 1, #slot4 do
		if slot4[slot9] ~= GameConfig:GetDefaultVoiceShortcut() and (not string.nilorempty(slot1:GetLocalVersion(slot10)) or not BootVoiceView.instance:isFirstDownloadDone()) then
			table.insert(slot2, slot10)

			if string.nilorempty(slot1.VoiceBranch) or not tonumber(slot15) then
				slot15 = 1

				logError("随包的语音分支错误：" .. slot1.VoiceBranch)
			end

			table.insert(slot3, slot15 .. "." .. (string.nilorempty(slot12) and "0" or slot12))
		end
	end
