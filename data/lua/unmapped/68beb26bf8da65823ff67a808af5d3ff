...
function slot0.getAllLangDownloadList(slot0)
	...
	for slot8, slot9 in pairs(slot3) do
		...
		if #slot9.res > 0 and ((not slot1 or not string.nilorempty(slot0._optionalUpdateInst:GetLocalVersion(slot8))) and tabletool.indexOf(slot2, slot8)) then
			slot11 = {}

			for slot15, slot16 in ipairs(slot9.res) do
				table.insert(slot11, {
					latest_ver = slot9.latest_ver,
					name = slot16.name,
					hash = slot16.hash,
					order = slot16.order,
					length = slot16.length
				})
			end

			table.sort(slot11, uv0._sortByOrder)
