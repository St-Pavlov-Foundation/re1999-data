...
function slot0.getLoginUrls(slot0)
	...
	if VersionValidator.instance:isInReviewing() then
		slot3 = slot0 and slot1.loginReviewBackup or slot1.loginReview
	elseif type(slot0 and slot1.loginBackup or slot1.login) == "table" then
		if not slot4[tostring(SDKMgr.instance:getChannelId()) or "100"] then
