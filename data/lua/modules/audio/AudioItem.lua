...
function slot0.seekPercent(slot0, slot1)
	if slot0._playingId and slot0._emitter then
		slot0._emitter:SeekPercent(slot0._audioId, slot1)
	end
end

function slot0.seekMilliSeconds(slot0, slot1)
	if slot0._playingId and slot0._emitter then
		slot0._emitter:SeekMilliSeconds(slot0._audioId, slot1)
	end
end
	...
	if slot0._audioLang then
		if GameConfig:GetCurVoiceShortcut() == LangSettings.shortcutTab[LangSettings.zh] then
			...
			if slot3 then
				...
				slot0._emitter:Emitter(slot0._audioId, slot0._audioLang, slot1)
		...
		slot0._emitter:Emitter(slot0._audioId, slot1)
	end
