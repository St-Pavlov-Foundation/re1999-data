-- chunkname: @modules/versionactivitybase/dungeon/view/VersionActivityDungeonBaseMapLevelView.lua

module("modules.versionactivitybase.dungeon.view.VersionActivityDungeonBaseMapLevelView", package.seeall)

local VersionActivityDungeonBaseMapLevelView = class("VersionActivityDungeonBaseMapLevelView", BaseView)

function VersionActivityDungeonBaseMapLevelView:onInitView()
	...
	self._txtMapChapterIndex = gohelper.findChildText(self.viewGO, "anim/versionactivity/right/title/#txt_mapName/#txt_mapChapterIndex")
	self._imagestar1 = gohelper.findChildImage(self.viewGO, "anim/versionactivity/right/title/stars/starLayout/#image_star1")
	self._imagestar2 = gohelper.findChildImage(self.viewGO, "anim/versionactivity/right/title/stars/starLayout/#image_star2")
...
function VersionActivityDungeonBaseMapLevelView:addEvents()
...
function VersionActivityDungeonBaseMapLevelView:removeEvents()
...
function VersionActivityDungeonBaseMapLevelView:_btnreplayStoryOnClick()
...
function VersionActivityDungeonBaseMapLevelView:refreshStoryIdList()
...
function VersionActivityDungeonBaseMapLevelView:_btncloseviewOnClick()
	self:startCloseTaskNextFrame()
end

function VersionActivityDungeonBaseMapLevelView:startCloseTaskNextFrame()
	TaskDispatcher.runDelay(self.reallyClose, self, 0.01)
end

function VersionActivityDungeonBaseMapLevelView:cancelStartCloseTask()
	TaskDispatcher.cancelTask(self.reallyClose, self)
end

function VersionActivityDungeonBaseMapLevelView:reallyClose()
	self:closeThis()
end

function VersionActivityDungeonBaseMapLevelView:_btnleftarrowOnClick()
	if self.mode == VersionActivityDungeonBaseEnum.DungeonMode.Hard then
		return
	end
...
function VersionActivityDungeonBaseMapLevelView:_btnrightarrowOnClick()
	if self.mode == VersionActivityDungeonBaseEnum.DungeonMode.Hard then
		return
	end
...
function VersionActivityDungeonBaseMapLevelView:refreshUIByMode(mode)
...
function VersionActivityDungeonBaseMapLevelView:startRefreshUI()
	self:refreshUI()
end

function VersionActivityDungeonBaseMapLevelView:_btnactivityrewardOnClick()
	DungeonController.instance:openDungeonRewardView(self.showEpisodeCo)
end

function VersionActivityDungeonBaseMapLevelView:_btnnormalStartOnClick()
...
function VersionActivityDungeonBaseMapLevelView:_btnhardStartOnClick()
	self:startBattle()
end

function VersionActivityDungeonBaseMapLevelView:_btnLockStartOnClick()
	GameFacade.showToast(ToastEnum.VersionActivityCanFight, self:getPreModeName())
end

function VersionActivityDungeonBaseMapLevelView:_btncloseruleOnClick()
	return
end

function VersionActivityDungeonBaseMapLevelView:startBattle()
...
function VersionActivityDungeonBaseMapLevelView:_playMainStory()
...
function VersionActivityDungeonBaseMapLevelView:_playSkipMainStory()
	DungeonRpc.instance:sendStartDungeonRequest(self.showEpisodeCo.chapterId, self.showEpisodeCo.id)
	self:onStoryFinished()
end

function VersionActivityDungeonBaseMapLevelView:playAfterStory(storyId)
	local param = {}
...
function VersionActivityDungeonBaseMapLevelView:_playStoryAndEnterFight(storyId)
...
function VersionActivityDungeonBaseMapLevelView:_enterFight()
	DungeonFightController.instance:enterFight(self.showEpisodeCo.chapterId, self.showEpisodeCo.id, 1)
end

function VersionActivityDungeonBaseMapLevelView:onStoryFinished()
...
function VersionActivityDungeonBaseMapLevelView:initLocalEpisodeMode()
	self.unlockedEpisodeModeDict = self:loadDict(PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastUnLockMode)
	self.lastEpisodeSelectModeDict = self:loadDict(PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastSelectMode)
end

function VersionActivityDungeonBaseMapLevelView:_onCurrencyChange(changeIds)
...
function VersionActivityDungeonBaseMapLevelView:_editableInitView()
	...
	self._simageactivitynormalbg:LoadImage(ResUrl.getVersionActivityDungeonIcon("bg002"))
	self._simageactivityhardbg:LoadImage(ResUrl.getVersionActivityDungeonIcon("bg003"))
...
function VersionActivityDungeonBaseMapLevelView:initViewParam()
	...
	self.normalConfig = DungeonConfig.instance:getVersionActivityDungeonNormalEpisode(self.originEpisodeId, VersionActivityEnum.DungeonChapterId.LeiMiTeBeiHard, VersionActivityEnum.DungeonChapterId.LeiMiTeBei)
...
function VersionActivityDungeonBaseMapLevelView:getEpisodeIndex()
	return DungeonConfig.instance:getEpisodeLevelIndexByEpisodeId(self.originEpisodeId)
end

function VersionActivityDungeonBaseMapLevelView:markSelectEpisode()
	if self.originEpisodeConfig.type == DungeonEnum.EpisodeType.Normal or self.originEpisodeConfig.type == DungeonEnum.EpisodeType.Boss then
...
function VersionActivityDungeonBaseMapLevelView:onUpdateParam()
	self:onOpen()
end

function VersionActivityDungeonBaseMapLevelView:onOpen()
...
function VersionActivityDungeonBaseMapLevelView:initMode()
		...
		if self.isSpecialEpisode then
			if not self.isFromJump then
				...
				if not self.needPlayUnlockModeAnimation then
					local mode = self.lastEpisodeSelectModeDict[tostring(self.specialEpisodeId)] or VersionActivityDungeonBaseEnum.DungeonMode.Story

					if self.mode2EpisodeDict[mode] then
						self.mode = mode
					else
						self:logError(episodeList, self.mode2EpisodeDict, self.mode)
					end
				end
			else
				self:checkNeedPlayModeUnLockAnimation()
...
function VersionActivityDungeonBaseMapLevelView:logError(episodeList, mode2EpisodeDict, mode)
	local str = string.format("cur mode : %s, episodeList : [", mode)

	for _, co in ipairs(episodeList) do
		str = str .. string.format("%s, ", co and co.id or "nil")
	end

	str = str .. "], mode2EpisodeDict : ["

	for key, co in pairs(mode2EpisodeDict) do
		str = str .. string.format("%s:%s, ", key, co and co.id or "nil")
	end
...
function VersionActivityDungeonBaseMapLevelView:refreshBg()
...
function VersionActivityDungeonBaseMapLevelView:refreshUI()
...
function VersionActivityDungeonBaseMapLevelView:refreshModeCanFight()
...
function VersionActivityDungeonBaseMapLevelView:refreshEpisodeTextInfo()
...
function VersionActivityDungeonBaseMapLevelView:refreshStar()
	...
	if string.nilorempty(advancedConditionText) then
		...
		self:setImage(self._imagestar2, passStory and self.showEpisodeMo.star >= DungeonEnum.StarType.Advanced)
	end
end

function VersionActivityDungeonBaseMapLevelView:refreshMode()
...
function VersionActivityDungeonBaseMapLevelView:refreshArrow()
...
function VersionActivityDungeonBaseMapLevelView:refreshReward()
	local rewardList = {}
...
function VersionActivityDungeonBaseMapLevelView:refreshStartBtn()
	self:refreshCostPower()
...
function VersionActivityDungeonBaseMapLevelView:refreshCostPower()
	local cost = 0
...
function VersionActivityDungeonBaseMapLevelView:checkNeedPlayModeUnLockAnimation()
...
function VersionActivityDungeonBaseMapLevelView:_playModeUnLockAnimation(animationName)
...
function VersionActivityDungeonBaseMapLevelView:playModeUnlockAnimation()
	if self.needPlayUnlockModeAnimation then
...
function VersionActivityDungeonBaseMapLevelView:onModeUnlockAnimationPlayDone()
...
function VersionActivityDungeonBaseMapLevelView:setImage(image, light)
	if light then
...
function VersionActivityDungeonBaseMapLevelView:buildEpisodeName()
	...
	return string.format("<size=112>%s</size>%s", firstName, remainName)
end

function VersionActivityDungeonBaseMapLevelView:buildColorText(text, color)
	return string.format("<color=%s>%s</color>", color, text)
end

function VersionActivityDungeonBaseMapLevelView:getPreModeName()
...
function VersionActivityDungeonBaseMapLevelView:getKey(key)
	return key .. self.userId
end

function VersionActivityDungeonBaseMapLevelView:loadDict(key)
	key = self:getKey(key)
...
function VersionActivityDungeonBaseMapLevelView:saveEpisodeUnlockMode()
	PlayerPrefsHelper.setString(self:getKey(PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastUnLockMode), cjson.encode(self.unlockedEpisodeModeDict))
end

function VersionActivityDungeonBaseMapLevelView:saveEpisodeLastSelectMode()
	PlayerPrefsHelper.setString(self:getKey(PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastSelectMode), cjson.encode(self.lastEpisodeSelectModeDict))
end

function VersionActivityDungeonBaseMapLevelView:onClose()
...
function VersionActivityDungeonBaseMapLevelView:onDestroyView()
...
return VersionActivityDungeonBaseMapLevelView
