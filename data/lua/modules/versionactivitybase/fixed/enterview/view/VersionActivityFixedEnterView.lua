...
slot0 = class("VersionActivityFixedEnterView", VersionActivityEnterBaseViewWithListNew)
slot1 = 2.1
...
function slot0._editableInitView(slot0)
	...
	slot0:setTabLevelSetting(VersionActivityEnterViewEnum.ActLevel.First, slot0._gotabitem1, VersionActivityFixedHelper.getVersionActivityEnterViewTabItem1())
	slot0:setTabLevelSetting(VersionActivityEnterViewEnum.ActLevel.Second, slot0._gotabitem2, VersionActivityFixedHelper.getVersionActivityEnterViewTabItem2())
...
function slot0._onTabScrollChange(slot0)
	for slot5, slot6 in ipairs(slot0.activityTabItemList) do
		if slot6:isShowRedDot() and slot0.viewPortHeight < -slot6:getAnchorY() + VersionActivityFixedHelper.getVersionActivityEnum().RedDotOffsetY - recthelper.getAnchorY(slot0.rectTrContent) then
...
function slot0.refreshBtnVisible(slot0, slot1)
	gohelper.setActive(slot0.goReplayBtn, VersionActivityFixedEnterHelper.GetIsShowReplayBtn(slot0.curActId))
	gohelper.setActive(slot0.goAchievementBtn, VersionActivityFixedEnterHelper.GetIsShowAchievementBtn(slot0.curActId))
end

function slot0.onOpen(slot0)
	uv0.super.onOpen(slot0)

	if slot0.curActId == VersionActivityFixedHelper.getVersionActivityEnum().ActivityId.Dungeon then
