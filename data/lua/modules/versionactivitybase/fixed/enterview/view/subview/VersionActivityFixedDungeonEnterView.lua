...
slot0 = class("VersionActivityFixedDungeonEnterView", BaseView)
...
function slot0.removeEvents(slot0)
	...
	slot0:removeEventCb(RedDotController.instance, RedDotEvent.UpdateRelateDotInfo, slot0.refreshRedDot, slot0, LuaEventSystem.Low)
...
function slot0._editableInitView(slot0)
	...
	slot0.actId = VersionActivityFixedHelper.getVersionActivityEnum().ActivityId.Dungeon
	slot0.animComp = VersionActivityFixedHelper.getVersionActivitySubAnimatorComp().get(slot0.viewGO, slot0)
	...
	slot0.goLock = slot0._btnLocked.gameObject
	slot0.actId = VersionActivityFixedHelper.getVersionActivityEnum().ActivityId.Dungeon
	slot0.actCo = ActivityConfig.instance:getActivityCo(slot0.actId)

	slot0:_setDesc()
	RedDotController.instance:addRedDot(slot0._goreddot, VersionActivityFixedHelper.getVersionActivityDungeonEnterReddotId())
end
...
function slot0.refreshRemainTime(slot0)
	...
	slot3 = ActivityModel.instance:getActivityInfo()[VersionActivityFixedHelper.getVersionActivityEnum().ActivityId.DungeonStore]
...
function slot0.refreshActivityState(slot0)
	...
	slot3 = slot1 == ActivityEnum.ActivityStatus.NotOpen

	gohelper.setActive(slot0.goEnter, slot2)
	gohelper.setActive(slot0.goFinish, not slot3 and not slot2)
	gohelper.setActive(slot0.goLock, slot3)
	gohelper.setActive(slot0._gotime, not (slot1 == ActivityEnum.ActivityStatus.Expired))
	gohelper.setActive(slot0.goStore, ActivityHelper.getActivityStatusAndToast(VersionActivityFixedHelper.getVersionActivityEnum().ActivityId.DungeonStore) == ActivityEnum.ActivityStatus.Normal)
end

function slot0.refreshStoreCurrency(slot0)
	slot0._txtStoreNum.text = GameUtil.numberDisplay(CurrencyModel.instance:getCurrency(VersionActivityFixedHelper.getVersionActivityCurrencyType()) and slot1.quantity or 0)
end

function slot0.refreshRedDot(slot0)
	slot1 = VersionActivityFixedHelper.getVersionActivityEnum().ActivityId.Dungeon

	gohelper.setActive(slot0._gohardModeUnLock, VersionActivityDungeonBaseController.instance:isOpenActivityHardDungeonChapter(slot1) and ActivityHelper.getActivityStatus(slot1) == ActivityEnum.ActivityStatus.Normal)
end
