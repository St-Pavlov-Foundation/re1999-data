...
slot0 = class("VersionActivityFixedEnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)
...
function slot0.afterSetData(slot0)
	...
	if not slot0.activityCo then
		...
		logError("VersionActivityFixedEnterViewTabItemBase.afterSetData error, no act config, actId:%s", slot0.actId)

		return
	end

	if not string.nilorempty(string.split(slot0.activityCo.tabBgPath, "#")[1]) then
		VersionActivityFixedHelper.setMainActivitySprite(slot0.imageSelectTabIcon, slot2)
	end

	if not string.nilorempty(slot1[2]) then
		VersionActivityFixedHelper.setMainActivitySprite(slot0.imageUnSelectTabIcon, slot3)
	end
...
function slot0.refreshNormalTag(slot0)
	...
	if VersionActivityFixedEnterHelper.GetIsShowTabRemainTime(slot0.actId) and slot1 then
		if VersionActivityFixedHelper.getVersionActivityEnum().MaxShowTimeOffset < slot1:getRealEndTimeStamp() - ServerTime.now() then
			return
		end
