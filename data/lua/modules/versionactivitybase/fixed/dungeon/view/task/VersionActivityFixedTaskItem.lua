-- chunkname: @modules/versionactivitybase/fixed/dungeon/view/task/VersionActivityFixedTaskItem.lua

module("modules.versionactivitybase.fixed.dungeon.view.task.VersionActivityFixedTaskItem", package.seeall)

local VersionActivityFixedTaskItem = class("VersionActivityFixedTaskItem", ListScrollCell)

function VersionActivityFixedTaskItem:init(go)
	self.viewGO = go
...
function VersionActivityFixedTaskItem:addEventListeners()
...
function VersionActivityFixedTaskItem:_btnNotFinishOnClick()
	if self.co.jumpId ~= 0 then
		...
		if GameFacade.jump(self.co.jumpId) then
			ViewMgr.instance:closeView(VersionActivityFixedHelper.getVersionActivityTaskViewName(self._bigVersion, self._smallVersion))
		end
	end
end

function VersionActivityFixedTaskItem:_btnFinishAllOnClick()
	VersionActivityFixedDungeonController.instance:dispatchEvent(VersionActivityFixedDungeonEvent.OnClickAllTaskFinish)
end

VersionActivityFixedTaskItem.FinishKey = "VersionActivityFixedTaskItem FinishKey"

function VersionActivityFixedTaskItem:_btnFinishOnClick()
	UIBlockMgr.instance:startBlock(VersionActivityFixedTaskItem.FinishKey)
...
function VersionActivityFixedTaskItem:_OnClickAllTaskFinish()
	if self.taskMo then
...
function VersionActivityFixedTaskItem:firstAnimationDone()
	self._view.viewContainer.taskAnimRemoveItem:removeByIndex(self._index, self.secondAnimationDone, self)
end

function VersionActivityFixedTaskItem:secondAnimationDone()
	UIBlockMgr.instance:endBlock(VersionActivityFixedTaskItem.FinishKey)
...
function VersionActivityFixedTaskItem:_editableInitView()
	self._bigVersion, self._smallVersion = VersionActivityFixedDungeonController.instance:getEnterVerison()
	self.rewardItemList = {}
end

function VersionActivityFixedTaskItem:onUpdateMO(taskMo)
	self.taskMo = taskMo
...
function VersionActivityFixedTaskItem:refreshNormalUI()
...
function VersionActivityFixedTaskItem:refreshRewardItems()
...
function VersionActivityFixedTaskItem:refreshGetAllUI()
	return
end

function VersionActivityFixedTaskItem:canGetReward()
	return self.taskMo.finishCount < self.co.maxFinishCount and self.taskMo.hasFinished
end

function VersionActivityFixedTaskItem:getAnimator()
	return self.animator
end

function VersionActivityFixedTaskItem:onDestroyView()
	self._simagenormalbg:UnLoadImage()
	self._simagegetallbg:UnLoadImage()
end

return VersionActivityFixedTaskItem
