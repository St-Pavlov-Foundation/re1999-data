-- chunkname: @modules/versionactivitybase/fixed/dungeon/view/map/scene/VersionActivityFixedDungeonMapScene.lua

module("modules.versionactivitybase.fixed.dungeon.view.map.scene.VersionActivityFixedDungeonMapScene", package.seeall)

local VersionActivityFixedDungeonMapScene = class("VersionActivityFixedDungeonMapScene", BaseView)

function VersionActivityFixedDungeonMapScene:onInitView()
...
function VersionActivityFixedDungeonMapScene:addEvents()
...
function VersionActivityFixedDungeonMapScene:removeEvents()
...
function VersionActivityFixedDungeonMapScene:_onOpenView(viewName)
...
function VersionActivityFixedDungeonMapScene:_onCloseView(viewName)
	if viewName == ViewName.StoryView then
		self:loadMap()
	end
end

function VersionActivityFixedDungeonMapScene:_onScreenResize()
	if self._sceneGo then
		local camera = CameraMgr.instance:getMainCamera()
		local scale = GameUtil.getAdapterScale()

		camera.orthographicSize = VersionActivityFixedHelper.getVersionActivityDungeonEnum(self._bigVersion, self._smallVersion).DungeonMapCameraSize * scale

		self:_initScene()
	end
end

function VersionActivityFixedDungeonMapScene:onModeChange()
	self:refreshMap()
end

function VersionActivityFixedDungeonMapScene:onActivityDungeonMoChange()
	local needTween = VersionActivityFixedDungeonModel.instance:getMapNeedTweenState()

	self:refreshMap(needTween)
end

function VersionActivityFixedDungeonMapScene:onClickElement(elementComp)
...
function VersionActivityFixedDungeonMapScene:onFocusElement(elementId, isForce)
...
function VersionActivityFixedDungeonMapScene:changeToElementEpisode(elementId)
...
function VersionActivityFixedDungeonMapScene:manualClickElement(elementId)
	local elementComp = self.mapSceneElementsView:getElementComp(elementId)
...
function VersionActivityFixedDungeonMapScene:_onDragBegin(param, pointerEventData)
...
function VersionActivityFixedDungeonMapScene:_onDrag(param, pointerEventData)
...
function VersionActivityFixedDungeonMapScene:_onDragEnd(param, pointerEventData)
	self._dragBeginPos = nil
end

function VersionActivityFixedDungeonMapScene:getDragWorldPos(pointerEventData)
...
function VersionActivityFixedDungeonMapScene:_editableInitView()
...
function VersionActivityFixedDungeonMapScene:_initMapRootNode()
	self._sceneRoot = UnityEngine.GameObject.New(VersionActivityFixedHelper.getVersionActivityDungeonEnum(self._bigVersion, self._smallVersion).SceneRootName)
...
function VersionActivityFixedDungeonMapScene:onUpdateParam()
	self:refreshMap()
end

function VersionActivityFixedDungeonMapScene:onOpen()
	MainCameraMgr.instance:addView(self.viewName, self._initCamera, nil, self)
	self:refreshMap()
end

function VersionActivityFixedDungeonMapScene:_initCamera()
	local camera = CameraMgr.instance:getMainCamera()
...
function VersionActivityFixedDungeonMapScene:refreshMap(needTween, mapCfg)
...
function VersionActivityFixedDungeonMapScene:_initElements()
	DungeonController.instance:dispatchEvent(DungeonMapElementEvent.OnInitElements)
end

function VersionActivityFixedDungeonMapScene:loadMap()
...
function VersionActivityFixedDungeonMapScene:buildLoadRes(allResPath, mapCfg)
...
function VersionActivityFixedDungeonMapScene:_loadSceneFinish()
...
function VersionActivityFixedDungeonMapScene:_initScene()
	...
	local cameraSizeRate = VersionActivityFixedHelper.getVersionActivityDungeonEnum(self._bigVersion, self._smallVersion).DungeonMapCameraSize / uiCameraSize
...
function VersionActivityFixedDungeonMapScene:_setMapPos()
...
function VersionActivityFixedDungeonMapScene:_addMapLight()
...
function VersionActivityFixedDungeonMapScene:_addMapAudio()
...
function VersionActivityFixedDungeonMapScene:tweenSetScenePos(targetPos, srcPos)
...
function VersionActivityFixedDungeonMapScene:getTargetPos(targetPos)
...
function VersionActivityFixedDungeonMapScene:tweenFrameCallback(value)
...
function VersionActivityFixedDungeonMapScene:tweenFinishCallback()
	self._tempVector:Set(self._tweenTargetPosX, self._tweenTargetPosY, 0)
	self:directSetScenePos(self._tempVector)
end

function VersionActivityFixedDungeonMapScene:directSetScenePos(targetPos)
...
function VersionActivityFixedDungeonMapScene:_updateElementArrow()
	DungeonController.instance:dispatchEvent(DungeonMapElementEvent.OnUpdateElementArrow)
end

function VersionActivityFixedDungeonMapScene:setVisible(isVisible)
...
function VersionActivityFixedDungeonMapScene:getSceneGo()
	return self._sceneGo
end

function VersionActivityFixedDungeonMapScene:onClose()
	self:killTween()
	self:_resetCamera()
end

function VersionActivityFixedDungeonMapScene:killTween()
	if self.tweenId then
		ZProj.TweenHelper.KillById(self.tweenId)
	end
end

function VersionActivityFixedDungeonMapScene:_resetCamera()
	local camera = CameraMgr.instance:getMainCamera()
...
function VersionActivityFixedDungeonMapScene:disposeOldMap()
...
function VersionActivityFixedDungeonMapScene:_disposeScene()
	self._oldScenePos = self._scenePos
...
return VersionActivityFixedDungeonMapScene
