	...
	if slot0._hasAudio then
		...
		if slot4 then
			if GameConfig:GetCurVoiceShortcut() == LangSettings.shortcutTab[LangSettings.zh] then
				slot7 = AudioConfig.instance:getAudioCOById(slot2.audio).bankName

				ZProj.AudioManager.Instance:LoadBank(slot7, slot4)
				slot0._emitter:Emitter(slot2.audio, slot4, slot0._onEmitterCallback, slot0)
				ZProj.AudioManager.Instance:UnloadBank(slot7)
			else
				slot0._emitter:Emitter(slot2.audio, slot4, slot0._onEmitterCallback, slot0)
			end
		else
