...
slot0 = class("LengZhou6TaskListModel", ListScrollModel)

function slot1(slot0)
	if slot0.id == LengZhou6Enum.TaskMOAllFinishId then
		return 1
...
function slot0.init(slot0)
	slot1 = {}
	slot2 = 0
	slot3 = LengZhou6Model.instance:getAct190Id()

	if TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.Activity190) ~= nil then
		for slot9, slot10 in ipairs(LengZhou6Config.instance:getTaskByActId(slot3)) do
			slot11 = LengZhou6TaskMo.New()
	...
	if slot2 > 1 then
		slot5 = LengZhou6TaskMo.New()
		slot5.id = LengZhou6Enum.TaskMOAllFinishId
...
function slot0.preFinish(slot0, slot1)
	...
	if slot1.id == LengZhou6Enum.TaskMOAllFinishId then
		for slot8, slot9 in ipairs(slot4) do
			if slot9:alreadyGotReward() and slot9.id ~= LengZhou6Enum.TaskMOAllFinishId then
	...
	if slot2 then
		if slot0:getById(LengZhou6Enum.TaskMOAllFinishId) and slot0:getGotRewardCount() < slot3 + 1 then
...
function slot0.getGotRewardCount(slot0, slot1)
	for slot7, slot8 in ipairs(slot1 or slot0:getList()) do
		if slot8:alreadyGotReward() and not slot8.preFinish and slot8.id ~= LengZhou6Enum.TaskMOAllFinishId then
			slot3 = 0 + 1
		end
	end
