...
function var_0_0.onOpen(arg_4_0)
	...
	if arg_4_0.viewParam.isFight then
		...
		for iter_4_0, iter_4_1 in ipairs(arg_4_0.fetterCoList) do
			...
			if arg_4_0.activeCnt >= iter_4_1.activeNum then
				...
				var_4_3.text = string.format("<color=#9c8052>（%d/%d）%s</color>", arg_4_0.activeCnt, iter_4_1.activeNum, var_4_5)
			else
				var_4_3.text = string.format("（<color=#ad2319>%d</color>/%d）%s", arg_4_0.activeCnt, iter_4_1.activeNum, var_4_5)
			end
		...
		var_4_1 = string.gsub(arg_4_0.fetterCoList[arg_4_0.activeLvl].desc, "（(.-)）", "")
	...
	if arg_4_0.viewParam.isPreview then
		for iter_4_2, iter_4_3 in ipairs(arg_4_0.fetterCoList) do
			...
			var_4_7.text = string.format("<color=#9c8052>（%d）%s</color>", iter_4_3.activeNum, var_4_9)

			arg_4_0:_fixHeight(var_4_7)
		end

		arg_4_0.fetterHeroList = Activity191Config.instance:getFetterHeroList(arg_4_0.tag, arg_4_0.actId)
		var_4_1 = string.gsub(arg_4_0.fetterCoList[0].desc, "（(.-)）", "")
	elseif arg_4_0.viewParam.isEnemy then
		...
		for iter_4_4, iter_4_5 in ipairs(arg_4_0.fetterCoList) do
			...
			if arg_4_0.activeCnt >= iter_4_5.activeNum then
				...
				var_4_12.text = string.format("<color=#9c8052>（%d/%d）%s</color>", arg_4_0.activeCnt, iter_4_5.activeNum, var_4_14)
			else
				var_4_12.text = string.format("（<color=#ad2319>%d</color>/%d）%s", arg_4_0.activeCnt, iter_4_5.activeNum, var_4_14)
			end
		...
		for iter_4_6, iter_4_7 in ipairs(arg_4_0.fetterCoList) do
			...
			if arg_4_0.activeCnt >= iter_4_7.activeNum then
				...
				var_4_16.text = string.format("<color=#9c8052>（%d/%d）%s</color>", arg_4_0.activeCnt, iter_4_7.activeNum, var_4_18)
			else
				var_4_16.text = string.format("（<color=#ad2319>%d</color>/%d）%s", arg_4_0.activeCnt, iter_4_7.activeNum, var_4_18)
			end
