module("modules.logic.versionactivity2_7.act191.view.Act191ModifyNameView", package.seeall)

slot0 = class("Act191ModifyNameView", BaseView)
...
function slot0._onBtnSure(slot0)
	...
	if GameUtil.utf8len(slot1) > 5 then
		GameFacade.showToast(ToastEnum.InformPlayerCharLen)

		return
	end

	Activity191Model.instance:getActInfo():getGameInfo():setTeamName(slot1)
	slot0:closeThis()
end
...
function slot0.onOpen(slot0)
	...
	slot2 = Activity191Model.instance:getActInfo():getGameInfo():getTeamInfo().name

	slot0._input:SetText(slot2)

	slot0._blurTweenId = ZProj.TweenHelper.DOTweenFloat(0, 1, 0.35, slot0._onFrame, slot0._onFinish, slot0, nil, EaseType.Linear)

	gohelper.setActive(slot0._btncleanname, not string.nilorempty(slot2))
end
