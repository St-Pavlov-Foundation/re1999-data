...
function slot0.onOpen(slot0)
	...
	if slot2:hasReward() and slot2.currLevel == slot0._co.id and not slot2.allPass then
			...
			for slot10, slot11 in ipairs(slot2.rewardItems) do
				if slot11.type == DiceHeroEnum.RewardType.Hero then
					...
					if not string.nilorempty(lua_dice_character.configDict[slot11.id].relicIds) then
						slot17 = slot12.relicIds

						for slot16, slot17 in ipairs(string.splitToNumber(slot17, "#")) do
		...
		gohelper.CreateObjList(slot0, slot0._createRewardItem, slot6, slot0._goreward, slot0._gorewarditem)
...
function slot0.nextStep(slot0)
	...
	if not table.remove(slot0._contentGos, 1) then
	...
	if recthelper.getHeight(slot0._transscroll) < recthelper.getHeight(slot0._transcontent) and not slot0._curTxtData then
		slot0._tweenId = ZProj.TweenHelper.DOTweenFloat(slot0.scrollContent.verticalNormalizedPosition, 0, 0.3, slot0.tweenFrameCallback, slot0.tweenFinishCallback, slot0)
	end
end

function slot0.checkSendLevelReq(slot0)
	if DiceHeroModel.instance:getGameInfo(slot0._co.chapter).currLevel == slot0._co.id and not slot1.allPass and slot0._co.rewardSelectType == DiceHeroEnum.GetRewardType.None then
...
function slot0._createRewardItem(slot0, slot1, slot2, slot3)
	slot6 = gohelper.findChildButtonWithAudio(slot1, "#btn_choose")
	slot2.anim = gohelper.findChildAnim(slot1, "")

	slot0:removeClickCb(slot6)
	slot0:addClickCb(slot6, slot0._onClickSelect, slot0, {
		index = slot2.index or slot3,
		data = slot2
	})
	gohelper.setActive(gohelper.findChild(slot1, "headbg/#go_cardicon"), false)
	gohelper.setActive(gohelper.findChildSingleImage(slot1, "headbg/#simage_icon"), true)

	if slot2.type == DiceHeroEnum.RewardType.Hero then
		gohelper.findChildTextMesh(slot1, "#txt_title").text = lua_dice_character.configDict[slot2.id] and slot10.name or ""
		gohelper.findChildTextMesh(slot1, "scroll_desc/viewport/#txt_desc").text = slot10 and slot10.desc or ""

		slot7:LoadImage(ResUrl.getHeadIconSmall(slot10.icon))
	elseif slot2.type == DiceHeroEnum.RewardType.SkillCard then
		slot4.text = lua_dice_card.configDict[slot2.id] and slot10.name or ""
		slot5.text = slot10 and slot10.desc or ""

		gohelper.setActive(slot8, true)
		gohelper.setActive(slot7, false)
	elseif slot2.type == DiceHeroEnum.RewardType.Relic then
		slot4.text = lua_dice_relic.configDict[slot2.id] and slot10.name or ""
		slot5.text = slot10 and slot10.desc or ""

		slot7:LoadImage("singlebg/v2a6_dicehero_singlebg/collection/" .. slot10.icon .. ".png")
	end
...
function slot0._onClickSelect(slot0, slot1)
	if slot1.data.type == DiceHeroEnum.RewardType.Hero and slot0._co.mode == 2 and not slot2.isShowAll then
		...
		if not string.nilorempty(lua_dice_character.configDict[slot2.id].relicIds) then
			slot9 = slot4.relicIds

			for slot8, slot9 in ipairs(string.splitToNumber(slot9, "#")) do
...
function slot0._delayRefrshReward(slot0)
	gohelper.CreateObjList(slot0, slot0._createRewardItem, slot0._rewardItem, slot0._goreward, slot0._gorewarditem)
end
