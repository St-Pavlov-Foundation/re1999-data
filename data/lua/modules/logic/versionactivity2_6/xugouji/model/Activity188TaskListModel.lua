...
slot0 = class("Activity188TaskListModel", ListScrollModel)

function slot0.init(slot0, slot1)
	slot3 = {}
	slot4 = 0

	if TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.Activity188) ~= nil then
		for slot9, slot10 in ipairs(Activity188Config.instance:getTaskList(slot1)) do
			slot11 = Activity188TaskMO.New()
	...
	if slot4 > 1 then
		slot5 = Activity188TaskMO.New()
		slot5.id = LoperaEnum.TaskMOAllFinishId
...
function slot0.getSortIndex(slot0)
	if slot0.id == XugoujiEnum.TaskMOAllFinishId then
		return 1
...
function slot0.preFinish(slot0, slot1)
	...
	if slot1.id == XugoujiEnum.TaskMOAllFinishId then
		for slot8, slot9 in ipairs(slot4) do
			if slot9:alreadyGotReward() and slot9.id ~= XugoujiEnum.TaskMOAllFinishId then
	...
	if slot2 then
		if slot0:getById(XugoujiEnum.TaskMOAllFinishId) and slot0:getGotRewardCount() < slot3 + 1 then
...
function slot0.getGotRewardCount(slot0, slot1)
	for slot7, slot8 in ipairs(slot1 or slot0:getList()) do
		if slot8:alreadyGotReward() and not slot8.preFinish and slot8.id ~= XugoujiEnum.TaskMOAllFinishId then
			slot3 = 0 + 1
		end
	end
