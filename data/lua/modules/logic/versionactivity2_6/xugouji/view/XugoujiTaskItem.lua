...
slot0 = class("XugoujiTaskItem", ListScrollCellExtend)
...
function slot0._btnnotfinishbgOnClick(slot0)
	if slot0._taskMO.config.jumpId > 0 then
		GameFacade.jump(slot0._taskMO.config.jumpId)
	end
end
...
function slot0._btngetallOnClick(slot0)
	XugoujiController.instance:dispatchEvent(XugoujiEvent.OneClickClaimReward, slot0._taskMO.activityId)
	UIBlockHelper.instance:startBlock("LoperaTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(slot0._delayFinishAll, slot0, 0.5)
end

function slot0._delayFinish(slot0)
	TaskRpc.instance:sendFinishTaskRequest(slot0._taskMO.config.id)
end
...
function slot0._editableInitView(slot0)
	slot0._animator = slot0.viewGO:GetComponent(uv0)
...
function slot0._editableAddEvents(slot0)
	LoperaController.instance:registerCallback(LoperaEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._editableRemoveEvents(slot0)
	LoperaController.instance:unregisterCallback(LoperaEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end
...
function slot0.onUpdateMO(slot0, slot1)
	...
	slot0:_refreshUI()
	slot0:_moveByRankDiff(Activity188TaskListModel.instance:getRankDiff(slot1))
end
...
function slot0._moveByRankDiff(slot0, slot1)
	if slot1 and slot1 ~= 0 then
		...
		slot0._rankDiffMoveId = ZProj.TweenHelper.DOAnchorPosY(slot0.viewTrs, 0, LoperaEnum.AnimatorTime.TaskRewardMoveUp)
	end
end
...
function slot0._refreshUI(slot0)
	...
	slot2 = slot1.id ~= LoperaEnum.TaskMOAllFinishId
...
slot0.prefabPath = "ui/viewres/versionactivity_2_6/v2a6_xugouji/v2a6_xugouji_taskitem.prefab"
