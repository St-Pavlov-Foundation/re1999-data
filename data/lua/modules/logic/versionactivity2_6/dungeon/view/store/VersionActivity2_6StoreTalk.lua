...
slot0 = class("VersionActivity2_6StoreTalk", BaseView)
...
function slot0._editableInitView(slot0)
	...
	slot0.actId = VersionActivity2_6Enum.ActivityId.DungeonStore
...
function slot0.onOpenFinish(slot0)
	if slot0.isFirstEnter then
		...
		slot1 = PlayerModel.instance:getPlayerPrefsKey(PlayerPrefsKey.Version2_6PlayedStoreGroupIdKey)
...
function slot0.triggerTalkByType(slot0, slot1, slot2)
	...
	TaskDispatcher.cancelTask(slot0._checkTriggerTalk, slot0)

	if not slot0._hasOpen then
		slot0._hasOpen = true

		slot0.chessAnim:Play("open", 0, 0)
		TaskDispatcher.runDelay(slot0._checkTriggerTalk, slot0, uv0.ChessOpenAnimDuration)
	else
		slot0:_checkTriggerTalk()
	end
end

function slot0._checkTriggerTalk(slot0)
	slot0:_triggerTalk(slot0:getRandomTalkCo(slot0.canTriggerList))
end
