module("modules.logic.versionactivity2_6.dungeon.view.task.VersionActivity2_6TaskItem", package.seeall)

slot0 = class("VersionActivity2_6TaskItem", ListScrollCell)
...
function slot0._btnNotFinishOnClick(slot0)
	if slot0.co.jumpId ~= 0 then
		...
		if GameFacade.jump(slot0.co.jumpId) then
			ViewMgr.instance:closeView(ViewName.VersionActivity2_6TaskView)
		end
	end
end
...
function slot0.secondAnimationDone(slot0)
	...
	if slot0.taskMo.getAll then
		TaskRpc.instance:sendFinishAllTaskRequest(TaskEnum.TaskType.ActivityDungeon, nil, , , , VersionActivity2_6Enum.ActivityId.Dungeon)
	else
