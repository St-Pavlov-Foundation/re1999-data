...
function slot0.getLangTitle(slot0)
	if slot0.titleMap[LangSettings.instance:getCurLangShortcut()] or slot0:getTemplateTitle() then
		return slot2
	end
end
...
function slot0.getJumpLink(slot0)
	if slot0.mailId == 0 then
		return slot0.jump and slot0.jump[LangSettings.instance:getCurLangShortcut()] or ""
	end
...
function slot0.getTemplateJumpTitle(slot0)
	if slot0.mailId == 0 then
		return slot0.jumpTitle and slot0.jumpTitle[LangSettings.instance:getCurLangShortcut()] or ""
	end
...
function slot0.getLangSender(slot0)
	if slot0.senderMap[LangSettings.instance:getCurLangShortcut()] or slot0:getTemplateSender() then
		return slot2
	end
end
...
function slot0.getLangContent(slot0)
	if slot0.contentMap[LangSettings.instance:getCurLangShortcut()] or slot0:getTemplateContent() then
		return slot2
	end
end
...
function slot0.getTemplateContent(slot0)
	...
	if slot0.params ~= "" then
		return GameUtil.getSubPlaceholderLuaLang(slot1.content, string.split(slot0.params, "#"))
	else
