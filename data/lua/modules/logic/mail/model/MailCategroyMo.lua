...
function MailCategroyMo:getLangTitle()
	...
	local title = self.titleMap[curLang] or self:getTemplateTitle()
...
function MailCategroyMo:getJumpLink()
	if self.mailId == 0 then
		...
		return self.jump and self.jump[curLang] or ""
	end
...
function MailCategroyMo:getTemplateJumpTitle()
	if self.mailId == 0 then
		...
		return self.jumpTitle and self.jumpTitle[curLang] or ""
	end
...
function MailCategroyMo:getLangSender()
	local curLang = LangSettings.instance:getCurLangShortcut()
	local sender = self.senderMap[curLang] or self:getTemplateSender()
...
function MailCategroyMo:getLangContent()
	local curLang = LangSettings.instance:getCurLangShortcut()
	local content = self.contentMap[curLang] or self:getTemplateContent()
