-- chunkname: @modules/logic/custompickchoice/view/SummonCustomPickItem.lua

module("modules.logic.custompickchoice.view.SummonCustomPickItem", package.seeall)

local SummonCustomPickItem = class("SummonCustomPickItem", LuaCompBase)

function SummonCustomPickItem:init(go)
	self._go = go
...
function SummonCustomPickItem:setClickCallBack(callBack, callbackObj)
	self._callBack = callBack
	self._callbackObj = callbackObj
end

SummonCustomPickItem.exSkillFillAmount = {
	0.2,
	0.4,
	0.6,
	0.79,
...
function SummonCustomPickItem:_onLongClickItem()
...
function SummonCustomPickItem:onClickSelf()
	logNormal("onClickChoice id = " .. tostring(self._mo.id))

	if self._callBack then
		self._callBack(self._callbackObj, self._mo.id)
	end
end

function SummonCustomPickItem:onUpdateMO(mo)
	self._mo = mo
...
function SummonCustomPickItem:refreshUI()
	local heroCo = HeroConfig.instance:getHeroCO(self._mo.id)

	if not heroCo then
		logError("SummonCustomPickItem.onUpdateMO error, heroConfig is nil, id:" .. tostring(self._mo.id))
...
function SummonCustomPickItem:refreshBaseInfo(heroCo)
	local skinConfig = SkinConfig.instance:getSkinCo(heroCo.skinId)

	if not skinConfig then
		logError("SummonCustomPickItem.onUpdateMO error, skinCfg is nil, id:" .. tostring(heroCo.skinId))
...
function SummonCustomPickItem:refreshExSkill()
...
function SummonCustomPickItem:onDestroy()
