-- chunkname: @modules/logic/versionactivity3_4/lusijian/view/LuSiJianTaskItem.lua

module("modules.logic.versionactivity3_4.lusijian.view.LuSiJianTaskItem", package.seeall)

local LuSiJianTaskItem = class("LuSiJianTaskItem", ListScrollCellExtend)

function LuSiJianTaskItem:onInitView()
...
function LuSiJianTaskItem:addEvents()
...
function LuSiJianTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function LuSiJianTaskItem:_btnnotfinishbgOnClick()
	if self._taskMo.config.jumpId > 0 then
		GameFacade.jump(self._taskMo.config.jumpId)
		ViewMgr.instance:closeView(ViewName.LuSiJianTaskView)
	end
end

function LuSiJianTaskItem:_btnfinishbgOnClick()
	self:_onOneClickClaimReward(self._taskMo.activityId)
	UIBlockHelper.instance:startBlock("LuSiJianTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinish, self, 0.5)
end

function LuSiJianTaskItem:_btngetallOnClick()
	LuSiJianController.instance:dispatchEvent(LuSiJianEvent.OneClickClaimReward, self._taskMo.activityId)
	UIBlockHelper.instance:startBlock("LuSiJianTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinishAll, self, 0.5)
end

function LuSiJianTaskItem:_delayFinish()
...
function LuSiJianTaskItem:_editableAddEvents()
	LuSiJianController.instance:registerCallback(LuSiJianEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function LuSiJianTaskItem:_editableRemoveEvents()
	LuSiJianController.instance:unregisterCallback(LuSiJianEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function LuSiJianTaskItem:_onOneClickClaimReward(actId)
...
function LuSiJianTaskItem:getAnimator()
	return self._animator
end

function LuSiJianTaskItem:onUpdateMO(mo)
	self._taskMo = mo

	local rankDiff = LuSiJianTaskListModel.instance:getRankDiff(mo)
...
function LuSiJianTaskItem:_moveByRankDiff(rankDiff)
...
function LuSiJianTaskItem:onSelect(isSelect)
	return
end

function LuSiJianTaskItem:_refreshUI()
...
function LuSiJianTaskItem:_onItemShow(cell_component, data, index)
...
function LuSiJianTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
