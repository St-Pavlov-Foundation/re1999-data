-- chunkname: @modules/logic/versionactivity4_0/sp_lilya/view/SpLilyaTaskItem.lua

module("modules.logic.versionactivity4_0.sp_lilya.view.SpLilyaTaskItem", package.seeall)

local SpLilyaTaskItem = class("SpLilyaTaskItem", ListScrollCellExtend)

function SpLilyaTaskItem:onInitView()
...
function SpLilyaTaskItem:addEvents()
...
function SpLilyaTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function SpLilyaTaskItem:_btnnotfinishbgOnClick()
	if self._taskMo.config.jumpId > 0 then
		GameFacade.jump(self._taskMo.config.jumpId)
		ViewMgr.instance:closeView(ViewName.SpLilyaTaskView)
	end
end

local BLOCK_KEY = "SpLilyaTaskItem_finishAnim"

function SpLilyaTaskItem:_btnfinishbgOnClick()
...
function SpLilyaTaskItem:_delayFinish()
	TaskRpc.instance:sendFinishTaskRequest(self._taskMo.config.id)
end

function SpLilyaTaskItem:_btngetallOnClick()
	self:_onOneClickClaimReward(self._taskMo.activityId)
	UIBlockHelper.instance:startBlock(BLOCK_KEY, 0.5)
	TaskDispatcher.runDelay(self._delayFinishAll, self, 0.5)
end

function SpLilyaTaskItem:_delayFinishAll()
	Activity220Controller.instance:finishAllTask(VersionActivity4_0Enum.ActivityId.SpLilya)
end

function SpLilyaTaskItem:_onOneClickClaimReward(actId)
...
function SpLilyaTaskItem:_editableInitView()
...
function SpLilyaTaskItem:onUpdateMO(mo)
	self._taskMo = mo
...
function SpLilyaTaskItem:_refreshUI()
...
function SpLilyaTaskItem:_onItemShow(cell_component, data, index)
...
function SpLilyaTaskItem:_moveByRankDiff(rankDiff)
...
function SpLilyaTaskItem:getAnimator()
	return self._animator
end

function SpLilyaTaskItem:onSelect(isSelect)
	return
end

function SpLilyaTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
