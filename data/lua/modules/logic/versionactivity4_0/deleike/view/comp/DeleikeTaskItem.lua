-- chunkname: @modules/logic/versionactivity4_0/deleike/view/comp/DeleikeTaskItem.lua

module("modules.logic.versionactivity4_0.deleike.view.comp.DeleikeTaskItem", package.seeall)

local DeleikeTaskItem = class("DeleikeTaskItem", ListScrollCellExtend)

function DeleikeTaskItem:onInitView()
...
function DeleikeTaskItem:addEvents()
...
function DeleikeTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function DeleikeTaskItem:_btnnotfinishbgOnClick()
	if self._taskMo.config.jumpId > 0 then
		GameFacade.jump(self._taskMo.config.jumpId)
		ViewMgr.instance:closeView(ViewName.DeleikeTaskView)
	end
end

local BLOCK_KEY = "DeleikeTaskItem_finishAnim"

function DeleikeTaskItem:_btnfinishbgOnClick()
...
function DeleikeTaskItem:_delayFinish()
	TaskRpc.instance:sendFinishTaskRequest(self._taskMo.config.id)
end

function DeleikeTaskItem:_btngetallOnClick()
	self:_onOneClickClaimReward(self._taskMo.activityId)
	UIBlockHelper.instance:startBlock(BLOCK_KEY, 0.5)
	TaskDispatcher.runDelay(self._delayFinishAll, self, 0.5)
end

function DeleikeTaskItem:_delayFinishAll()
	Activity220Controller.instance:finishAllTask(VersionActivity4_0Enum.ActivityId.Deleike)
end

function DeleikeTaskItem:_onOneClickClaimReward(actId)
...
function DeleikeTaskItem:_editableInitView()
...
function DeleikeTaskItem:onUpdateMO(mo)
	self._taskMo = mo
...
function DeleikeTaskItem:_refreshUI()
...
function DeleikeTaskItem:_onItemShow(cell_component, data, index)
...
function DeleikeTaskItem:_moveByRankDiff(rankDiff)
...
function DeleikeTaskItem:getAnimator()
	return self._animator
end

function DeleikeTaskItem:onSelect(isSelect)
	return
end

function DeleikeTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
