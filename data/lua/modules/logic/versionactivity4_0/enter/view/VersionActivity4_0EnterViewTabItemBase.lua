-- chunkname: @modules/logic/versionactivity4_0/enter/view/VersionActivity4_0EnterViewTabItemBase.lua

module("modules.logic.versionactivity4_0.enter.view.VersionActivity4_0EnterViewTabItemBase", package.seeall)

local VersionActivity4_0EnterViewTabItemBase = class("VersionActivity4_0EnterViewTabItemBase", VersionActivityEnterViewBaseTabItem)

function VersionActivity4_0EnterViewTabItemBase:_editableInitView()
...
function VersionActivity4_0EnterViewTabItemBase:_getTagPath()
	return "#go_tag"
end

function VersionActivity4_0EnterViewTabItemBase:afterSetData()
	if self.actId then
	...
	if not self.activityCo then
		...
		logError("VersionActivity4_0EnterViewTabItemBase.afterSetData error, no act config, actId:%s", self.actId)
	...
	if not string.nilorempty(selectBg) then
		UISpriteSetMgr.instance:setV4a0MainActivitySprite(self.imageSelectTabIcon, selectBg)
	end

	local unselectBg = iconCoArr[2]

	if not string.nilorempty(unselectBg) then
		UISpriteSetMgr.instance:setV4a0MainActivitySprite(self.imageUnSelectTabIcon, unselectBg)
	end

	self.redDotIcon = RedDotController.instance:addRedDot(self.goRedDot, self.activityCo.redDotId, self.redDotUid)
end

function VersionActivity4_0EnterViewTabItemBase:childRefreshSelect()
...
function VersionActivity4_0EnterViewTabItemBase:childRefreshUI()
...
function VersionActivity4_0EnterViewTabItemBase:refreshTag()
	self:clearTag()
...
function VersionActivity4_0EnterViewTabItemBase:clearTag()
...
function VersionActivity4_0EnterViewTabItemBase:refreshNormalTag()
	...
	local isShowRemainTime = VersionActivity4_0EnterHelper.GetIsShowTabRemainTime(self.actId)

	if isShowRemainTime and actInfoMo then
		local offsetSecond = actInfoMo:getRealEndTimeStamp() - ServerTime.now()

		if offsetSecond > VersionActivity4_0Enum.MaxShowTimeOffset then
			return
		end
...
function VersionActivity4_0EnterViewTabItemBase:refreshNotUnlockTag()
	gohelper.setActive(self.goTagLock, false)
...
function VersionActivity4_0EnterViewTabItemBase:refreshLockTag()
...
function VersionActivity4_0EnterViewTabItemBase:isShowRedDot()
	return self.redDotIcon and self.redDotIcon.show
end

return VersionActivity4_0EnterViewTabItemBase
