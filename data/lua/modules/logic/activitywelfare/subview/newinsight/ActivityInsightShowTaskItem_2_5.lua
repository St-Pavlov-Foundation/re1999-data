-- chunkname: @modules/logic/activitywelfare/subview/newinsight/ActivityInsightShowTaskItem_2_5.lua

module("modules.logic.activitywelfare.subview.newinsight.ActivityInsightShowTaskItem_2_5", package.seeall)

local ActivityInsightShowTaskItem_2_5 = class("ActivityInsightShowTaskItem_2_5", LuaCompBase)

function ActivityInsightShowTaskItem_2_5:init(go, index)
	self.go = go
	self._index = index
...
function ActivityInsightShowTaskItem_2_5:addEvents()
...
function ActivityInsightShowTaskItem_2_5:removeEvents()
	self._btngoto:RemoveClickListener()
	self._btncanget:RemoveClickListener()
	self._btnuse:RemoveClickListener()
end

function ActivityInsightShowTaskItem_2_5:_btngotoOnClick()
	if self._config.jumpId > 0 then
		GameFacade.jump(self._config.jumpId)
	end
end

function ActivityInsightShowTaskItem_2_5:_btncangetOnClick()
	TaskRpc.instance:sendFinishTaskRequest(self._config.id)
end

function ActivityInsightShowTaskItem_2_5:_btnuseOnClick()
	local data = {}
...
function ActivityInsightShowTaskItem_2_5:setTask(taskId)
	self._taskId = taskId

	self:refresh()
end

function ActivityInsightShowTaskItem_2_5:refresh()
...
function ActivityInsightShowTaskItem_2_5:_refreshBtns()
	...
	if self._taskMO.finishCount >= 1 then
		local activityId = ActivityEnum.Activity.V2a5_NewInsight
...
function ActivityInsightShowTaskItem_2_5:_refreshTaskRewards()
...
function ActivityInsightShowTaskItem_2_5:destroy()
	self:removeEvents()
end

return ActivityInsightShowTaskItem_2_5
