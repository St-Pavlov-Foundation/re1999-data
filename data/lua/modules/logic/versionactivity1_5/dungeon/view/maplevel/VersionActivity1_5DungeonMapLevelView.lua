...
slot0 = class("VersionActivity1_5DungeonMapLevelView", BaseView)
...
function slot0.refreshStoryIdList(slot0)
	...
	if slot0.originEpisodeConfig.beforeStory > 0 and StoryModel.instance:isStoryHasPlayed(slot0.originEpisodeConfig.beforeStory) then
		table.insert(slot0.storyIdList, slot0.originEpisodeConfig.beforeStory)
	end

	if slot0.originEpisodeConfig.afterStory > 0 and StoryModel.instance:isStoryHasPlayed(slot0.originEpisodeConfig.afterStory) then
		table.insert(slot0.storyIdList, slot0.originEpisodeConfig.afterStory)
	end
end
...
function slot0.initLocalEpisodeMode(slot0)
	slot0.unlockedEpisodeModeDict = slot0:loadDict(VersionActivity1_5EnterController.GetActivityPrefsKey(PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastUnLockMode))
	slot0.lastEpisodeSelectModeDict = slot0:loadDict(VersionActivity1_5EnterController.GetActivityPrefsKey(PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastSelectMode))
end
...
function slot0.refreshEye(slot0)
	...
	slot2 = slot0.originEpisodeConfig.chapterId == VersionActivity1_5DungeonEnum.DungeonChapterId.LeiMiTeBeiHard
...
function slot0._editableInitView(slot0)
	...
	slot0._simageactivitynormalbg:LoadImage(ResUrl.getV1a5DungeonSingleBg("v1a5_dungeon_bg_storypanel"))
	slot0._simageactivityhardbg:LoadImage(ResUrl.getV1a5DungeonSingleBg("v1a5_dungeon_bg_hardpanel"))
...
function slot0.getEpisodeIndex(slot0)
	return VersionActivity1_5DungeonController.instance:getEpisodeIndex(slot0.originEpisodeId)
end

function slot0.markSelectEpisode(slot0)
	if slot0.originEpisodeConfig.type == DungeonEnum.EpisodeType.Act1_5Dungeon or slot0.originEpisodeConfig.type == DungeonEnum.EpisodeType.Boss then
...
function slot0.refreshEpisodeTextInfo(slot0)
	...
	slot2 = slot0.mode == VersionActivityDungeonBaseEnum.DungeonMode.Hard and "#cfccc9" or "#cfccc9"
...
function slot0.refreshStar(slot0)
	slot0:setImage(slot0._imagestar1, slot0.showEpisodeCo.id and DungeonModel.instance:hasPassLevelAndStory(slot1), slot1)
	...
	if string.nilorempty(DungeonConfig.instance:getEpisodeAdvancedConditionText(slot1)) then
		...
		slot0:setImage(slot0._imagestar2, slot2 and DungeonEnum.StarType.Advanced <= slot0.showEpisodeMo.star, slot1)
	end
end
...
function slot0.refreshMode(slot0)
	...
	gohelper.setActive(gohelper.findChild(slot0.txtLockType.gameObject, "icon"), slot1)
end
...
function slot0.refreshStartBtn(slot0)
	...
	gohelper.setActive(slot0._btnreplayStory.gameObject, DungeonModel.instance:hasPassLevelAndStory(slot0.originEpisodeConfig.id) and slot0.storyIdList and #slot0.storyIdList > 0)
...
function slot0._playModeUnLockAnimation(slot0, slot1)
	slot2 = slot0.lockGoType:GetComponent(typeof(UnityEngine.Animator))
	slot2.enabled = true
...
function slot0.setImage(slot0, slot1, slot2, slot3)
	if slot2 then
		UISpriteSetMgr.instance:setV1a5DungeonSprite(slot1, VersionActivity1_5DungeonEnum.EpisodeStarType[DungeonConfig.instance:getEpisodeCO(slot3).chapterId])
	else
		UISpriteSetMgr.instance:setV1a5DungeonSprite(slot1, VersionActivity1_5DungeonEnum.EpisodeStarEmptyType[DungeonConfig.instance:getEpisodeCO(slot3).chapterId])
	end
end
...
function slot0.buildEpisodeName(slot0)
	...
	return slot0:buildColorText(string.format("<size=112>%s</size>%s", slot2, slot3), slot0.mode == VersionActivityDungeonBaseEnum.DungeonMode.Hard and "#cfccc9" or "#cfccc9")
end
...
function slot0.saveEpisodeUnlockMode(slot0)
	PlayerPrefsHelper.setString(slot0:getKey(VersionActivity1_5EnterController.GetActivityPrefsKey(PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastUnLockMode)), cjson.encode(slot0.unlockedEpisodeModeDict))
end

function slot0.saveEpisodeLastSelectMode(slot0)
	PlayerPrefsHelper.setString(slot0:getKey(VersionActivity1_5EnterController.GetActivityPrefsKey(PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastSelectMode)), cjson.encode(slot0.lastEpisodeSelectModeDict))
end
