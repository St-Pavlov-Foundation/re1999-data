...
function slot0.refreshCostTime(slot0)
	...
	slot0._txtcosttime.text = string.format("<color=%s>%s</color>", slot3, string.format("%s%s%s%s%s%s%s", luaLang("dispatch_cost_time"), Mathf.Floor(slot2 / TimeUtil.OneHourSecond), luaLang("time_hour"), Mathf.Floor(slot6 / TimeUtil.OneMinuteSecond), luaLang("time_minute"), slot6 % TimeUtil.OneMinuteSecond, luaLang("time_second")))
end
