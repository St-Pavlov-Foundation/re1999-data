...
function slot0._refreshRemainTime(slot0)
	slot2 = ActivityModel.instance:getActivityInfo()[VersionActivity1_5Enum.ActivityId.PeaceUlu]:getRealEndTimeStamp() - ServerTime.now()
	slot5 = Mathf.Floor(slot2 % TimeUtil.OneDaySecond / TimeUtil.OneHourSecond)

	if Mathf.Floor(slot2 / TimeUtil.OneDaySecond) >= 1 then
		slot0._txtremaintime.text = string.format(luaLang("remain"), string.format("%s%s%s%s", slot3, luaLang("time_day"), slot5, luaLang("time_hour2")))

		return
	end

	if slot5 >= 1 then
		slot0._txtremaintime.text = string.format(luaLang("remain"), slot5 .. luaLang("time_hour2"))

		return
	end

	if Mathf.Floor(slot4 % TimeUtil.OneHourSecond / TimeUtil.OneMinuteSecond) >= 1 then
		slot0._txtremaintime.text = string.format(luaLang("remain"), slot7 .. luaLang("time_minute2"))

		return
	end

	slot0._txtremaintime.text = string.format(luaLang("remain"), "<1" .. luaLang("time_minute2"))
end
...
function slot0._refreshSchedule(slot0)
	...
	slot6 = (268 - 40) * (slot0:_getnextIndex() - 1) - 174
