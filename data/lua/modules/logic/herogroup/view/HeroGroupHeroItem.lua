...
slot0 = class("HeroGroupHeroItem", LuaCompBase)
...
function slot0.initEquips(slot0, slot1)
	...
	if slot0.isLock or slot0.isAid or slot0.isRoleNumLock or not OpenModel.instance:isFunctionUnlock(OpenEnum.UnlockFunc.Equip) and not slot0.trialCO and not HeroGroupTrialModel.instance:haveTrialEquip() then
		...
		slot0._equipMO = EquipModel.instance:getEquip(HeroGroupModel.instance:getCurGroupMO():getPosEquips(slot0.mo.id - 1).equipUid[1]) or HeroGroupTrialModel.instance:getEquipMo(slot4)

		if HeroGroupModel.instance:getCurGroupMO().isReplay then
			slot0._equipMO = nil

			if HeroGroupModel.instance:getCurGroupMO().replay_equip_data[slot0.mo.heroUid] and EquipConfig.instance:getEquipCo(slot5.equipId) then
				slot0._equipMO = {
					config = slot6,
...
function slot0._onEndDrag(slot0, slot1, slot2)
	...
	slot0:_setToPos(slot0._equip.moveContainer.transform, recthelper.rectToRelativeAnchorPos(slot4._equipGO.transform.position, slot0._equipGO.transform), true, function ()
		EquipTeamListModel.instance:openTeamEquip(uv0.mo.id - 1, uv0._heroMO)
		...
		if (EquipModel.instance:getEquip(EquipTeamListModel.instance:getTeamEquip(uv0.mo.id - 1)[1]) or HeroGroupTrialModel.instance:getEquipMo(slot2)) and slot2 or nil then
			EquipTeamShowItem.removeEquip(slot0, true)
		end
		...
		if slot2 then
			EquipTeamShowItem.replaceEquip(slot1, slot2, true)
		end

		if slot3 then
			EquipTeamShowItem.replaceEquip(slot0, slot3, true)
		end
...
function slot0.resetGrayFactor(slot0)
	slot0._commonHeroCard:setGrayFactor(0)
end
