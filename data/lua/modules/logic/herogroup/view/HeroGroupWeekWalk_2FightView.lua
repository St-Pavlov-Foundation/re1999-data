...
slot0 = class("HeroGroupWeekWalk_2FightView", BaseView)
slot1 = 4
...
function slot0.onInitView(slot0)
	...
	slot0._btnTry = gohelper.findChildButtonWithAudio(slot0.viewGO, "#go_container/#go_topbtns/btn_Try")
	...
	slot0._gospace = gohelper.findChild(slot0.viewGO, "#go_container/btnContain/horizontal/#go_space")
	...
	slot0._godoubledroptimes = gohelper.findChild(slot0.viewGO, "#go_container/btnContain/horizontal/#go_doubletimes")
	slot0._txtdoubledroptimes = gohelper.findChildText(slot0.viewGO, "#go_container/btnContain/horizontal/#go_doubletimes/#txt_doubletimes")
...
function slot0.addEvents(slot0)
	...
	if slot0._btnTry then
		slot0._btnTry:AddClickListener(slot0._btnTryOnClick, slot0)
	end
...
function slot0.removeEvents(slot0)
	...
	if slot0._btnTry then
		slot0._btnTry:RemoveClickListener()
	end
...
function slot0._btnTryOnClick(slot0)
	if not slot0:_getTryoutCharacter() then
		return
	end
...
function slot0._getTryoutCharacter(slot0)
	if slot0._chapterConfig.actId <= 0 then
		return
	end

	if not ActivityConfig.instance:getActivityCo(slot0._chapterConfig.actId) or slot1.tryoutcharacter <= 0 then
		return
	end

	return slot1.tryoutcharacter
end
...
function slot0.onOpen(slot0)
	...
	if slot0._goTrialTips.activeSelf and not BossRushModel.instance:isSpecialLayerCurBattle() then
		gohelper.setActive(slot0._goTrialTipsBg, true)
	end

	if slot0._chapterConfig.type == DungeonEnum.ChapterType.TrialHero then
		if not slot0:_getTryoutCharacter() then
			return
		end

		if GameUtil.playerPrefsGetStringByUserId(PlayerPrefsKey.HeroGroupFightView_TrialHeroAlert .. slot5, "0") == "0" then
			GameUtil.playerPrefsSetStringByUserId(PlayerPrefsKey.HeroGroupFightView_TrialHeroAlert .. slot5, "1")
			TaskDispatcher.runDelay(slot0._btnTryOnClick, slot0, 0.5)
		end
	end
end

function slot0.refreshFightCount(slot0, slot1)
	if slot0:_getfreeCount() > 0 then
		slot0._txtfightCount.text = string.format("%s<color=#E45A57> -%s</color>", slot2, math.min(slot2, slot0._multiplication))
	end

	gohelper.setActive(slot0._gofightCount, slot2 > 0)

	if slot1 then
		recthelper.setAnchorX(slot0._gofightCount.transform, -410)
	else
		recthelper.setAnchorX(slot0._gofightCount.transform, -196)
	end
end
...
function slot0._setTrialNumTips(slot0)
	...
	if (HeroGroupTrialModel.instance:getLimitNum() or 0) > 0 then
		if slot4 >= 4 then
			-- Nothing
		else
			slot3[1] = formatLuaLang("herogroup_trial_limit_tip", slot4)
		end
	end
...
function slot0.setGroupChangeToast(slot0, slot1)
	slot0._changeToastId = slot1
end

function slot0._groupDropValueChanged(slot0, slot1)
	slot2 = nil
	slot2 = HeroGroupModel.instance:getGroupTypeName() and slot1 or slot1 + 1

	gohelper.setActive(slot0._btnmodifyname, slot2 ~= 0)

	if HeroGroupModel.instance:setHeroGroupSelectIndex(slot2) then
		slot0:_checkEquipClothSkill()
		GameFacade.showToast(slot0._changeToastId or ToastEnum.SeasonGroupChanged)
...
function slot0._initFightGroupDrop(slot0)
	...
	slot1 = {
		[slot5] = HeroGroupModel.instance:getCommonGroupName(slot5)
	}

	for slot5 = 1, 4 do
	end

	gohelper.setActive(slot0._btnmodifyname, HeroGroupModel.instance.curGroupSelectIndex ~= 0)
...
function slot0._refreshUI(slot0)
	...
	if slot0._btnTry then
		gohelper.setActive(slot0._btnTry, slot0._chapterConfig.type == DungeonEnum.ChapterType.TrialHero)
	end
...
function slot0._dispatchGuideEventOnOpenFinish(slot0)
	if slot0._episodeId then
		...
		HeroGroupController.instance:dispatchEvent(HeroGroupEvent.OpenHeroGroupFinishWithChapterId, slot0.episodeConfig.chapterId)
		HeroGroupController.instance:dispatchEvent(HeroGroupEvent.OpenHeroGroupFinishWithEpisodeType, slot0.episodeConfig.type)
	end
end

function slot0._dispatchNoEquipEvent(slot0)
	for slot5 = 1, 4 do
		if EquipModel.instance:getEquip(HeroGroupModel.instance:getCurGroupMO():getPosEquips(slot5 - 1).equipUid[1]) then
			return
		end
	end
...
function slot0.onClose(slot0)
	...
	TaskDispatcher.cancelTask(slot0._btnTryOnClick, slot0)
...
function slot0._refreshReplay(slot0)
	if slot0._chapterConfig.type == DungeonEnum.ChapterType.WeekWalk then
		...
		recthelper.setWidth(slot0._goReplayBtn.transform, slot0._replayMode and 416 or 83)
...
function slot0._refreshCloth(slot0)
	...
	slot4 = PlayerClothModel.instance:getById(PlayerClothModel.instance:getSpEpisodeClothID() or HeroGroupModel.instance:getCurGroupMO().clothId)
...
function slot0._checkFirstPosHasEquip(slot0)
	slot3 = HeroGroupModel.instance:getCurGroupMO():getPosEquips(0).equipUid and slot2[1]
...
function slot0._onClickStart(slot0)
	...
	slot0:_closemultcontent()
	slot0:_checkEnterFight()
end

function slot0._checkEnterFight(slot0)
	if not FightController.instance:setFightHeroSingleGroup() then
		return
	end

	if not WeekWalk_2Model.instance:getCurMapInfo():getBattleInfoByBattleId(HeroGroupModel.instance.battleId):getChooseSkillId() then
		MessageBoxController.instance:showMsgBox(MessageBoxIdDefine.WeekWalk_2NoBuff, MsgBoxEnum.BoxType.Yes_No, slot0._enterFight, nil, , slot0)
...
function slot0._refreshBtns(slot0, slot1)
	...
	gohelper.setActive(slot0._gospace, not slot0._replayMode and slot1)
...
function slot0.refreshDoubleDropTips(slot0)
	slot1, slot2, slot3 = DoubleDropModel.instance:isShowDoubleByEpisode(slot0._episodeId, true)

	gohelper.setActive(slot0._godoubledroptimes, slot1)

	if slot1 then
		slot0._txtdoubledroptimes.text = GameUtil.getSubPlaceholderLuaLang(luaLang("double_drop_remain_times"), {
			slot2,
			slot3
		})
	end
