-- chunkname: @modules/logic/herogroup/view/EnemyInfoView.lua

module("modules.logic.herogroup.view.EnemyInfoView", package.seeall)

local EnemyInfoView = class("EnemyInfoView", BaseViewExtended)

function EnemyInfoView:onInitView()
	...
	self._txthp = gohelper.findChildText(self.viewGO, "enemyinfo/hpbar/txt_hp/image_HPFrame/#txt_hp")
	...
	self._multiHpRoot = gohelper.findChild(self.viewGO, "enemyinfo/hpbar/txt_hp/image_HPFrame/image_HPBG")
	self._multiHpItem = gohelper.findChild(self.viewGO, "enemyinfo/hpbar/txt_hp/image_HPFrame/image_HPBG/image_HpItem")
...
function EnemyInfoView:addEvents()
...
function EnemyInfoView:removeEvents()
...
function EnemyInfoView:_editableInitView()
...
function EnemyInfoView:_onClosePassiveView()
	gohelper.setActive(self._gobuffpassiveview, false)

	self.isopenpassiveview = false
end

function EnemyInfoView:_refreshUI()
...
function EnemyInfoView:_refreshEnemyItem(i, monsterGroupIds, enemyGroupItemTable)
	...
	for j = 1, #monsterIds do
		...
		gohelper.setActive(enemyItemTable.bosstag, FightHelper.isBossId(monsterGroupConfig.bossId, monsterId))
...
function EnemyInfoView:_initEnemyItem(enemyItemGO, monsterId, groupIndex)
	...
	function enemyItemTable.onClickEnemyItem(enemyItemTable, clickInfo, isOpen)
		local isSelected = enemyItemTable._isSelected
...
function EnemyInfoView:_setEnemyItem(enemyItemTable, monsterConfig, skinConfig, groupIndex, j)
...
function EnemyInfoView:_refreshInfo(clickInfo)
	if clickInfo.index ~= self.selectMonsterIndex then
		...
		self._txtlevel.text = HeroConfig.instance:getLevelDisplayVariant(monsterConfig.level)
		...
		if self.viewParam.showHpQuestionMark then
			self._txthp.text = "????"
		else
			self._txthp.text = CharacterDataConfig.instance:getMonsterHp(monsterId)
		end
		...
		self:_detectBossMultiHp(monsterConfig)
	end
end

function EnemyInfoView:_getBossId(groupIndex)
	local monsterGroupId = FightModel.instance:getSelectMonsterGroupId(groupIndex)
...
function EnemyInfoView:_refreshSpeicalSkillIcon(monsterConfig)
...
function EnemyInfoView:_onBuffPassiveSkillClick()
...
function EnemyInfoView:_setPassiveSkillTip(skillgo, skillInfo)
...
function EnemyInfoView:_refreshPassiveSkill(monsterConfig, groupIndex)
	...
	if passiveSkillIds and #passiveSkillIds > 0 then
		...
		for i = 1, #passiveSkillIds do
			...
			local txt = FightConfig.instance:getSkillEffectDesc(nil, skillConfig)
...
function EnemyInfoView:_refreshSkill(monsterConfig)
	local skillIds = {}
...
function EnemyInfoView:_refreshSuper(monsterConfig)
...
function EnemyInfoView:createSuperItem()
...
function EnemyInfoView:_refreshAttribute(monsterConfig)
...
function EnemyInfoView:_onMonsterAttrItemShow(obj, data, index)
...
function EnemyInfoView:_setInfoShowState()
...
function EnemyInfoView:onUpdateParam()
...
function EnemyInfoView:_detectBossMultiHp(monsterConfig)
	local monsterTemplateConfig = lua_monster_template.configDict[monsterConfig.template]
	local multiHpNum = #FightConfig.instance:getMultiHpListByMonsterId(monsterConfig.id)

	gohelper.setActive(self._multiHpRoot, multiHpNum > 1)

	if multiHpNum > 1 then
		self:com_createObjList(nil, multiHpNum, self._multiHpRoot, self._multiHpItem)
	end
end

function EnemyInfoView:onClose()
	self:removeEventCb(HeroGroupController.instance, HeroGroupEvent.OnClickEnemyItem, self._refreshInfo, self)
end

function EnemyInfoView:onDestroyView()
