-- chunkname: @modules/logic/versionactivity3_5/lamona/view/LamonaTaskItem.lua

module("modules.logic.versionactivity3_5.lamona.view.LamonaTaskItem", package.seeall)

local LamonaTaskItem = class("LamonaTaskItem", ListScrollCellExtend)

function LamonaTaskItem:onInitView()
...
function LamonaTaskItem:addEvents()
...
function LamonaTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function LamonaTaskItem:_btnnotfinishbgOnClick()
	if self._taskMo.config.jumpId > 0 then
		GameFacade.jump(self._taskMo.config.jumpId)
		ViewMgr.instance:closeView(ViewName.LamonaTaskView)
	end
end

local BLOCK_KEY = "LamonaTaskItem_finishAnim"

function LamonaTaskItem:_btnfinishbgOnClick()
...
function LamonaTaskItem:_delayFinish()
	TaskRpc.instance:sendFinishTaskRequest(self._taskMo.config.id)
end

function LamonaTaskItem:_btngetallOnClick()
	self:_onOneClickClaimReward(self._taskMo.activityId)
	UIBlockHelper.instance:startBlock(BLOCK_KEY, 0.5)
...
function LamonaTaskItem:_editableInitView()
...
function LamonaTaskItem:onUpdateMO(mo)
	self._taskMo = mo
...
function LamonaTaskItem:_refreshUI()
...
function LamonaTaskItem:_onItemShow(cell_component, data, index)
...
function LamonaTaskItem:_moveByRankDiff(rankDiff)
...
function LamonaTaskItem:getAnimator()
	return self._animator
end

function LamonaTaskItem:onSelect(isSelect)
	return
end

function LamonaTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
