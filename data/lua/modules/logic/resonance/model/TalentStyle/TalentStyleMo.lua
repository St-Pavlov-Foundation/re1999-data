	...
	if string.nilorempty(slot0._tagStr) then
		...
		if not string.nilorempty(slot0._styleCo.tag) then
			for slot6, slot7 in ipairs(string.splitToNumber(slot1, "#")) do
				if string.nilorempty(slot0._tagStr) then
					slot0._tagStr = lua_character_attribute.configDict[slot7] and slot8.name or luaLang("talent_style_special_tag_" .. slot7)
				else
					slot0._tagStr = string.format("%s    %s", slot0._tagStr, slot9)
				end
			end
		end
	end
