...
function slot0.beforePlayEffectData(slot0)
	...
	slot0._powerId = slot0.actEffectData.configEffect
...
function slot0.onStart(slot0)
	...
	if not slot0._powerData then
		logError(string.format("找不到灵光数据,灵光id:%s, 角色或怪物id:%s, 步骤类型:%s, actId:%s", slot0._powerId, slot0._entityMO.modelId, slot0.fightStepData.actType, slot0.fightStepData.actId))
		slot0:onDone(true)
	...
	if slot0._oldValue ~= slot0._newValue then
		FightController.instance:dispatchEvent(FightEvent.PowerChange, slot0.actEffectData.targetId, slot0._powerId, slot0._oldValue, slot0._newValue)
	end
