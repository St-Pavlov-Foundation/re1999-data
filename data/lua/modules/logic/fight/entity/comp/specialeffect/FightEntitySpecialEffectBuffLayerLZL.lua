...
slot0 = class("FightEntitySpecialEffectBuffLayerLZL", FightEntitySpecialEffectBase)
...
function slot0.onBuffUpdate(slot0, slot1, slot2, slot3, slot4)
	...
	if not lua_fight_lzl_buff_float.configDict[slot6] then
		return
	end
	...
	if not slot7[slot9.layer] then
		logError(string.format("冷周六飘字表没找到buffTypeId = `%s`, layer = `%s` 的配置", slot6, slot10))
