...
function slot0._refreshContentData(slot0)
	...
	slot0._simageicon:LoadImage(ResUrl.getTechniqueLangIcon(slot0.cur_select_data.picture1, slot0.cur_select_data.isCn == 1))
		...
		if gohelper.findChild(slot0.viewGO, "#go_center/content/" .. slot7.id) then
			...
			if slot0.cur_select_data.id == slot7.id then
				for slot12, slot13 in ipairs(slot2) do
					gohelper.findChildTextMesh(slot8, "txt_" .. slot12).text = string.gsub(string.gsub(slot13, "%{", string.format("<color=%s>", "#ff906a")), "%}", "</color>")
				end
			end
		end
	end
