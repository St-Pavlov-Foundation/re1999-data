...
function slot0.onOpen(slot0)
	...
	gohelper.setActive(slot0._txtFbNameEn.gameObject, slot11)

	if slot11 then
		slot0:_setFbName(slot10)
	end

	slot12 = PlayerModel.instance:getExpNowAndMax()
	slot0._txtLv.text = "<size=36>LV </size>" .. PlayerModel.instance:getPlayerLevel()

	slot0._sliderExp:SetValue(slot12[1] / slot12[2])

	slot0._txtExp.text = slot12[1] .. "/" .. slot12[2]

	if slot1:getPlayerExp() and slot13 > 0 then
		gohelper.setActive(slot0._txtAddExp.gameObject, true)

		slot0._txtAddExp.text = "EXP+" .. slot13
	else
...
function slot0._setTag(slot0, slot1)
	...
	if slot1.chapterId == DungeonEnum.ChapterId.RoleDuDuGu then
		slot4 = false
	end
...
function slot0._onClickClose(slot0)
	...
	if FightModel.instance:getAfterStory() > 0 and (DungeonConfig.instance:getChapterCO(slot0._curChapterId).type == DungeonEnum.ChapterType.RoleStory or slot2.id == DungeonEnum.ChapterId.RoleDuDuGu or not StoryModel.instance:isStoryFinished(slot1)) then
		uv0._storyId = slot1
	...
	if slot3 then
		...
		if slot2.special then
			...
			if slot2.quantity then
				...
				slot0._txtdemand.text = string.format("%s%s<color=#81ce83>%s</color>", slot5.name, luaLang("fightsuccview_quantity"), GameUtil.numberDisplay(slot6))
