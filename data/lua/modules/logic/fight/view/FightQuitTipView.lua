...
function slot0._addRuleItemDesc(slot0, slot1, slot2, slot3)
	...
	gohelper.findChildText(slot5, "desc").text = string.format("<color=%s>[%s]</color>%s", ({
		"#6680bd",
		"#d05b4c",
		"#c7b376"
	})[slot2], luaLang("dungeon_add_rule_target_" .. slot2), string.gsub(slot1.desc, "%【(.-)%】", "<color=#6680bd>【%1】</color>"))
...
function slot0._refreshWeekwalkVer2Condition(slot0)
	...
	slot6 = uv0.sortWeekWalkVer2Task
...
function slot0._getWeekWalkVer2CupProgressDesc(slot0, slot1)
	...
	for slot15, slot16 in ipairs(GameUtil.splitString2(slot7[2], true, slot15, slot16)) do
