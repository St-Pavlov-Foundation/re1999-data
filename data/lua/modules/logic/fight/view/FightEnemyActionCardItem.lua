...
function slot0.init(slot0, slot1, slot2)
	...
	slot0.skillCardLv = FightCardDataHelper.getSkillLv(slot0.entityId, slot0.skillId)
	...
	for slot6 = 0, 4 do
		...
		slot0.lvGoList[slot6] = slot7
		slot0.lvImgIconList[slot6] = gohelper.findChildSingleImage(slot7, "imgIcon")
		slot0.lvImgCompList[slot6] = gohelper.findChildImage(slot7, "imgIcon")
	end

	slot0.goTag = gohelper.findChild(slot0.goCard, "tag")
	slot0.tagPosLevelDic = {}

	for slot6 = 0, 4 do
		slot7, slot8 = recthelper.getAnchor(gohelper.findChild(slot0.goCard, "tag/pos" .. slot6).transform)
		slot0.tagPosLevelDic[slot6] = {
			slot7,
			slot8
		}
	end
...
function slot0.refreshCard(slot0)
	for slot4, slot5 in pairs(slot0.lvGoList) do
	...
	for slot5, slot6 in pairs(slot0.lvImgIconList) do
	...
	gohelper.setActive(slot0.starGo, slot0.skillCardLv < FightEnum.UniqueSkillCardLv and slot0.skillCardLv > 0)

	slot0.starCanvas.alpha = 1

	for slot6, slot7 in ipairs(slot0.innerStartGoList) do
		gohelper.setActive(slot7, slot6 == slot0.skillCardLv)

		if slot0.innerStartCanvasList[slot6] then
			slot0.innerStartCanvasList[slot6].alpha = 1
		end
	end
	...
	if slot0.tagPosLevelDic[slot0.skillCardLv] then
		recthelper.setAnchor(slot0.tagRootTr, slot3[1], slot3[2])
	end
...
function slot0.destroy(slot0)
	for slot4, slot5 in pairs(slot0.lvImgIconList) do
		slot5:UnLoadImage()
	end
