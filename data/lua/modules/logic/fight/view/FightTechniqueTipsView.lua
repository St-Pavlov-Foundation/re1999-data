...
function slot0._refreshView(slot0)
	...
	slot0._simageicon:LoadImage(ResUrl.getTechniqueLangIcon(slot0.config.picture2 or "", slot0.config.isCn == 1))
		...
		if gohelper.findChild(slot0.viewGO, "#go_center/content/" .. slot6.id) then
			...
			if slot0.config.id == slot6.id then
				for slot11, slot12 in ipairs(slot1) do
					gohelper.findChildTextMesh(slot7, "txt_" .. slot11).text = string.gsub(string.gsub(slot12, "%{", string.format("<color=%s>", "#ff906a")), "%}", "</color>")
				end
			end
		end
	end
