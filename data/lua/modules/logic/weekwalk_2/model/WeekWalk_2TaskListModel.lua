...
slot0 = class("WeekWalk_2TaskListModel", ListScrollModel)
...
function slot0._canGet(slot0, slot1)
	if uv0.instance:getTaskMo(slot1) then
		if slot2.hasFinished and not (lua_task_weekwalk_ver2.configDict[slot1].maxFinishCount <= slot2.finishCount) then
			return true
		end
	end
end
...
function slot0.checkPeriods(slot0, slot1)
	...
	return WeekWalk_2Model.instance:getInfo() and (string.splitToNumber(slot1.periods, "#")[1] or 0) <= slot5.issueId and slot5.issueId <= (slot2[2] or 0)
end
...
function slot0.showLayerTaskList(slot0, slot1, slot2)
	...
	for slot12, slot13 in ipairs(WeekWalk_2Config.instance:getWeekWalkTaskList(slot1)) do
		if slot13.layerId == (lua_weekwalk_ver2.configDict[slot2] and slot6.layer or 0) and slot0:checkPeriods(slot13) then
...
function slot0.showTaskList(slot0, slot1, slot2)
	...
	table.sort(WeekWalk_2Config.instance:getWeekWalkTaskList(slot1), slot0._sort)
...
function slot0.canGetRewardNum(slot0, slot1, slot2)
	slot3 = WeekWalk_2Config.instance:getWeekWalkTaskList(slot1)
	slot4 = 0
	slot5 = 0
	slot6 = nil

	if slot2 then
		slot6 = lua_weekwalk_ver2.configDict[slot2].layer
	end
	...
	for slot10, slot11 in ipairs(slot3) do
		if uv0.instance:getTaskMo(slot11.id) then
			slot14 = lua_task_weekwalk_ver2.configDict[slot11.id].maxFinishCount <= slot12.finishCount
			slot15 = slot12.hasFinished

			if (not slot6 or slot11.layerId == slot6) and slot0:checkPeriods(slot11) then
				if not slot14 then
					slot5 = slot5 + 1
				end
...
function slot0.canGetReward(slot0, slot1)
	for slot6, slot7 in ipairs(WeekWalk_2Config.instance:getWeekWalkTaskList(slot1)) do
		if not uv0.instance:getTaskMo(slot7.id) then
			return false
		end

		if slot8.hasFinished and not (lua_task_weekwalk_ver2.configDict[slot7.id].maxFinishCount <= slot8.finishCount) then
			return true
		end
	end
end
...
function slot0._taskStatus(slot0, slot1)
	slot3 = lua_task_weekwalk_ver2.configDict[slot1.id]
...
function slot0.updateTaskList(slot0)
	slot0._taskList = TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.WeekWalk_2)
end
