...
slot0 = class("WeekWalk_2ResetBattleItem", ListScrollCellExtend)
...
function slot0._setStarStatus(slot0, slot1, slot2)
	gohelper.setActive(slot0._gostars2, WeekWalk_2Enum.MaxStar == 2)
	gohelper.setActive(slot0._gostars3, slot3 == 3)

	for slot7 = 1, slot3 do
		slot9 = slot7 <= slot1

		gohelper.setActive(gohelper.findChild(slot0[string.format("_go%sstar%s", slot3, slot7)], "full"), slot9)

		if slot9 then
			slot11 = slot10:GetComponent(gohelper.Type_Image)
			slot11.enabled = false

			WeekWalk_2Helper.setCupEffect(slot0._resetView:getResInst(slot0._resetView.viewContainer._viewSetting.otherRes.weekwalkheart_star, slot11.gameObject), slot2:getCupInfo(slot7))
		end
	end
end
...
function slot0._setNormalStatus(slot0)
	...
	slot0:_setStarStatus(slot0._battleInfo.star, slot0._battleInfo)
end
