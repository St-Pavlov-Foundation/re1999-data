...
function SurvivalScenePreloader:init(sceneId, levelId)
	...
	if not GameResMgr.IsFromEditorDir then
		...
		self._loader:addPath(self:getMapBlockCommonAbPath())
	else
...
function SurvivalScenePreloader:getBlockRes(group, prefabName)
	...
	if not GameResMgr.IsFromEditorDir and string.find(resPath, "survival/scenes/") then
		assetItem = self._loader:getAssetItem(abPath)
	else
		assetItem = self._loader:getAssetItem(resPath)
	end
