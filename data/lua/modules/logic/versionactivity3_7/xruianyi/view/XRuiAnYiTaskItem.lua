-- chunkname: @modules/logic/versionactivity3_7/xruianyi/view/XRuiAnYiTaskItem.lua

module("modules.logic.versionactivity3_7.xruianyi.view.XRuiAnYiTaskItem", package.seeall)

local XRuiAnYiTaskItem = class("XRuiAnYiTaskItem", ListScrollCellExtend)

function XRuiAnYiTaskItem:onInitView()
...
function XRuiAnYiTaskItem:addEvents()
...
function XRuiAnYiTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function XRuiAnYiTaskItem:_btnnotfinishbgOnClick()
	if self._taskMo.config.jumpId > 0 then
		GameFacade.jump(self._taskMo.config.jumpId)
		ViewMgr.instance:closeView(ViewName.XRuiAnYiTaskView)
	end
end

function XRuiAnYiTaskItem:_btnfinishbgOnClick()
	self:_onOneClickClaimReward(self._taskMo.activityId)
	UIBlockHelper.instance:startBlock("XRuiAnYiTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinish, self, 0.5)
end

function XRuiAnYiTaskItem:_btngetallOnClick()
	XRuiAnYiController.instance:dispatchEvent(XRuiAnYiEvent.OneClickClaimReward, self._taskMo.activityId)
	UIBlockHelper.instance:startBlock("XRuiAnYiTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(self._delayFinishAll, self, 0.5)
end

function XRuiAnYiTaskItem:_delayFinish()
...
function XRuiAnYiTaskItem:_editableAddEvents()
	XRuiAnYiController.instance:registerCallback(XRuiAnYiEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function XRuiAnYiTaskItem:_editableRemoveEvents()
	XRuiAnYiController.instance:unregisterCallback(XRuiAnYiEvent.OneClickClaimReward, self._onOneClickClaimReward, self)
end

function XRuiAnYiTaskItem:_onOneClickClaimReward(actId)
...
function XRuiAnYiTaskItem:getAnimator()
	return self._animator
end

function XRuiAnYiTaskItem:onUpdateMO(mo)
	self._taskMo = mo

	local rankDiff = XRuiAnYiTaskListModel.instance:getRankDiff(mo)
...
function XRuiAnYiTaskItem:_moveByRankDiff(rankDiff)
...
function XRuiAnYiTaskItem:onSelect(isSelect)
	return
end

function XRuiAnYiTaskItem:_refreshUI()
...
function XRuiAnYiTaskItem:_onItemShow(cell_component, data, index)
...
function XRuiAnYiTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
