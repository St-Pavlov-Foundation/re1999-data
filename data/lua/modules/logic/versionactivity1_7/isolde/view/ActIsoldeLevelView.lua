module("modules.logic.versionactivity1_7.isolde.view.ActIsoldeLevelView", package.seeall)

slot0 = class("ActIsoldeLevelView", BaseView)
...
function slot0.addEvents(slot0)
	...
	slot0._animEvent:AddEventListener(ActIsoldeEnum.AnimEvt.OnStoryOpenEnd, slot0._onStoryOpenEnd, slot0)
	slot0._animEvent:AddEventListener(ActIsoldeEnum.AnimEvt.OnFightOpenEnd, slot0._onFightOpenEnd, slot0)
	slot0._animEvent:AddEventListener(ActIsoldeEnum.AnimEvt.OnGoStoryEnd, slot0._onGoStoryEnd, slot0)
end
...
function slot0._btnTaskOnClick(slot0)
	ViewMgr.instance:openView(ViewName.ActIsoldeTaskView)
end
...
function slot0.onOpen(slot0)
	slot0:addEventCb(ActIsoldeController.instance, ActIsoldeEvent.StoryItemClick, slot0.OnStoryItemClick, slot0)
	slot0:addEventCb(ActIsoldeController.instance, ActIsoldeEvent.FightItemClick, slot0.OnFightItemClick, slot0)
	slot0:addEventCb(ActIsoldeController.instance, ActIsoldeEvent.TabSwitch, slot0.OnTabSwitch, slot0)
	...
	if slot0.viewParam and slot0.viewParam.needShowFight then
		if ActIsoldeModel.instance:getEnterFightIndex() then
		...
		if ActIsoldeModel.instance:getFirstEnter() then
			ActIsoldeModel.instance:clearFirstEnter()
			slot0:_lockScreen(true)
...
function slot0.OnEndDungeonPush(slot0)
	ActIsoldeModel.instance:updateData()
...
function slot0._initStageItems(slot0)
	...
	for slot7 = 1, #RoleActivityConfig.instance:getStoryLevelList(slot0.actId) do
		slot0.storyItemList[slot7] = MonoHelper.addNoUpdateLuaComOnceToGo(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[1], gohelper.findChild(slot0._gostoryStages, "stage" .. slot7)), IsoldeStoryItem, slot0)
	...
	for slot8 = 1, #RoleActivityConfig.instance:getBattleLevelList(slot0.actId) / 2 do
		slot0.fightItemList[slot8] = MonoHelper.addNoUpdateLuaComOnceToGo(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[2], gohelper.findChild(slot0._gofightStages, "stage" .. slot8)), IsoldeFightItem, slot0)
...
function slot0._playStoryFinishAnim(slot0)
	if ActIsoldeModel.instance:getNewFinishStoryLvl() then
		...
		ActIsoldeModel.instance:clearNewFinishStoryLvl()
	end
end
...
function slot0._playFightFinishAnim(slot0)
	...
	if ActIsoldeModel.instance:getNewFinishFightLvl() then
		for slot5, slot6 in ipairs(slot0.fightItemList) do
			if slot6.id == slot1 then
		...
		ActIsoldeModel.instance:clearNewFinishFightLvl()
...
function slot0._lockScreen(slot0, slot1)
	if slot1 then
		...
		UIBlockMgr.instance:startBlock("IsoldeLock")
	else
		UIBlockMgr.instance:endBlock("IsoldeLock")
