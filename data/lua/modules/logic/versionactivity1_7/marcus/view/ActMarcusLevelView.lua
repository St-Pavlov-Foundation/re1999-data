...
slot0 = class("ActMarcusLevelView", BaseView)
...
function slot0.addEvents(slot0)
	...
	slot0._animEvent:AddEventListener(ActMarcusEnum.AnimEvt.OnStoryOpenEnd, slot0._onStoryOpenEnd, slot0)
	slot0._animEvent:AddEventListener(ActMarcusEnum.AnimEvt.OnFightOpenEnd, slot0._onFightOpenEnd, slot0)
	slot0._animEvent:AddEventListener(ActMarcusEnum.AnimEvt.OnGoStoryEnd, slot0._onGoStoryEnd, slot0)
end
...
function slot0._editableInitView(slot0)
	slot0.actId = VersionActivity1_7Enum.ActivityId.Marcus
...
function slot0.onOpen(slot0)
	slot0:addEventCb(ActMarcusController.instance, ActMarcusEvent.StoryItemClick, slot0.OnStoryItemClick, slot0)
	slot0:addEventCb(ActMarcusController.instance, ActMarcusEvent.FightItemClick, slot0.OnFightItemClick, slot0)
	slot0:addEventCb(ActMarcusController.instance, ActMarcusEvent.TabSwitch, slot0.OnTabSwitch, slot0)
	...
	if slot0.viewParam and slot0.viewParam.needShowFight then
		if ActMarcusModel.instance:getEnterFightIndex() then
...
function slot0._initStageItems(slot0)
	...
	for slot7 = 1, #RoleActivityConfig.instance:getStoryLevelList(slot0.actId) do
		slot0.storyItemList[slot7] = MonoHelper.addNoUpdateLuaComOnceToGo(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[1], gohelper.findChild(slot0._gostoryStages, "stage" .. slot7)), MarcusStoryItem, slot0)
	...
	for slot8 = 1, #RoleActivityConfig.instance:getBattleLevelList(slot0.actId) / 2 do
		slot0.fightItemList[slot8] = MonoHelper.addNoUpdateLuaComOnceToGo(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[2], gohelper.findChild(slot0._gofightStages, "stage" .. slot8)), MarcusFightItem, slot0)
...
function slot0._playStoryFinishAnim(slot0)
	if ActMarcusModel.instance:getNewFinishStoryLvl() then
...
function slot0._playFightFinishAnim(slot0)
	...
	if ActMarcusModel.instance:getNewFinishFightLvl() then
		for slot5, slot6 in ipairs(slot0.fightItemList) do
			if slot6.id == slot1 then
