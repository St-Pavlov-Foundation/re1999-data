...
function var_0_0.getStoreChargeConfig(arg_17_0, arg_17_1, arg_17_2)
	local var_17_0 = arg_17_0._storeChargeConfig.configDict[arg_17_1][arg_17_2]
	...
	if arg_28_1.refreshTime == StoreEnum.RefreshTime.Forever then
		if var_28_0 > 0 then
			...
			elseif arg_28_3 > 0 then
				return string.format("%s:%d", luaLang("store_buylimit"), arg_28_2)
			else
				return string.format("%s:%d", luaLang("store_buylimit_forever"), arg_28_2)
			end
		else
			return nil
		end
	elseif arg_28_1.refreshTime == StoreEnum.RefreshTime.Day then
		return string.format("%s:%d", luaLang("store_buylimit_day"), arg_28_2)
	elseif arg_28_1.refreshTime == StoreEnum.RefreshTime.Week then
		return string.format("%s:%d", luaLang("store_buylimit_week"), arg_28_2)
	elseif arg_28_1.refreshTime == StoreEnum.RefreshTime.Month then
		return string.format("%s:%d", luaLang("store_buylimit_month"), arg_28_2)
	else
		return string.format("%s:%d", luaLang("store_buylimit"), arg_28_2)
	end
end
...
function var_0_0.getRemainText(arg_29_0, arg_29_1, arg_29_2, arg_29_3, arg_29_4)
	if arg_29_2 == StoreEnum.RefreshTime.Forever then
		if arg_29_1 > 0 then
			if arg_29_4 > 0 then
				return string.format("%s: %d", luaLang("store_buylimit"), arg_29_3)
			else
				return string.format("%s: %d", luaLang("store_buylimit_forever"), arg_29_3)
			end
		else
			return nil
		end
	elseif arg_29_2 == StoreEnum.RefreshTime.Day then
		return string.format("%s: %d", luaLang("store_buylimit_day"), arg_29_3)
	elseif arg_29_2 == StoreEnum.RefreshTime.Week then
		return string.format("%s: %d", luaLang("store_buylimit_week"), arg_29_3)
	elseif arg_29_2 == StoreEnum.RefreshTime.Month then
		return string.format("%s: %d", luaLang("store_buylimit_month"), arg_29_3)
	else
		return string.format("%s: %d", luaLang("store_buylimit"), arg_29_3)
	end
end
...
function var_0_0.getChargeRemainText(arg_30_0, arg_30_1, arg_30_2, arg_30_3, arg_30_4)
	if arg_30_2 == StoreEnum.ChargeRefreshTime.Forever then
		if arg_30_1 > 0 then
			if arg_30_4 > 0 then
				return string.format("%s: %d", luaLang("store_buylimit"), arg_30_3)
			else
				return string.format("%s: %d", luaLang("store_buylimit_forever"), arg_30_3)
			end
		else
			return nil
		end
	...
	elseif arg_30_2 == StoreEnum.ChargeRefreshTime.Day then
		return string.format("%s: %d", luaLang("store_buylimit_day"), arg_30_3)
	elseif arg_30_2 == StoreEnum.ChargeRefreshTime.Week then
		return string.format("%s: %d", luaLang("store_buylimit_week"), arg_30_3)
	elseif arg_30_2 == StoreEnum.ChargeRefreshTime.Month then
		return string.format("%s: %d", luaLang("store_buylimit_month"), arg_30_3)
	else
		return string.format("%s: %d", luaLang("store_buylimit"), arg_30_3)
	end
end
