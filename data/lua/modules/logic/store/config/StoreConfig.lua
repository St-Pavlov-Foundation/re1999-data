...
function slot0.getStoreChargeConfig(slot0, slot1, slot2)
	if not slot0._storeChargeConfig.configDict[slot1][slot2] then
...
function slot0.getRemain(slot0, slot1, slot2, slot3)
	...
	if slot1.refreshTime == StoreEnum.RefreshTime.Forever then
		if slot1.maxBuyCount > 0 then
			...
			elseif slot3 > 0 then
				return string.format("%s:%d", luaLang("store_buylimit"), slot2)
			else
				return string.format("%s:%d", luaLang("store_buylimit_forever"), slot2)
			end
		else
			return nil
		end
	elseif slot1.refreshTime == StoreEnum.RefreshTime.Day then
		return string.format("%s:%d", luaLang("store_buylimit_day"), slot2)
	elseif slot1.refreshTime == StoreEnum.RefreshTime.Week then
		return string.format("%s:%d", luaLang("store_buylimit_week"), slot2)
	elseif slot1.refreshTime == StoreEnum.RefreshTime.Month then
		return string.format("%s:%d", luaLang("store_buylimit_month"), slot2)
	else
		return string.format("%s:%d", luaLang("store_buylimit"), slot2)
	end
end
...
function slot0.getRemainText(slot0, slot1, slot2, slot3, slot4)
	if slot2 == StoreEnum.RefreshTime.Forever then
		if slot1 > 0 then
			if slot4 > 0 then
				return string.format("%s: %d", luaLang("store_buylimit"), slot3)
			else
				return string.format("%s: %d", luaLang("store_buylimit_forever"), slot3)
			end
		else
			return nil
		end
	elseif slot2 == StoreEnum.RefreshTime.Day then
		return string.format("%s: %d", luaLang("store_buylimit_day"), slot3)
	elseif slot2 == StoreEnum.RefreshTime.Week then
		return string.format("%s: %d", luaLang("store_buylimit_week"), slot3)
	elseif slot2 == StoreEnum.RefreshTime.Month then
		return string.format("%s: %d", luaLang("store_buylimit_month"), slot3)
	else
		return string.format("%s: %d", luaLang("store_buylimit"), slot3)
	end
end
