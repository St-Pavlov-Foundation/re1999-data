...
function var_0_0.getStoreChargeConfig(arg_20_0, arg_20_1, arg_20_2)
	local var_20_0 = arg_20_0._storeChargeConfig.configDict[arg_20_1][arg_20_2]
	...
	if arg_31_1.refreshTime == StoreEnum.RefreshTime.Forever then
		if var_31_0 > 0 then
			...
			elseif arg_31_3 > 0 then
				return string.format("%s:%d", luaLang("store_buylimit"), arg_31_2)
			else
				return string.format("%s:%d", luaLang("store_buylimit_forever"), arg_31_2)
			end
		else
			return nil
		end
	elseif arg_31_1.refreshTime == StoreEnum.RefreshTime.Day then
		return string.format("%s:%d", luaLang("store_buylimit_day"), arg_31_2)
	elseif arg_31_1.refreshTime == StoreEnum.RefreshTime.Week then
		return string.format("%s:%d", luaLang("store_buylimit_week"), arg_31_2)
	elseif arg_31_1.refreshTime == StoreEnum.RefreshTime.Month then
		return string.format("%s:%d", luaLang("store_buylimit_month"), arg_31_2)
	else
		return string.format("%s:%d", luaLang("store_buylimit"), arg_31_2)
	end
end
...
function var_0_0.getRemainText(arg_32_0, arg_32_1, arg_32_2, arg_32_3, arg_32_4)
	if arg_32_2 == StoreEnum.RefreshTime.Forever then
		if arg_32_1 > 0 then
			if arg_32_4 > 0 then
				return string.format("%s: %d", luaLang("store_buylimit"), arg_32_3)
			else
				return string.format("%s: %d", luaLang("store_buylimit_forever"), arg_32_3)
			end
		else
			return nil
		end
	elseif arg_32_2 == StoreEnum.RefreshTime.Day then
		return string.format("%s: %d", luaLang("store_buylimit_day"), arg_32_3)
	elseif arg_32_2 == StoreEnum.RefreshTime.Week then
		return string.format("%s: %d", luaLang("store_buylimit_week"), arg_32_3)
	elseif arg_32_2 == StoreEnum.RefreshTime.Month then
		return string.format("%s: %d", luaLang("store_buylimit_month"), arg_32_3)
	else
		return string.format("%s: %d", luaLang("store_buylimit"), arg_32_3)
	end
end
...
function var_0_0.getChargeRemainText(arg_33_0, arg_33_1, arg_33_2, arg_33_3, arg_33_4)
	if arg_33_2 == StoreEnum.ChargeRefreshTime.Forever then
		if arg_33_1 > 0 then
			if arg_33_4 > 0 then
				return string.format("%s: %d", luaLang("store_buylimit"), arg_33_3)
			else
				return string.format("%s: %d", luaLang("store_buylimit_forever"), arg_33_3)
			end
		else
			return nil
		end
	...
	elseif arg_33_2 == StoreEnum.ChargeRefreshTime.Day then
		return string.format("%s: %d", luaLang("store_buylimit_day"), arg_33_3)
	elseif arg_33_2 == StoreEnum.ChargeRefreshTime.Week then
		return string.format("%s: %d", luaLang("store_buylimit_week"), arg_33_3)
	elseif arg_33_2 == StoreEnum.ChargeRefreshTime.Month then
		return string.format("%s: %d", luaLang("store_buylimit_month"), arg_33_3)
	else
		return string.format("%s: %d", luaLang("store_buylimit"), arg_33_3)
	end
end
