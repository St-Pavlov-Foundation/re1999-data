...
function var_0_0.initCharge(arg_1_0, arg_1_1, arg_1_2)
	...
	arg_1_0.cost = arg_1_0.config.price
...
function var_0_0.init(arg_2_0, arg_2_1, arg_2_2, arg_2_3, arg_2_4)
	...
	if arg_2_4 == nil then
		if string.nilorempty(arg_2_0.config.offlineTime) then
			arg_2_0.offlineTime = 0
		else
			arg_2_0.offlineTime = tonumber(arg_2_0.config.offlineTime) / 1000
		end
	end
...
function var_0_0.initRedDotTime(arg_3_0)
	if string.nilorempty(arg_3_0.config.newStartTime) then
		...
		arg_3_0.newStartTime = TimeUtil.stringToTimestamp(arg_3_0.config.newStartTime) - ServerTime.clientToServerOffset()
	end

	if string.nilorempty(arg_3_0.config.newEndTime) then
		arg_3_0.newEndTime = 0
	else
		arg_3_0.newEndTime = TimeUtil.stringToTimestamp(arg_3_0.config.newEndTime) - ServerTime.clientToServerOffset()
	end
end
...
function var_0_0.getDiscount(arg_7_0)
	if arg_7_0._offInfos[1] == StoreEnum.Discount.Discount then
		return arg_7_0._offInfos[2] or 0
	end

	return 0
end

function var_0_0.needShowNew(arg_8_0)
	if arg_8_0:isSoldOut() then
		return false
	else
		local var_8_0 = ServerTime.now()
		local var_8_1 = var_8_0 >= arg_8_0.newStartTime and var_8_0 <= arg_8_0.newEndTime

		return RedDotModel.instance:isDotShow(RedDotEnum.DotNode.StoreChargeGoodsRead, arg_8_0.goodsId) and var_8_1
	end
end
