-- chunkname: @modules/logic/store/view/decorate/DecorateGoodsItem.lua

module("modules.logic.store.view.decorate.DecorateGoodsItem", package.seeall)
...
function DecorateGoodsItem:init(go, mo)
	...
	self._imagebanner = gohelper.findChildImage(self._goroot, "#go_Item/#go_has/#simage_banner")
	self._imagerareicon = gohelper.findChildImage(self._goroot, "#go_Item/#go_has/#simage_icon")
	self._txtname = gohelper.findChildText(self._goroot, "#go_Item/#go_has/#txt_name")
...
function DecorateGoodsItem:_initItems()
...
function DecorateGoodsItem:_refreshDetail()
	local curGoodId = DecorateStoreModel.instance:getCurGood(self._mo.belongStoreId)
	local curDecorateConfig = DecorateStoreConfig.instance:getDecorateConfig(curGoodId)
	local isBundleSubGood = curDecorateConfig.fatherGoods == self._mo.goodsId
	local curGoodMo = StoreModel.instance:getGoodsMO(curDecorateConfig.fatherGoods)
	...
	local iconImg = ResUrl.getDecorateStoreImg(self._decorateConfig.smalllmg)

	if self._decorateConfig.subType == ItemEnum.SubType.PlayerBg then
		iconImg = ResUrl.getPlayerCardIcon("banner/" .. self._decorateConfig.smalllmg)
	end

	if self._decorateConfig.rare > 0 then
		gohelper.setActive(self._simagerareicon.gameObject, true)
		self._simagerareicon:LoadImage(iconImg)
