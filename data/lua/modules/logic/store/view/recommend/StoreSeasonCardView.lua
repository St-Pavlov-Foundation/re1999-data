...
function slot0._editableInitView(slot0)
	...
	if PayModel.getProductOriginPriceSymbol then
		...
		if string.nilorempty(PayModel.instance:getProductOriginPriceSymbol(uv0)) then
			...
			slot0._txtcosticon.text = string.format("%s<size=30>%s</size>", string.sub(slot3, 1, slot6), string.sub(slot3, slot6 + 1, string.len(slot3)))
		else
			slot0._txtcosticon.text = string.format("<size=30>%s</size>%s", slot1, slot3)
		end
	end
end
...
function slot0._refreshRemainDay(slot0)
	if slot0._seasonCardInfo then
		...
		elseif slot1 == StoreEnum.MonthCardStatus.NotEnoughOneDay then
			if slot0._seasonCardInfo.hasGetBonus then
				slot0._txtlefttimetips.text = luaLang("StoreSeasonCardView_not_enough_one_day")
			else
				slot0._txtlefttimetips.text = luaLang("not_enough_one_day")
			end
		else
