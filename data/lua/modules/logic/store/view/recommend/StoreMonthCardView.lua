...
function StoreMonthCardView:refreshRemainDay()
	if self.monthCardInfo ~= nil then
		...
		elseif remainDay == StoreEnum.MonthCardStatus.NotEnoughOneDay then
			self._txtlefttimetips.text = luaLang("not_enough_one_day") .. (self.monthCardInfo.hasGetBonus and luaLang("today_reward") or "")
		else
			self._txtlefttimetips.text = formatLuaLang("remain_day", remainDay) .. (self.monthCardInfo.hasGetBonus and luaLang("today_reward") or "")
		end
	else
		self._txtlefttimetips.text = luaLang("not_purchase")
	end
