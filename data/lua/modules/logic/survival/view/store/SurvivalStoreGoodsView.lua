...
function SurvivalStoreGoodsView:_editableInitView()
	...
	self:addEventCb(SurvivalStoreController.instance, SurvivalEvent.OnSurvivalOutsideShopBuyReply, self.onBuyGoodsSuccess, self)
end
...
function SurvivalStoreGoodsView:refreshUI()
	...
	if self.itemType == MaterialEnum.MaterialType.Hero then
		self._txtgoodsDesc.text = ItemModel.instance:getItemConfig(self.itemType, self.itemId).desc2
	else
		self._txtgoodsDesc.text = ItemModel.instance:getItemConfig(self.itemType, self.itemId).desc
	end
