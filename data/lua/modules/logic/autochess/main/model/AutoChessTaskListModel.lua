-- chunkname: @modules/logic/autochess/main/model/AutoChessTaskListModel.lua

module("modules.logic.autochess.main.model.AutoChessTaskListModel", package.seeall)

local AutoChessTaskListModel = class("AutoChessTaskListModel", ListScrollModel)

function AutoChessTaskListModel:init(actId)
	local taskDict = TaskModel.instance:getAllUnlockTasks(TaskEnum.TaskType.AutoChess)
	local dataList = {}
	local rewardCount = 0

	if taskDict ~= nil then
		local taskCfgList = AutoChessConfig.instance:getTaskByActId(actId)

		for _, taskCfg in ipairs(taskCfgList) do
			local mo = AutoChessTaskMO.New()
	...
	if rewardCount > 1 then
		local allMO = AutoChessTaskMO.New()

		allMO:initAllMo(actId)
		table.insert(dataList, allMO)
	end

	table.sort(dataList, AutoChessTaskListModel.sortMO)
...
function AutoChessTaskListModel.sortMO(objA, objB)
	local sidxA = AutoChessTaskListModel.getSortIndex(objA)
	local sidxB = AutoChessTaskListModel.getSortIndex(objB)
...
function AutoChessTaskListModel:createMO(co, taskMO)
	local mo = {}
...
function AutoChessTaskListModel:getRankDiff(mo)
...
function AutoChessTaskListModel:refreshRankDiff()
	self._idIdxList = {}
...
function AutoChessTaskListModel:preFinish(taskMO)
	if not taskMO then
		return
	end
	...
	if isCanSort then
		...
		table.sort(taskMOList, AutoChessTaskListModel.sortMO)
...
function AutoChessTaskListModel:getGotRewardCount(moList)
...
AutoChessTaskListModel.instance = AutoChessTaskListModel.New()

return AutoChessTaskListModel
