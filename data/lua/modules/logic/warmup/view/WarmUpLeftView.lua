-- chunkname: @modules/logic/warmup/view/WarmUpLeftView.lua

module("modules.logic.warmup.view.WarmUpLeftView", package.seeall)

local WarmUpLeftView = class("WarmUpLeftView", BaseView)

function WarmUpLeftView:onInitView()
...
function WarmUpLeftView:_editableInitView()
...
function WarmUpLeftView:onDataUpdateFirst()
...
function WarmUpLeftView:onDataUpdate()
	self:_refresh()
end

function WarmUpLeftView:onSwitchEpisode()
	local isDone = self:_checkIsDone()
...
function WarmUpLeftView:_refresh()
	...
	if isDone then
		...
		elseif States.SwipeDone == state then
			...
			logError("[WarmUpLeftView] invalid state: " .. tostring(state))
			self:_setActive_dayGo(nil)
		end
	end
end

function WarmUpLeftView:onClose()
	GameUtil.onDestroyViewMember(self, "_drag")
	self._animEvent:RemoveAllEventListener()
end

function WarmUpLeftView:onDestroyView()
	GameUtil.onDestroyViewMember(self, "_drag")
end

function WarmUpLeftView:_setActive_dayGo(index)
	for i, go in ipairs(self._dayItemList) do
		gohelper.setActive(go, index == i)
	end
end

function WarmUpLeftView:_setActive_dayGoCur()
	self:_setActive_dayGo(self:_episode2Index())
end

function WarmUpLeftView:_onDragBegin()
	self:_setActive_guide(false)
end

function WarmUpLeftView:_onDragEnd()
...
function WarmUpLeftView:_playAnimIdle(cb, cbObj)
	self:_playAnim("idleclose", cb, cbObj)
end

function WarmUpLeftView:_playAnimOpened(cb, cbObj)
	self:_playAnim("idleopen", cb, cbObj)
end

function WarmUpLeftView:_playAnimOpen(cb, cbObj)
	self:_playAnim(UIAnimationName.Open, cb, cbObj)
end

function WarmUpLeftView:_playAnim(name, cb, cbObj)
	self._animatorPlayer:Play(name, cb or function()
		return
	end, cbObj)
end

function WarmUpLeftView:_playAnimAfterSwipe()
	self:_setActive_drag(false)
...
local kBlock_Click = "WarmUpLeftView:kBlock_Click"
local kTimeout = 9.99

function WarmUpLeftView:_playAnimAfterSwiped()
...
function WarmUpLeftView:_play_ui_shengyan_item_appeared()
	return
end

function WarmUpLeftView:_play_ui_shengyan_unsheathe_dagger()
	return
end

function WarmUpLeftView:_play_ui_fuleyuan_yure_whoosh()
	return
end

return WarmUpLeftView
