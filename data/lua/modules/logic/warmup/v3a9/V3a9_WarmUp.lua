-- chunkname: @modules/logic/warmup/v3a9/V3a9_WarmUp.lua

module("modules.logic.warmup.v3a9.V3a9_WarmUp", package.seeall)

local V3a9_WarmUp = class("V3a9_WarmUp", BaseView)

function V3a9_WarmUp:onInitView()
...
function V3a9_WarmUp:_btngetrewardOnClick()
...
function V3a9_WarmUp:_editableInitView()
...
function V3a9_WarmUp:onDataUpdateFirst()
	self:_refreshOnce()
end

function V3a9_WarmUp:onDataUpdate()
	self:_refresh()
end

function V3a9_WarmUp:onSwitchEpisode()
...
function V3a9_WarmUp:onUpdateActivity()
	self._descScrollRect:StopMovement()
	self:_setTaskContentToEnd()
	self:_refresh()
end

function V3a9_WarmUp:onUpdateParam()
	self:_refreshOnce()
	self:_refresh()
end

function V3a9_WarmUp:onOpen()
...
function V3a9_WarmUp:_refreshRightView()
...
function V3a9_WarmUp:_setActive_goWrongChannel(isBlockDesc)
...
function V3a9_WarmUp:_refreshData()
...
function V3a9_WarmUp:_setSelectIndex(index, isFocus)
...
function V3a9_WarmUp:_taskScrollToIndex(index)
...
function V3a9_WarmUp:onClickTab(mo)
	...
	AudioMgr.instance:trigger(AudioEnum.UI.stop_ui_chongran_yure_walk_loop)
...
function V3a9_WarmUp:_refreshRewards()
	local co = self.viewContainer:getEpisodeConfigCur()
...
function V3a9_WarmUp:openDesc(cb, cbObj)
	...
	AudioMgr.instance:trigger(AudioEnum.UI.play_ui_chongran_yure_walk_loop)
...
function V3a9_WarmUp:_tweenDescUpdateCb(value)
	local bottom = lerp(0, self._taskDescViewportHeight, value)

	self:_setMaskPaddingBottom(bottom)
end

function V3a9_WarmUp:_tweenDescEndCb(maskDurationTime, cb, cbObj)
...
function V3a9_WarmUp:_resetTaskContentPos()
	recthelper.setAnchorY(self._txtTaskContentTran, 0)
end

function V3a9_WarmUp:episode2Index(episodeId)
	return episodeId
end

function V3a9_WarmUp:index2EpisodeId(index)
...
function V3a9_WarmUp:_setMaskPaddingBottom(bottom)
	self._taskDescMask.padding = Vector4(0, bottom, 0, 0)
end

function V3a9_WarmUp:_autoSelectTab()
...
function V3a9_WarmUp:_create_WarmUp_radiotaskitem(index)
	local go = gohelper.cloneInPlace(self._goradiotaskitem)
	local item = V3a9_WarmUp_radiotaskitem.New({
		parent = self,
...
function V3a9_WarmUp:_create_WarmUp_rewarditem(index)
	local go = gohelper.cloneInPlace(self._gorewarditem)
	local item = V3a9_WarmUp_rewarditem.New({
		parent = self,
		baseViewContainer = self.viewContainer
	})
...
function V3a9_WarmUp:_resetTweenDescPos()
	GameUtil.onDestroyViewMember_TweenId(self, "_movetweenId")
	GameUtil.onDestroyViewMember_TweenId(self, "_tweenId")
	self:_resetTaskContentPos()
end

function V3a9_WarmUp:_playAnim(name, cb, cbObj)
	self._animatorPlayer:Play(name, cb, cbObj)
end

function V3a9_WarmUp:tweenSwitch(cb, cbObj)
	self:_playAnim(UIAnimationName.Switch, cb, cbObj)
end

function V3a9_WarmUp:_onSwitch()
	local lastEpisodeId = self.viewContainer:getCurSelectedEpisode()
	local curEpisodeId
...
function V3a9_WarmUp:playRewardItemsHasGetAnim()
...
function V3a9_WarmUp:_setTaskContentToEnd()
	recthelper.setAnchorY(self._txtTaskContentTran, self:_getTaskContentEndPosY())
end

function V3a9_WarmUp:_getTaskContentEndPosY()
