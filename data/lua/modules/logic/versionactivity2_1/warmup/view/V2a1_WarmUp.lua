-- chunkname: @modules/logic/versionactivity2_1/warmup/view/V2a1_WarmUp.lua

module("modules.logic.versionactivity2_1.warmup.view.V2a1_WarmUp", package.seeall)

local V2a1_WarmUp = class("V2a1_WarmUp", BaseView)

function V2a1_WarmUp:onInitView()
...
function V2a1_WarmUp:_btngetrewardOnClick()
...
function V2a1_WarmUp:_editableInitView()
...
function V2a1_WarmUp:onDataUpdateFirst()
	self:_refreshOnce()
end

function V2a1_WarmUp:onDataUpdate()
	self:_refresh()
end

function V2a1_WarmUp:onSwitchEpisode()
...
function V2a1_WarmUp:onUpdateParam()
	self:_refreshOnce()
	self:_refresh()
end

function V2a1_WarmUp:onOpen()
...
function V2a1_WarmUp:onDestroyView()
	return
end

function V2a1_WarmUp:_refreshOnce()
	self:_showDeadline()
	self:_refreshTabList()
	self:_autoSelectTab()
end

function V2a1_WarmUp:_refresh()
	self:_refreshData()
	self:_refreshTabList()
...
function V2a1_WarmUp:_refreshRightView()
	...
	if isRecevied or localIsPlay then
		self:_setActive_goWrongChannel(false)
	elseif not self.viewContainer:checkIsOpen() then
...
function V2a1_WarmUp:_setActive_goWrongChannel(isBlockDesc)
...
function V2a1_WarmUp:_refreshData()
...
function V2a1_WarmUp:_refreshTabList()
	...
	for i = 1, maxEpisodeCount do
		...
		if i > #self._itemTabList then
			item = self:_create_V2a1_WarmUp_radiotaskitem(i)
...
function V2a1_WarmUp:_setSelectIndex(index, isFocus)
...
function V2a1_WarmUp:_taskScrollToIndex(index)
...
function V2a1_WarmUp:onClickTab(mo)
...
function V2a1_WarmUp:_refreshRewards()
	local co = self.viewContainer:getEpisodeConfigCur()
	...
	for i = 1, itemCount do
		...
		if i > #self._rewardItemList then
			item = self:_create_V2a1_WarmUp_rewarditem(i)
...
function V2a1_WarmUp:openDesc(cb, cbObj)
...
function V2a1_WarmUp:_tweenDescUpdateCb(value)
	local bottom = lerp(0, self._taskDescViewportHeight, value)

	self:_setMaskPaddingBottom(bottom)
end

function V2a1_WarmUp:_tweenDescEndCb(maskDurationTime, cb, cbObj)
...
function V2a1_WarmUp:_resetTaskContentPos()
	recthelper.setAnchorY(self._txtTaskContentTran, 0)
end

function V2a1_WarmUp:episode2Index(episodeId)
	return episodeId
end

function V2a1_WarmUp:index2EpisodeId(index)
...
function V2a1_WarmUp:_setMaskPaddingBottom(bottom)
	self._taskDescMask.padding = Vector4(0, bottom, 0, 0)
end

function V2a1_WarmUp:_autoSelectTab()
...
function V2a1_WarmUp:_create_V2a1_WarmUp_radiotaskitem(index)
	local go = gohelper.cloneInPlace(self._goradiotaskitem)
	local item = V2a1_WarmUp_radiotaskitem.New({
		parent = self,
...
function V2a1_WarmUp:_create_V2a1_WarmUp_rewarditem(index)
	local go = gohelper.cloneInPlace(self._gorewarditem)
	local item = V2a1_WarmUp_rewarditem.New({
		parent = self,
		baseViewContainer = self.viewContainer
	})
...
function V2a1_WarmUp:_resetTweenDescPos()
	GameUtil.onDestroyViewMember_TweenId(self, "_movetweenId")
	GameUtil.onDestroyViewMember_TweenId(self, "_tweenId")
	AudioMgr.instance:trigger(AudioEnum.UI.play_ui_wulu_atticletter_write_stop)
	self:_resetTaskContentPos()
...
function V2a1_WarmUp:_playAnim(name, cb, cbObj)
	self._animatorPlayer:Play(name, cb, cbObj)
end

function V2a1_WarmUp:tweenSwitch(cb, cbObj)
	self:_playAnim(UIAnimationName.Switch, cb, cbObj)
end

function V2a1_WarmUp:_onSwitch()
	local lastEpisodeId = self.viewContainer:getCurSelectedEpisode()
	local curEpisodeId
...
function V2a1_WarmUp:playRewardItemsHasGetAnim()
...
function V2a1_WarmUp:setBlock_scroll(isBlock)
