...
slot0 = class("VersionActivity1_6NormalStoreGoodsView", BaseView)
...
function slot0.refreshRemain(slot0)
	if slot0.storeGoodsCo.maxBuyCount == 0 then
		gohelper.setActive(slot0._goRemain, false)
	else
		gohelper.setActive(slot0._goRemain, true)

		slot0._txtremain.text = GameUtil.getSubPlaceholderLuaLang(luaLang("store_buylimit_count"), {
			slot0.remainBuyCount
		})
	end
end
