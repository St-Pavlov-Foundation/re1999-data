...
slot0 = class("ActQuNiangLevelView", BaseView)
...
function slot0.addEvents(slot0)
	...
	slot0._animEvent:AddEventListener(ActQuNiangEnum.AnimEvt.OnStoryOpenEnd, slot0._onStoryOpenEnd, slot0)
	slot0._animEvent:AddEventListener(ActQuNiangEnum.AnimEvt.OnFightOpenEnd, slot0._onFightOpenEnd, slot0)
	slot0._animEvent:AddEventListener(ActQuNiangEnum.AnimEvt.OnGoStoryEnd, slot0._onGoStoryEnd, slot0)
end
...
function slot0._btnStoryOnClick(slot0, slot1)
	...
	if slot1 then
		slot0._anim:Play("openstory", -1, 0)
	else
		slot0._anim:Play("gostory", -1, 0)
...
function slot0._btnFightOnClick(slot0, slot1)
	...
	if slot1 then
		slot0._anim:Play("openfight", -1, 0)
	else
		slot0._anim:Play("gofight", -1, 0)
		AudioMgr.instance:trigger(AudioEnum.UI.play_ui_shuori_story_switch)
	end
end
...
function slot0._editableInitView(slot0)
	...
	slot0.enterConfig = RoleActivityConfig.instance:getActivityEnterInfo(ActQuNiangEnum.ActivityId)
	slot0.actConfig = ActivityConfig.instance:getActivityCo(ActQuNiangEnum.ActivityId)
...
function slot0.onOpen(slot0)
	slot0:addEventCb(ActQuNiangController.instance, ActQuNiangEvent.StoryItemClick, slot0.OnStoryItemClick, slot0)
	slot0:addEventCb(ActQuNiangController.instance, ActQuNiangEvent.FightItemClick, slot0.OnFightItemClick, slot0)
	slot0:addEventCb(ActQuNiangController.instance, ActQuNiangEvent.TabSwitch, slot0.OnTabSwitch, slot0)
	...
	RedDotController.instance:addRedDot(gohelper.findChild(slot0._btnTask.gameObject, "#go_reddot"), ActivityConfig.instance:getActivityCo(VersionActivity1_6Enum.ActivityId.Role1).redDotId, VersionActivity1_6Enum.ActivityId.Role1)
	slot0:OnDotChange()
	slot0:_showLeftTime()
	TaskDispatcher.runRepeat(slot0._showLeftTime, slot0, 1)

	if slot0.viewParam and slot0.viewParam.needShowFight then
		if ActQuNiangModel.instance:getEnterFightIndex() then
...
function slot0.OnCheckActEnd(slot0, slot1)
	if string.nilorempty(slot1) or slot1 == 0 then
		return
	end

	if slot1 ~= ActQuNiangEnum.ActivityId then
		return
	end
...
function slot0._initStageItems(slot0)
	...
	for slot7 = 1, #RoleActivityConfig.instance:getStoryLevelList(ActQuNiangEnum.ActivityId) do
		slot0.storyItemList[slot7] = MonoHelper.addNoUpdateLuaComOnceToGo(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[1], gohelper.findChild(slot0._gostoryStages, "stage" .. slot7)), QuNiangStoryItem, slot0)

		slot0.storyItemList[slot7]:setParam(slot3[slot7], slot7)
	...
	for slot8 = 1, #RoleActivityConfig.instance:getBattleLevelList(ActQuNiangEnum.ActivityId) / 2 do
		slot0.fightItemList[slot8] = MonoHelper.addNoUpdateLuaComOnceToGo(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[2], gohelper.findChild(slot0._gofightStages, "stage" .. slot8)), QuNiangFightItem, slot0)

		slot0.fightItemList[slot8]:setParam(slot4[2 * slot8 - 1], slot8)
...
function slot0._playFirstUnlock(slot0)
	...
	TaskDispatcher.runDelay(slot0._unlockStoryEnd, slot0, 0.83)
end

function slot0._playStoryFinishAnim(slot0)
	if ActQuNiangModel.instance:getNewFinishStoryLvl() then
...
function slot0._finishStoryEnd(slot0)
	slot0.storyItemList[slot0.finishStoryIndex]:refreshStatus()
	slot0.storyItemList[slot0.finishStoryIndex]:playStarAnim()
	...
	if slot0.finishStoryIndex == #slot0.storyItemList then
		...
		TaskDispatcher.runDelay(slot0._playPathAnim, slot0, 0.67)
	end
end
...
function slot0._playPathAnim(slot0)
	...
	TaskDispatcher.runDelay(slot0._unlockStory, slot0, 0.5)
end

function slot0._unlockStory(slot0)
	slot0.storyItemList[slot0.finishStoryIndex + 1]:playUnlock()
	TaskDispatcher.runDelay(slot0._unlockStoryEnd, slot0, 1.7)
end
...
function slot0._playFightFinishAnim(slot0)
	...
	if ActQuNiangModel.instance:getNewFinishFightLvl() then
		for slot5, slot6 in ipairs(slot0.fightItemList) do
			if slot6.id == slot1 then
...
function slot0._initPathStatus(slot0)
	if slot0.latestStoryItem > 1 then
		slot0._animPath:Play("go" .. slot0.latestStoryItem - 1, -1, 1)
	else
		slot0._animPath.speed = 0

		slot0._animPath:Play("go1", -1, 0)
	end
end
