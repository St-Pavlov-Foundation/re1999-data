module("modules.logic.versionactivity1_6.quniang.view.QuNiangFightItem", package.seeall)

slot0 = class("QuNiangFightItem", LuaCompBase)
...
function slot0._btnOnClick(slot0)
	...
	ActQuNiangController.instance:dispatchEvent(ActQuNiangEvent.FightItemClick, slot0.index)
end
...
function slot0._btnOnHard(slot0)
	if not ActQuNiangModel.instance:isLevelUnlock(slot0.hardConfig.id) then
		GameFacade.showToast(ToastEnum.DungeonIsLockNormal)
...
function slot0.refreshStatus(slot0)
	slot0.unlock = ActQuNiangModel.instance:isLevelUnlock(slot0.id)

	gohelper.setActive(slot0._goLock, not slot0.unlock)
	gohelper.setActive(slot0._goHardLock, not ActQuNiangModel.instance:isLevelUnlock(slot0.hardConfig.id))
	slot0:refreshStar()
end

function slot0.refreshStar(slot0)
	slot1 = ActQuNiangModel.instance:isLevelPass(slot0.id)
	slot2 = ActQuNiangModel.instance:isLevelPass(slot0.hardConfig.id)
...
function slot0.enterFight(slot0, slot1)
	...
	if slot1.chapterId and slot3 and slot4 > 0 then
		ActQuNiangModel.instance:setEnterFightIndex(slot0.index)
