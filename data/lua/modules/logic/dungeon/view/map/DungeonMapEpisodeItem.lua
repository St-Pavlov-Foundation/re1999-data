...
function slot0.onUpdateParam(slot0)
	...
	if slot0._config.chainEpisode ~= 0 then
		gohelper.setActive(slot0._goflag, not DungeonModel.instance:hasPassLevelAndStory(slot1))
	else
	...
	slot3 = slot0:refreshLockTip()
	slot0._isResourceTypeLock = DungeonModel.instance:chapterListIsResType(DungeonConfig.instance:getChapterCO(slot0._config.chapterId).type) and slot3

	slot0:_refreshUI(slot4, slot3)
	slot0:showStatus()
	slot0:_showAllElementTipView()
	slot0:refreshV1a7Fall()

	if DungeonModel.isBattleEpisode(slot0._config) and slot2.type == DungeonEnum.ChapterType.Normal then
		slot5 = string.splitToNumber(slot0._config.icon, "#")
		slot7 = slot5[2]
		slot8, slot9 = nil

		if slot5[1] and slot7 then
			slot8, slot9 = ItemModel.instance:getItemConfigAndIcon(slot6, slot7)
		end
		...
		if not string.nilorempty(slot9) then
			slot0._simagefall:LoadImage(slot9)
		end
	end
