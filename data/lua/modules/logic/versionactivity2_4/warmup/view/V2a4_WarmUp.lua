...
slot0 = class("V2a4_WarmUp", BaseView)
...
function slot0.onInitView(slot0)
	...
	slot0._gotask_reddot = gohelper.findChild(slot0.viewGO, "Right/#btn_task/#go_task_reddot")
	slot0._btnanswercall = gohelper.findChildButtonWithAudio(slot0.viewGO, "Right/#btn_answercall")
...
function slot0.addEvents(slot0)
	...
	slot0._btnanswercall:AddClickListener(slot0._btnanswercallOnClick, slot0)
	slot0._btnplay:AddClickListener(slot0._btnplaylOnClick, slot0)
end

function slot0.removeEvents(slot0)
	slot0._btngetreward:RemoveClickListener()
	slot0._btntask:RemoveClickListener()
	slot0._btnanswercall:RemoveClickListener()
...
slot3 = table.concat
slot4 = _G.Vector4
slot5 = string.splitToNumber
slot6 = string.split
slot7 = SLFramework.AnimatorPlayer
slot8 = "switch"
...
function slot0._btnanswercallOnClick(slot0)
	AudioMgr.instance:trigger(AudioEnum.UI.Stop_UI_Bus)
	AudioMgr.instance:trigger(AudioEnum.UI.play_ui_diqiu_yure_click_20249044)
	Activity125Controller.instance:sendGetTaskInfoRequest(slot0._openV2a4_WarmUp_DialogueView, slot0)
end
...
function slot0._btnplaylOnClick(slot0)
	...
	uv0(slot1, uv1 .. "?" .. uv2("timestamp=%s", ServerTime.now() * 1000))
	uv0(slot1, uv2("gameId=%s", SDKMgr.instance:getGameId()))
	uv0(slot1, uv2("gameRoleId=%s", PlayerModel.instance:getMyUserId()))
	uv0(slot1, uv2("channelUserId=%s", LoginModel.instance.channelUserId))
	uv0(slot1, uv2("deviceModel=%s", WebViewController.instance:urlEncode(UnityEngine.SystemInfo.deviceModel)))
	uv0(slot1, uv2("deviceId=%s", SDKMgr.instance:getDeviceInfo().deviceId))
	uv0(slot1, uv2("os=%s", WebViewController.instance:urlEncode(UnityEngine.SystemInfo.operatingSystem)))
	uv0(slot1, uv2("token=%s", SDKMgr.instance:getGameSdkToken()))
	uv0(slot1, uv2("channelId=%s", SDKMgr.instance:getChannelId()))
	uv0(slot1, uv2("isEmulator=%s", SDKMgr.instance:isEmulator() and 1 or 0))

	slot2 = uv3(slot1, "&")

	logNormal(slot2)
	WebViewController.instance:openWebView(slot2, false, slot0._onWebViewCb, slot0)
end
...
function slot0._openV2a4_WarmUp_DialogueView(slot0)
	AudioMgr.instance:trigger(AudioEnum.UI.Stop_UI_Bus)
	ViewMgr.instance:openView(ViewName.V2a4_WarmUp_DialogueView, {
		level = slot0.viewContainer:getCurSelectedEpisode()
	})
end
...
function slot0._editableInitView(slot0)
	...
	slot0._phoneAnimation = slot0._btnanswercall.gameObject:GetComponent(gohelper.Type_Animation)
	slot0._btnplayGo = slot0._btnplay.gameObject
	...
	RedDotController.instance:addRedDot(slot0._gotask_reddot, RedDotEnum.DotNode.Activity125Task, 0)
end

function slot0.onDataUpdateFirst(slot0)
	slot0:_refreshOnce()
	slot0:_refreshPhoneAnim()
...
function slot0._refreshPhoneAnim(slot0)
	slot4 = slot0._phoneAnimation.clip.name

	if slot0.viewContainer:isEpisodeFinished(slot0.viewContainer:getCurSelectedEpisode()) then
		slot0._phoneAnimation:Stop(slot4)
		slot0._phoneAnimation:Play(slot4)
		slot0._phoneAnimation:Sample()
		slot0._phoneAnimation:Stop(slot4)
		AudioMgr.instance:trigger(AudioEnum.UI.Stop_UI_Bus)
	else
		slot0._phoneAnimation:Play(slot4)
...
function slot0._refreshData(slot0)
	...
	slot0._txtTaskContent.text = slot0.viewContainer:getEpisodeConfigCur().text
...
function slot0._refreshTabList(slot0)
	for slot6 = 1, slot0.viewContainer:getEpisodeCount() do
		...
		if slot6 > #slot0._itemTabList then
			table.insert(slot0._itemTabList, slot0:_create_V2a4_WarmUp_radiotaskitem(slot6))
		else
...
function slot0._refreshRewards(slot0)
	slot0._rewardCount = #uv0(slot0.viewContainer:getEpisodeConfigCur().clientbonus or "", "|")
	...
	for slot8 = 1, slot4 do
		...
		if slot8 > #slot0._rewardItemList then
			table.insert(slot0._rewardItemList, slot0:_create_V2a4_WarmUp_rewarditem(slot8))
		else
...
function slot0._create_V2a4_WarmUp_radiotaskitem(slot0, slot1)
	slot3 = V2a4_WarmUp_radiotaskitem.New({
		parent = slot0,
...
function slot0._create_V2a4_WarmUp_rewarditem(slot0, slot1)
	slot3 = V2a4_WarmUp_rewarditem.New({
		parent = slot0,
		baseViewContainer = slot0.viewContainer
	})
...
function slot0._refreshActive_btnplay(slot0)
	...
	gohelper.setActive(slot0._btnplayGo, TimeUtil.stringToTimestamp(slot1) <= ServerTime.now())
end
