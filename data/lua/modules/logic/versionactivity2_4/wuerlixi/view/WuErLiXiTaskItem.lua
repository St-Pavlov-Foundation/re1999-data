...
slot0 = class("WuErLiXiTaskItem", ListScrollCellExtend)
...
function slot0._btnnotfinishbgOnClick(slot0)
	if slot0._act180TaskMO.config.jumpId > 0 then
		GameFacade.jump(slot0._act180TaskMO.config.jumpId)
	end
end

function slot0._btnfinishbgOnClick(slot0)
	slot0:_onOneClickClaimReward(slot0._act180TaskMO.activityId)
...
function slot0._btngetallOnClick(slot0)
	WuErLiXiController.instance:dispatchEvent(WuErLiXiEvent.OneClickClaimReward, slot0._act180TaskMO.activityId)
	UIBlockHelper.instance:startBlock("WuErLiXiTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(slot0._delayFinishAll, slot0, 0.5)
end

function slot0._delayFinish(slot0)
	TaskRpc.instance:sendFinishTaskRequest(slot0._act180TaskMO.config.id)
end
...
function slot0._editableAddEvents(slot0)
	WuErLiXiController.instance:registerCallback(WuErLiXiEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._editableRemoveEvents(slot0)
	WuErLiXiController.instance:unregisterCallback(WuErLiXiEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._onOneClickClaimReward(slot0, slot1)
	if slot0._act180TaskMO and slot0._act180TaskMO.activityId == slot1 and slot0._act180TaskMO:alreadyGotReward() then
...
function slot0.onUpdateMO(slot0, slot1)
	slot0._act180TaskMO = slot1
	slot0._scrollRewards.parentGameObject = slot0._view._csListScroll.gameObject

	slot0:_refreshUI()
	slot0:_moveByRankDiff(WuErLiXiTaskListModel.instance:getRankDiff(slot1))
end
...
function slot0._refreshUI(slot0)
	if not slot0._act180TaskMO then
		return
	end
...
slot0.prefabPath = "ui/viewres/versionactivity_2_4/v2a4_wuerlixi/v2a4_wuerlixi_taskitem.prefab"
