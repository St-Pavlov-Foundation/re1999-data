...
slot0 = class("PinballTaskItem", ListScrollCellExtend)
...
function slot0._btnnotfinishbgOnClick(slot0)
	if slot0._act178TaskMO.config.jumpId > 0 then
		GameFacade.jump(slot0._act178TaskMO.config.jumpId)
	end
end

function slot0._btnfinishbgOnClick(slot0)
	slot0:_onOneClickClaimReward(slot0._act178TaskMO.activityId)
...
function slot0._btngetallOnClick(slot0)
	PinballController.instance:dispatchEvent(PinballEvent.OneClickClaimReward, slot0._act178TaskMO.activityId)
	UIBlockHelper.instance:startBlock("PinballTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(slot0._delayFinishAll, slot0, 0.5)
end

function slot0._delayFinish(slot0)
	TaskRpc.instance:sendFinishTaskRequest(slot0._act178TaskMO.config.id)
end
...
function slot0._editableAddEvents(slot0)
	PinballController.instance:registerCallback(PinballEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._editableRemoveEvents(slot0)
	PinballController.instance:unregisterCallback(PinballEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._onOneClickClaimReward(slot0, slot1)
	if slot0._act178TaskMO and slot0._act178TaskMO.activityId == slot1 and slot0._act178TaskMO:alreadyGotReward() then
...
function slot0.onUpdateMO(slot0, slot1)
	slot0._act178TaskMO = slot1
	slot0._scrollRewards.parentGameObject = slot0._view._csListScroll.gameObject

	slot0:_refreshUI()
	slot0:_moveByRankDiff(PinballTaskListModel.instance:getRankDiff(slot1))
end
...
function slot0._refreshUI(slot0)
	if not slot0._act178TaskMO then
		return
	end
...
slot0.prefabPath = "ui/viewres/versionactivity_2_4/v2a4_pinball/v2a4_pinball_taskitem.prefab"
