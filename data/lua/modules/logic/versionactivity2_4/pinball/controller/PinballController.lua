...
function slot0.removeBuilding(slot0, slot1)
	...
	for slot8, slot9 in pairs(slot3) do
		if lua_activity178_resource.configDict[VersionActivity2_4Enum.ActivityId.Pinball][slot8] then
			table.insert(slot4, string.format("%s+%s", slot10.name, slot9))
		end
	end

	slot0._tempIndex = slot1

	GameFacade.showMessageBox(MessageBoxIdDefine.PinballRemoveBuilding, MsgBoxEnum.BoxType.Yes_No, slot0._realRemoveBuilding, nil, , slot0, nil, , table.concat(slot4, " "))
end
