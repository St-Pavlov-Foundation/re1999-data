...
slot0 = class("VersionActivity2_4DungeonMapLevelView", BaseView)
slot1 = 0.4
slot2 = 2.7
...
function slot0.addEvents(slot0)
	...
	slot0:addEventCb(VersionActivity2_4SudokuController.instance, VersionActivity2_4DungeonEvent.SudokuCompleted, slot0._onSudokuCompleted, slot0)
...
function slot0.removeEvents(slot0)
	...
	slot0:removeEventCb(VersionActivity2_4SudokuController.instance, VersionActivity2_4DungeonEvent.SudokuCompleted, slot0._onSudokuCompleted, slot0)
...
function slot0._onSudokuCompleted(slot0)
	if slot0.showEpisodeCo.id ~= VersionActivity2_4Enum.SudokuEpisodeId then
		return
	end

	DungeonRpc.instance:sendEndDungeonRequest(false)

	if not StoryModel.instance:isStoryFinished(slot0.showEpisodeCo.afterStory) then
		slot0:playSudokuAfterStory(slot0.showEpisodeCo.afterStory)
...
function slot0.startBattle(slot0)
	if slot0.showEpisodeCo.id == VersionActivity2_4Enum.SudokuEpisodeId then
		slot0:_playStoryAndOpenSudoku(slot0.showEpisodeCo.beforeStory)

		return
	elseif slot0.showEpisodeCo.type == DungeonEnum.EpisodeType.Story then
	...
	if slot0.isSpecialEpisode then
		...
		VersionActivity2_4DungeonController.instance:savePlayerPrefs(VersionActivity2_4DungeonEnum.PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastSelectMode, cjson.encode(slot0.lastEpisodeSelectModeDict))
	end
...
function slot0._playStoryAndOpenSudoku(slot0, slot1)
	DungeonRpc.instance:sendStartDungeonRequest(slot0.showEpisodeCo.chapterId, slot0.showEpisodeCo.id)

	if StoryModel.instance:isStoryFinished(slot1) then
		slot0:_openSudokuView()
	...
	StoryController.instance:playStory(slot1, {
		mark = true,
		episodeId = slot0.showEpisodeCo.id
	}, slot0._openSudokuView, slot0)
end

function slot0._openSudokuView(slot0)
	DungeonModel.instance.curSendEpisodeId = nil
...
function slot0.playSudokuAfterStory(slot0, slot1)
	StoryController.instance:playStory(slot1, {
		mark = true,
		episodeId = slot0.showEpisodeCo.id
	}, function ()
...
function slot0.initLocalEpisodeMode(slot0)
	slot0.unlockedEpisodeModeDict = VersionActivity2_4DungeonController.instance:loadDictFromStr(VersionActivity2_4DungeonController.instance:getPlayerPrefs(VersionActivity2_4DungeonEnum.PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastUnLockMode, ""))
	slot0.lastEpisodeSelectModeDict = VersionActivity2_4DungeonController.instance:loadDictFromStr(VersionActivity2_4DungeonController.instance:getPlayerPrefs(VersionActivity2_4DungeonEnum.PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastSelectMode, ""))
end
...
function slot0.initViewParam(slot0)
	...
	slot0.index = VersionActivity2_4DungeonConfig.instance:getEpisodeIndex(slot0.originEpisodeId)
...
function slot0.refreshStoryIdList(slot0)
	slot1 = slot0.originEpisodeConfig.type == DungeonEnum.EpisodeType.Story
	slot2 = slot0.mode == VersionActivityDungeonBaseEnum.DungeonMode.Hard

	if slot0.showEpisodeCo.id == VersionActivity2_4Enum.SudokuEpisodeId then
		slot0.storyIdList = {}

		if slot0.originEpisodeConfig.beforeStory > 0 and StoryModel.instance:isStoryHasPlayed(slot4) then
			table.insert(slot0.storyIdList, slot4)
		end

		if slot0.originEpisodeConfig.afterStory > 0 and StoryModel.instance:isStoryHasPlayed(slot5) then
			table.insert(slot0.storyIdList, slot5)
		end
	elseif slot1 or slot2 then
		slot0.storyIdList = nil

		return
	else
		slot4 = slot0.originEpisodeConfig
		...
		if slot4.beforeStory > 0 and StoryModel.instance:isStoryHasPlayed(slot7) then
			table.insert(slot0.storyIdList, slot7)
		end

		if slot4.afterStory > 0 and StoryModel.instance:isStoryHasPlayed(slot8) then
			table.insert(slot0.storyIdList, slot8)
		end
	end
end
...
function slot0.refreshEpisodeTextInfo(slot0)
	slot2 = nil
	slot2 = (DungeonConfig.instance:getChapterCO(slot0.showEpisodeCo.chapterId).id ~= VersionActivity2_4DungeonEnum.DungeonChapterId.Story or slot0.showEpisodeCo) and VersionActivity2_4DungeonConfig.instance:getStoryEpisodeCo(slot0.showEpisodeCo.id)
...
function slot0.setStarImage(slot0, slot1, slot2, slot3)
	if slot2 then
		UISpriteSetMgr.instance:setV2a4DungeonSprite(slot1, VersionActivity2_4DungeonEnum.EpisodeStarType[DungeonConfig.instance:getEpisodeCO(slot3).chapterId].light)
	else
		UISpriteSetMgr.instance:setV2a4DungeonSprite(slot1, slot5.empty)
	end
end
...
function slot0.refreshEye(slot0)
	...
	slot2 = slot0.originEpisodeConfig.chapterId == VersionActivity2_4DungeonEnum.DungeonChapterId.Hard
...
function slot0.onModeUnlockAnimationPlayDone(slot0)
	...
	VersionActivity2_4DungeonController.instance:savePlayerPrefs(VersionActivity2_4DungeonEnum.PlayerPrefsKey.ActivityDungeonSpecialEpisodeLastUnLockMode, cjson.encode(slot0.unlockedEpisodeModeDict))
