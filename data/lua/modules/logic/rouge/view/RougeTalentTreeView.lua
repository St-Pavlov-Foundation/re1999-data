...
function slot0.onInitView(slot0)
	...
	slot0._txtdetaildec = gohelper.findChildText(slot0.viewGO, "#go_detail/#txt_talentdec")
...
function slot0._refreshDetail(slot0, slot1, slot2)
	...
	if not ((slot1.isOrigin ~= 1 or (slot1.id ~= RougeEnum.OutsideConst.StartNode or false) and RougeTalentModel.instance:checkBigNodeLock(slot1.talent)) and RougeTalentModel.instance:checkNodeLock(slot1)) then
		...
		if slot4 then
			slot0._txtlocked2.text = GameUtil.getSubPlaceholderLuaLang(luaLang("rouge_talenttree_origin"), {
				RougeTalentModel.instance:getHadConsumeTalentPoint(),
				RougeTalentConfig.instance:getConfigByTalent(slot0._season, slot1.talent).cost
			})
		else
