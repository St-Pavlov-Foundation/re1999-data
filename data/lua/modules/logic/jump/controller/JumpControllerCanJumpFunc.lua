...
function slot0.canJumpToStoreView(slot0, slot1)
	...
	if StoreModel.instance:isTabOpen(string.splitToNumber(slot1, "#")[2]) then
		if slot5[3] then
			...
			if slot9.isOnline and (string.nilorempty(slot9.onlineTime) and slot12 or TimeUtil.stringToTimestamp(slot10) - ServerTime.clientToServerOffset()) <= slot12 and slot12 <= (string.nilorempty(slot11) and slot12 or TimeUtil.stringToTimestamp(slot11) - ServerTime.clientToServerOffset()) then
...
function slot0.canJumpToAct1_5DungeonView(slot0, slot1)
	slot2, slot3, slot4 = ActivityHelper.getActivityStatusAndToast(VersionActivity2_0Enum.ActivityId.EnterView)
