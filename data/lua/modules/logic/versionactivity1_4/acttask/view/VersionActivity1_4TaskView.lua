...
function VersionActivity1_4TaskView:refreshRemainTime()
	...
	if day > 0 then
		self.txtTime.text = string.format(luaLang("remain"), string.format("%s%s%s%s", day, luaLang("time_day"), hour, luaLang("time_hour")))
	else
		local minuteSecond = hourSecond % TimeUtil.OneHourSecond
		local minute = Mathf.Floor(minuteSecond / TimeUtil.OneMinuteSecond)

		if hour > 0 then
			self.txtTime.text = string.format(luaLang("remain"), string.format("%s%s%s%s", hour, luaLang("time_hour"), minute, luaLang("time_minute2")))
		elseif minute > 0 then
			self.txtTime.text = string.format(luaLang("remain"), string.format("%s%s", minute, luaLang("time_minute")))
		else
			self.txtTime.text = string.format(luaLang("remain"), string.format("<1%s", luaLang("time_minute")))
		end
	end
end
