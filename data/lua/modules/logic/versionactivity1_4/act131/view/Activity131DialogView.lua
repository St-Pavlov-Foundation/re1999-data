...
slot0 = class("Activity131DialogView", BaseView)
...
function slot0.onOpen(slot0)
	...
	slot0._elementInfo = slot0.viewParam

	slot0:_playStory()
	slot0._simagebg:LoadImage(ResUrl.getWeekWalkBg("bg_wz.png"))
	NavigateMgr.instance:addSpace(ViewName.Activity131DialogView, slot0._onSpace, slot0)
end
...
function slot0._addSectionHistory(slot0, slot1, slot2)
	slot3 = Activity131Config.instance:getDialog(slot0._dialogId, slot1)
	slot4 = nil
	slot4 = slot1 == slot0._mainSectionId and slot2 > #slot3 or slot2 >= #slot3

	for slot8, slot9 in ipairs(slot3) do
		if slot8 < slot2 or slot4 then
			if slot9.type == Activity131Enum.dialogType.options then
				slot10 = string.split(slot9.content, "#")
				slot12 = {}
				slot13 = {}

				for slot17, slot18 in ipairs(string.split(slot9.param, "#")) do
					if Activity131Config.instance:getDialog(slot0._dialogId, slot18) and slot19.type == "random" then
...
function slot0._setSectionData(slot0, slot1, slot2)
	slot0._sectionList = Activity131Config.instance:getDialog(slot0._dialogId, slot1)
...
function slot0._playNextDialog(slot0)
	...
	if slot1.type == Activity131Enum.dialogType.dialog then
		slot0:_showDialog(Activity131Enum.dialogType.dialog, slot1.content, slot1.speaker)
		...
		slot0:_refreshDialogBtnState()
	elseif slot1.type == Activity131Enum.dialogType.option then
		slot0:_showOptionByConfig(slot1)
	elseif slot1.type == Activity131Enum.dialogType.talk then
		slot0:_showTalk(Activity131Enum.dialogType.talk, slot1)
...
function slot0._showTalk(slot0, slot1, slot2)
	...
	slot0._isSkip = false
	slot0._txtdialogdesc.text = slot2.content
...
function slot0._showOptionByConfig(slot0, slot1)
	...
	if slot1 and slot1.type == Activity131Enum.dialogType.option then
		slot0:_updateHistory()
...
function slot0._skipOption(slot0, slot1, slot2)
	...
	if Activity131Model.instance:getCurMapId() >= 201 and slot4 <= 205 then
		slot3 = #slot1
	end
...
function slot0._fadeOutDone(slot0)
	...
	if slot0._elementInfo:getNextType() == Activity131Enum.ElementType.Battle then
...
function slot0._sendFinishDialog(slot0)
	...
	if Activity131Config.instance:getOptionParamList(slot0._dialogId) and #slot2 > 0 then
		slot3 = 1
	...
	Activity131Rpc.instance:sendAct131DialogRequest(VersionActivity1_4Enum.ActivityId.Role6, Activity131Model.instance:getCurEpisodeId(), slot0._elementInfo.elementId, slot1)
end
...
function slot0._addDialogOption(slot0, slot1, slot2, slot3)
	...
	if slot1 == 1 then
		slot9 = Activity131Model.instance:getCurMapConfig().type ~= 1
...
function slot0._checkOption(slot0, slot1)
	if not Activity131Config.instance:getDialog(slot0._dialogId, slot1) then
		slot0:_playNextSectionOrDialog()
...
function slot0._updateHistory(slot0)
	...
	Activity131Rpc.instance:sendAct131DialogHistoryRequest(VersionActivity1_4Enum.ActivityId.Role6, Activity131Model.instance:getCurEpisodeId(), slot0._elementInfo.elementId, slot1)
end
