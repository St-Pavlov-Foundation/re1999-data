...
function slot0.refreshMaxCount(slot0)
	slot0.maxCount = Activity129Config.instance:getConstValue1(slot0.actId, Activity129Enum.ConstEnum.MaxMoreDraw)

	if not Activity129Model.instance:getSelectPoolId() then
		return
	end

	slot4, slot5 = Activity129Model.instance:getActivityMo(slot0.actId):getPoolMo(slot1):getPoolDrawCount()
	...
	if slot5 > 0 then
		...
		slot0.maxCount = math.min(slot6, slot0.maxCount)
	end
end
