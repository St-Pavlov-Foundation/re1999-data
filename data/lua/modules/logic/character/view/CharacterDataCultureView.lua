...
function slot0._refreshDesc(slot0, slot1)
	...
	gohelper.setActive(slot0._txttitleen1.gameObject, slot1 ~= 3)
...
function slot0._getAfterContent(slot0, slot1)
	...
	for slot9, slot10 in ipairs(slot2) do
		if 0 < SLFramework.UGUI.GuiHelper.GetPreferredWidth(slot0._txtindenthelper, slot10) then
			slot4 = slot5
		end
	end
	...
	for slot13, slot14 in ipairs(slot2) do
		...
		table.insert(slot9, string.format("<indent=0%%%%><color=#352725><b>%s</b></color>:</indent><indent=%d%%%%>", slot14, slot5))
	end

	for slot13 = 1, #slot8 do
		slot1 = string.gsub(slot1, slot8[slot13], slot9[slot13])
	end
