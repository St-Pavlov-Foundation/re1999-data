-- chunkname: @modules/logic/versionactivity2_0/dungeon/view/map/VersionActivity2_0DungeonMapInteractView.lua

module("modules.logic.versionactivity2_0.dungeon.view.map.VersionActivity2_0DungeonMapInteractView", package.seeall)

local VersionActivity2_0DungeonMapInteractView = class("VersionActivity2_0DungeonMapInteractView", BaseView)

function VersionActivity2_0DungeonMapInteractView:onInitView(go)
	...
	self.interactItemAnim = ZProj.ProjAnimatorPlayer.Get(self._gointeractitem)
...
function VersionActivity2_0DungeonMapInteractView:initNoneContainer()
...
function VersionActivity2_0DungeonMapInteractView:initFightContainer()
...
function VersionActivity2_0DungeonMapInteractView:initDialogueContainer()
...
function VersionActivity2_0DungeonMapInteractView:initTalkContainer()
...
function VersionActivity2_0DungeonMapInteractView:addEvents()
	...
	self:addEventCb(VersionActivity2_0DungeonController.instance, VersionActivity2_0DungeonEvent.OnClickElement, self.showInteractUI, self)
...
function VersionActivity2_0DungeonMapInteractView:removeEvents()
	...
	self:removeEventCb(VersionActivity2_0DungeonController.instance, VersionActivity2_0DungeonEvent.OnClickElement, self.showInteractUI, self)
...
function VersionActivity2_0DungeonMapInteractView:_editableInitView()
	self._handleTypeMap = {
	...
	self._isClickClose = false
end

function VersionActivity2_0DungeonMapInteractView:showInteractUI(mapElement)
	if self._show then
		return
	end
	...
	if type(mapElement) ~= "table" then
		mapElement = self.viewContainer.mapSceneElements:getElementComp(tonumber(mapElement))
	end
...
function VersionActivity2_0DungeonMapInteractView:onRecheck(elementCo)
	if self._show then
		return
	end
...
function VersionActivity2_0DungeonMapInteractView:show()
	if self._show then
		return
	end
...
function VersionActivity2_0DungeonMapInteractView:hide()
	...
	if isOpenGraffitiEntrance then
		self.viewContainer.mapView:showRestaurantPartBtnUI()
		self.viewContainer.mapSceneElements:onHideInteractUI()
	else
		VersionActivity2_0DungeonController.instance:dispatchEvent(VersionActivity2_0DungeonEvent.OnHideInteractUI)
	end
end

function VersionActivity2_0DungeonMapInteractView:refreshUI()
...
function VersionActivity2_0DungeonMapInteractView:refreshRewards()
	local rewardStr = self._config.retroReward

	if self._isRecheck or string.nilorempty(rewardStr) then
	...
	for index, reward in ipairs(rewardList) do
		...
		rewardItem.icon:setMOValue(reward[1], reward[2], reward[3], nil, true)
...
function VersionActivity2_0DungeonMapInteractView:createRewardItem()
...
function VersionActivity2_0DungeonMapInteractView:refreshNoneUI()
...
function VersionActivity2_0DungeonMapInteractView:setFinishText()
...
function VersionActivity2_0DungeonMapInteractView:refreshFightUI()
...
function VersionActivity2_0DungeonMapInteractView:refreshDialogueUI()
...
function VersionActivity2_0DungeonMapInteractView:refreshTalkUI()
	self._sectionStack = {}
...
function VersionActivity2_0DungeonMapInteractView:_playSection(sectionId, dialogIndex)
	self:_setSectionData(sectionId, dialogIndex)
	self:_playNextDialog()
end

function VersionActivity2_0DungeonMapInteractView:_setSectionData(sectionId, dialogIndex)
...
function VersionActivity2_0DungeonMapInteractView:_playNextDialog()
...
function VersionActivity2_0DungeonMapInteractView:_showDialog(type, text, speaker, audio)
...
function VersionActivity2_0DungeonMapInteractView:setIsChat(isChat)
	gohelper.setActive(self._godesc, not isChat)
	gohelper.setActive(self._gochat, isChat)
end

function VersionActivity2_0DungeonMapInteractView:_addDialogOption(index, sectionId, text)
...
function VersionActivity2_0DungeonMapInteractView:_onOptionClick(param)
...
function VersionActivity2_0DungeonMapInteractView:_refreshDialogBtnState(showOption, hasNext)
...
function VersionActivity2_0DungeonMapInteractView:onClickRoot()
	if self._isClickClose then
		return
	end

	self.interactItemAnim:Play(UIAnimationName.Close, self.hide, self)
...
function VersionActivity2_0DungeonMapInteractView:_btncloseOnClick()
	if self._isClickClose then
		return
	end
...
function VersionActivity2_0DungeonMapInteractView:_onClickNoneBtn()
	self:hide()
	AudioMgr.instance:trigger(AudioEnum.UI.play_ui_checkpoint_continuemesh)
	self:finishElement()
end

function VersionActivity2_0DungeonMapInteractView:_onClickFightBtn()
	self:hide()
	AudioMgr.instance:trigger(AudioEnum.UI.play_ui_checkpoint_continuemesh)
	...
	VersionActivity2_0DungeonModel.instance:setLastEpisodeId(self.activityDungeonMo.episodeId)
	DungeonFightController.instance:enterFight(config.chapterId, episodeId)
end

function VersionActivity2_0DungeonMapInteractView:_onClickEnterDialogueBtn()
	if self.isFinish and not self._isRecheck then
		self:hide()
		self:finishElement()
	...
	VersionActivity2_0DungeonController.instance:openDialogueView(self.dialogueId, function()
		if self._isRecheck then
			self:hide()
			DungeonController.instance:onAgainOpenRecheckView(self._config.id)
		end
...
function VersionActivity2_0DungeonMapInteractView:finishElement(dialogIds)
	...
	if self._isRecheck then
		...
		DungeonRpc.instance:sendMapElementRequest(elementId, dialogIds, self.updateAct161Info, self)
	end
end

function VersionActivity2_0DungeonMapInteractView:updateAct161Info()
	Activity161Controller.instance:initAct161Info()
end

function VersionActivity2_0DungeonMapInteractView:_btnnextOnClick()
	self:_playNextSectionOrDialog()
	AudioMgr.instance:trigger(AudioEnum.UI.play_ui_checkpoint_continuemesh)
end

function VersionActivity2_0DungeonMapInteractView:_playNextSectionOrDialog()
	if #self._sectionList >= self._dialogIndex then
		self:_playNextDialog()
...
function VersionActivity2_0DungeonMapInteractView:_btnfinishtalkOnClick()
	self:hide()
...
function VersionActivity2_0DungeonMapInteractView:onDialogueInfoChange(dialogueId)
	if dialogueId == self.dialogueId then
		self:refreshDialogueUI()
	end
end

function VersionActivity2_0DungeonMapInteractView:onCloseViewFinishCall(viewName)
	if viewName == ViewName.DialogueView and self.isFinish then
		self:refreshUI()
	end
end

function VersionActivity2_0DungeonMapInteractView:beforeJump()
	self:hide()
end

function VersionActivity2_0DungeonMapInteractView:onClose()
	self._isClickClose = false
end

function VersionActivity2_0DungeonMapInteractView:onDestroyView()
	self.rootClick:RemoveClickListener()
end

return VersionActivity2_0DungeonMapInteractView
