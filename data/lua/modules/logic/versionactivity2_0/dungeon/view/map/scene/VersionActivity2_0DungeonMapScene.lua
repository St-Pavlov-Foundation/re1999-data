-- chunkname: @modules/logic/versionactivity2_0/dungeon/view/map/scene/VersionActivity2_0DungeonMapScene.lua

module("modules.logic.versionactivity2_0.dungeon.view.map.scene.VersionActivity2_0DungeonMapScene", package.seeall)

local VersionActivity2_0DungeonMapScene = class("VersionActivity2_0DungeonMapScene", BaseView)

function VersionActivity2_0DungeonMapScene:onInitView()
...
function VersionActivity2_0DungeonMapScene:addEvents()
	...
	self:addEventCb(VersionActivity2_0DungeonController.instance, VersionActivity2_0DungeonEvent.FocusElement, self.onFocusElement, self)
	self:addEventCb(VersionActivity2_0DungeonController.instance, VersionActivity2_0DungeonEvent.ManualClickElement, self.manualClickElement, self)
...
function VersionActivity2_0DungeonMapScene:removeEvents()
	...
	self:removeEventCb(VersionActivity2_0DungeonController.instance, VersionActivity2_0DungeonEvent.FocusElement, self.onFocusElement, self)
	self:removeEventCb(VersionActivity2_0DungeonController.instance, VersionActivity2_0DungeonEvent.ManualClickElement, self.manualClickElement, self)
...
function VersionActivity2_0DungeonMapScene:_onOpenView(viewName)
...
function VersionActivity2_0DungeonMapScene:_onCloseView(viewName)
	if viewName == ViewName.StoryView then
		self:loadMap()
	end
end

function VersionActivity2_0DungeonMapScene:_onScreenResize()
	if self._sceneGo then
		local camera = CameraMgr.instance:getMainCamera()
		local scale = GameUtil.getAdapterScale()

		camera.orthographicSize = VersionActivity2_0DungeonEnum.DungeonMapCameraSize * scale

		self:_initScene()
	end
end

function VersionActivity2_0DungeonMapScene:onModeChange()
	self:refreshMap()
end

function VersionActivity2_0DungeonMapScene:onActivityDungeonMoChange()
	local needTween = VersionActivity2_0DungeonModel.instance:getMapNeedTweenState()

	self:refreshMap(needTween)
end

function VersionActivity2_0DungeonMapScene:onClickElement(elementComp)
...
function VersionActivity2_0DungeonMapScene:focusElementByCo(elementCo)
...
function VersionActivity2_0DungeonMapScene:onFocusElement(elementId, isForce)
...
function VersionActivity2_0DungeonMapScene:changeToElementEpisode(elementId)
...
function VersionActivity2_0DungeonMapScene:manualClickElement(elementId)
	local elementComp = self.mapSceneElementsView:getElementComp(elementId)
...
function VersionActivity2_0DungeonMapScene:_onDragBegin(param, pointerEventData)
	VersionActivity2_0DungeonModel.instance:setDraggingMapState(true)
...
function VersionActivity2_0DungeonMapScene:_onDrag(param, pointerEventData)
...
function VersionActivity2_0DungeonMapScene:_onDragEnd(param, pointerEventData)
	self._dragBeginPos = nil

	VersionActivity2_0DungeonModel.instance:setDraggingMapState(false)
end

function VersionActivity2_0DungeonMapScene:getDragWorldPos(pointerEventData)
...
function VersionActivity2_0DungeonMapScene:_editableInitView()
...
function VersionActivity2_0DungeonMapScene:_initMapRootNode()
	self._sceneRoot = UnityEngine.GameObject.New(VersionActivity2_0DungeonEnum.SceneRootName)
...
function VersionActivity2_0DungeonMapScene:onUpdateParam()
	self:refreshMap()
end

function VersionActivity2_0DungeonMapScene:onOpen()
	MainCameraMgr.instance:addView(self.viewName, self._initCamera, nil, self)
	self:refreshMap()
end

function VersionActivity2_0DungeonMapScene:_initCamera()
	local camera = CameraMgr.instance:getMainCamera()
...
function VersionActivity2_0DungeonMapScene:refreshMap(needTween, mapCfg)
	self._mapCfg = mapCfg or VersionActivity2_0DungeonConfig.instance:getEpisodeMapConfig(self.activityDungeonMo.episodeId)
	...
	if self._mapCfg.id == self._lastLoadMapId then
		...
		if VersionActivity2_0DungeonModel.instance:getOpenGraffitiEntranceState() then
			self:_setGraffitiMapSceneCanvas()
		else
			self:_setMapPos()
		end
	else
...
function VersionActivity2_0DungeonMapScene:_initElements()
	DungeonController.instance:dispatchEvent(DungeonMapElementEvent.OnInitElements)
end

function VersionActivity2_0DungeonMapScene:loadMap()
...
function VersionActivity2_0DungeonMapScene:buildLoadRes(allResPath, mapCfg)
...
function VersionActivity2_0DungeonMapScene:_loadSceneFinish()
	...
	self:_initElements()
	self:_addMapAudio()
	self:_addSceneCanvas()
end

function VersionActivity2_0DungeonMapScene:_initScene()
	...
	local cameraSizeRate = VersionActivity2_0DungeonEnum.DungeonMapCameraSize / uiCameraSize
...
function VersionActivity2_0DungeonMapScene:_setMapPos()
...
function VersionActivity2_0DungeonMapScene:_addMapLight()
...
function VersionActivity2_0DungeonMapScene:_addMapAudio()
...
function VersionActivity2_0DungeonMapScene:_addSceneCanvas()
	local assetUrl = self._sceneCanvas
	local assetItem = self._mapLoader:getAssetItem(assetUrl)
	local mainPrefab = assetItem:GetResource(assetUrl)

	self._graffitiCanvasGo = gohelper.clone(mainPrefab, self._sceneGo, "graffitiMapSceneCanvas")
	self._graffitiCanvasGo:GetComponent("Canvas").worldCamera = CameraMgr.instance:getMainCamera()

	self:_setGraffitiMapSceneCanvas()
end

function VersionActivity2_0DungeonMapScene:_setGraffitiMapSceneCanvas()
	Activity161Controller.instance:dispatchEvent(Activity161Event.SetGraffitiMapSceneCanvas, {
		mapConfig = self._mapCfg,
		canvasGO = self._graffitiCanvasGo,
		sceneGO = self._sceneGo
	})
	self:_setMapPos()
end

function VersionActivity2_0DungeonMapScene:tweenSetScenePos(targetPos, srcPos)
...
function VersionActivity2_0DungeonMapScene:getTargetPos(targetPos)
...
function VersionActivity2_0DungeonMapScene:tweenFrameCallback(value)
...
function VersionActivity2_0DungeonMapScene:tweenFinishCallback()
	self._tempVector:Set(self._tweenTargetPosX, self._tweenTargetPosY, 0)
	self:directSetScenePos(self._tempVector)
end

function VersionActivity2_0DungeonMapScene:directSetScenePos(targetPos)
	...
	VersionActivity2_0DungeonController.instance:dispatchEvent(VersionActivity2_0DungeonEvent.OnMapPosChanged, self._scenePos, self.needTween)
	self:_updateElementArrow()
end

function VersionActivity2_0DungeonMapScene:_updateElementArrow()
	DungeonController.instance:dispatchEvent(DungeonMapElementEvent.OnUpdateElementArrow)
end

function VersionActivity2_0DungeonMapScene:setVisible(isVisible)
...
function VersionActivity2_0DungeonMapScene:getSceneGo()
	return self._sceneGo
end

function VersionActivity2_0DungeonMapScene:onClose()
	self:killTween()
	self:_resetCamera()
end

function VersionActivity2_0DungeonMapScene:killTween()
	if self.tweenId then
		ZProj.TweenHelper.KillById(self.tweenId)
	end
end

function VersionActivity2_0DungeonMapScene:_resetCamera()
	local camera = CameraMgr.instance:getMainCamera()
...
function VersionActivity2_0DungeonMapScene:_onRecheckElement(elementId, isGM)
...
function VersionActivity2_0DungeonMapScene:onDestroyView()
...
function VersionActivity2_0DungeonMapScene:disposeOldMap()
...
function VersionActivity2_0DungeonMapScene:_disposeScene()
	self._oldScenePos = self._scenePos
...
return VersionActivity2_0DungeonMapScene
