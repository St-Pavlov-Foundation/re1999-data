...
slot0 = class("V2a0_WarmUp", BaseView)

function slot0.onInitView(slot0)
	slot0._simagefullbgdark = gohelper.findChildSingleImage(slot0.viewGO, "#simage_fullbg_dark")
...
function slot0._refreshRightView(slot0)
	...
	if slot1 or slot2 then
		...
		slot0:_setActive_goWrongChannel(not slot0.viewContainer:checkEyeIsClicked())
	end
...
function slot0._refreshTabList(slot0)
	for slot6 = 1, slot0.viewContainer:getEpisodeCount() do
		...
		if slot6 > #slot0._itemTabList then
			table.insert(slot0._itemTabList, slot0:_create_V2a0_WarmUp_radiotaskitem(slot6))
		else
...
function slot0._refreshRewards(slot0)
	...
	for slot8 = 1, slot4 do
		...
		if slot8 > #slot0._rewardItemList then
			table.insert(slot0._rewardItemList, slot0:_create_V2a0_WarmUp_rewarditem(slot8))
		else
...
function slot0.openDesc(slot0, slot1, slot2)
	...
	slot0._tweenId = ZProj.TweenHelper.DOTweenFloat(1, 0, slot0.viewContainer:getEpisodeConfigCur().time, slot0._tweenDescUpdateCb, function ()
...
function slot0._create_V2a0_WarmUp_radiotaskitem(slot0, slot1)
	slot3 = V2a0_WarmUp_radiotaskitem.New({
		parent = slot0,
...
function slot0._create_V2a0_WarmUp_rewarditem(slot0, slot1)
	slot3 = V2a0_WarmUp_rewarditem.New({
		parent = slot0,
		baseViewContainer = slot0.viewContainer
	})
...
function slot0.tweenSwitch(slot0, slot1, slot2)
	slot0:_playAnim(uv0, slot1, slot2)
end
