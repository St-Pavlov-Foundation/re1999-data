-- chunkname: @modules/logic/versionactivity3_9/naxisuosi/view/NaxisuosiTaskItem.lua

module("modules.logic.versionactivity3_9.naxisuosi.view.NaxisuosiTaskItem", package.seeall)

local NaxisuosiTaskItem = class("NaxisuosiTaskItem", ListScrollCellExtend)

function NaxisuosiTaskItem:onInitView()
...
function NaxisuosiTaskItem:addEvents()
...
function NaxisuosiTaskItem:removeEvents()
	self._btnnotfinishbg:RemoveClickListener()
	self._btnfinishbg:RemoveClickListener()
	self._btngetall:RemoveClickListener()
end

function NaxisuosiTaskItem:_btnnotfinishbgOnClick()
	if self._taskMo.config.jumpId > 0 then
		GameFacade.jump(self._taskMo.config.jumpId)
		ViewMgr.instance:closeView(ViewName.NaxisuosiTaskView)
	end
end

local BLOCK_KEY = "NaxisuosiTaskItem_finishAnim"

function NaxisuosiTaskItem:_btnfinishbgOnClick()
...
function NaxisuosiTaskItem:_delayFinish()
	TaskRpc.instance:sendFinishTaskRequest(self._taskMo.config.id)
end

function NaxisuosiTaskItem:_btngetallOnClick()
	self:_onOneClickClaimReward(self._taskMo.activityId)
	UIBlockHelper.instance:startBlock(BLOCK_KEY, 0.5)
...
function NaxisuosiTaskItem:_editableInitView()
...
function NaxisuosiTaskItem:onUpdateMO(mo)
	self._taskMo = mo
...
function NaxisuosiTaskItem:_refreshUI()
...
function NaxisuosiTaskItem:_onItemShow(cell_component, data, index)
...
function NaxisuosiTaskItem:_moveByRankDiff(rankDiff)
...
function NaxisuosiTaskItem:getAnimator()
	return self._animator
end

function NaxisuosiTaskItem:onSelect(isSelect)
	return
end

function NaxisuosiTaskItem:onDestroyView()
	if self._rankDiffMoveId then
		ZProj.TweenHelper.KillById(self._rankDiffMoveId)
