...
slot0 = class("VersionActivity1_3NormalStoreGoodsView", BaseView)
...
function slot0.onInitView(slot0)
	...
	slot0._txtremain = gohelper.findChildText(slot0.viewGO, "root/propinfo/info/remain/#txt_remain")
...
function slot0._editableInitView(slot0)
	...
	slot0.goRemain = gohelper.findChild(slot0.viewGO, "root/propinfo/info/remain")
...
function slot0.refreshUI(slot0)
	...
	if slot0.storeGoodsCo.maxBuyCount == 0 then
		gohelper.setActive(slot0.goRemain, false)
	else
		gohelper.setActive(slot0._txtremain.gameObject, true)

		slot0._txtremain.text = string.format("%s:%d", luaLang("store_buylimit"), slot0.remainBuyCount)
	end
...
function slot0.refreshByCount(slot0)
	...
	slot0._txtitemcount.text = string.format("x%s", GameUtil.numberDisplay(slot0.oneItemCount * slot0.currentBuyCount))
