...
slot0 = class("DungeonMapInteractive1_3ItemComp", BaseView)
...
function slot0._showFight(slot0)
	...
	if slot2 then
		slot0._txtinfo.text = slot0._config.finishText
	else
		slot0._txtinfo.text = slot0._config.desc
...
function slot0._delayScroll(slot0)
	...
	if slot0._curScrollGo then
		...
		ZProj.TweenHelper.DOLocalMoveY(slot2.transform, 229, 0.26, slot0._scrollEnd, slot0, slot2)
	else
		slot1 = 0.1
	end

	slot3 = gohelper.cloneInPlace(slot0._goscroll)

	for slot7, slot8 in ipairs(slot0._dialogItemList) do
		gohelper.addChild(slot3, slot8)

		slot8.transform.position = slot8.transform.position
		slot10, slot11, slot12 = transformhelper.getLocalPos(slot8.transform)

		transformhelper.setLocalPos(slot8.transform, slot10, slot11, 0)
	end
