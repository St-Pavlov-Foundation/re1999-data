-- chunkname: @modules/logic/seasonver/act123/view3_5/Season123_3_5HeroGroupHeroItem.lua

module("modules.logic.seasonver.act123.view3_5.Season123_3_5HeroGroupHeroItem", package.seeall)

local Season123_3_5HeroGroupHeroItem = class("Season123_3_5HeroGroupHeroItem", LuaCompBase)

function Season123_3_5HeroGroupHeroItem:ctor(SeasonHeroGroupListView)
	self._seasonHeroGroupListView = SeasonHeroGroupListView
end

function Season123_3_5HeroGroupHeroItem:init(go)
	self.go = go
...
function Season123_3_5HeroGroupHeroItem:onUpdateMO(mo, viewParam, replayHeroData)
	self.mo = mo
...
function Season123_3_5HeroGroupHeroItem:checkSubGoVisable()
	if self.isLock then
		return false
	end
...
function Season123_3_5HeroGroupHeroItem:_initData()
	self._firstEnter = true

	gohelper.setActive(self._goselectdebuff, false)

	self._gocardlist = gohelper.findChild(self.go, "heroitemani/equipcard/cardlist")
	self.cardList = {}

	for i = 1, Activity123Enum.HeroCardNum do
		local go = gohelper.findChild(self.go, string.format("heroitemani/equipcard/cardlist/carditem%s", i))

		self.cardList[i] = Season123_3_5HeroGroupCardItem.New(go, self, {
			slot = i
		})
	end
end

function Season123_3_5HeroGroupHeroItem:_addEvents()
...
function Season123_3_5HeroGroupHeroItem:_updateSeasonEquips()
	self:_updateEquips()
	self:_updateEquipCards()
	self:_updateAct123Hp()
end

function Season123_3_5HeroGroupHeroItem:_onClickHeroItem()
...
function Season123_3_5HeroGroupHeroItem:_onClickEquip()
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season123_3_5HeroGroupHeroItem:_updateEquips()
	self._equipType = -1
	...
	if self.isLock or self.isAid or self.isRoleNumLock or not OpenModel.instance:isFunctionUnlock(OpenEnum.UnlockFunc.Equip) and not self.trialCO and not HeroGroupTrialModel.instance:haveTrialEquip() then
		...
		local hasEquip = self._equipMO or trialEquipCO

		gohelper.setActive(self._equip.equipIcon.gameObject, hasEquip)
		gohelper.setActive(self._equip.equipRare.gameObject, hasEquip)
		gohelper.setActive(self._equip.equipGolv, hasEquip)
...
Season123_3_5HeroGroupHeroItem.NonCardSlotBlackMaskHeight = 362
Season123_3_5HeroGroupHeroItem.HasCardSlotBlackMashHeight = 200

function Season123_3_5HeroGroupHeroItem:_updateEquipCards()
	local curGroupMO = HeroGroupModel.instance:getCurGroupMO()
	...
	if not self.isAid then
		local layer = curGroupMO.isReplay and context.layer or nil

		for slot, cardItem in pairs(self.cardList) do
			local equipUid, equipId = self:getEquipId(slot)

			cardItem:setData(self.mo, layer, equipId, equipUid)
		end
	end

	local isShow = self:isSeasonCardShow()

	gohelper.setActive(self._gocardlist, isShow)

	if isShow then
		recthelper.setHeight(self._goblackmask.transform, Season123_3_5HeroGroupHeroItem.NonCardSlotBlackMaskHeight)
	else
		recthelper.setHeight(self._goblackmask.transform, Season123_3_5HeroGroupHeroItem.HasCardSlotBlackMashHeight)
	end
end

function Season123_3_5HeroGroupHeroItem:isSeasonCardShow()
	if self.isAid then
		return false
	end

	for slot, cardItem in pairs(self.cardList) do
		if cardItem.slotUnlock then
			return true
		end
	end
end

function Season123_3_5HeroGroupHeroItem:getEquipId(slot)
	local equipUid, equipId = Activity123Enum.EmptyUid, 0
	local curGroupMO = HeroGroupModel.instance:getCurGroupMO()

	if curGroupMO.isReplay then
		local equipdata = curGroupMO.replay_activity104Equip_data[self.mo.heroUid]

		equipId = equipdata and equipdata[1] and equipdata[1].equipId or 0
	else
		local seasonEquipMo = curGroupMO:getAct104PosEquips(self.mo.id - 1)
		local equipUids = seasonEquipMo.equipUid or {}
		local trailEquipId = self.trialCO and self.trialCO[string.format("act104EquipId%d", slot)] or 0

		if trailEquipId > 0 then
			equipId = trailEquipId
		else
			equipUid = equipUids[slot]
			equipId = tonumber(equipUid)
		end
	end

	return equipUid, equipId
end

function Season123_3_5HeroGroupHeroItem:_updateAct123Hp()
	local context = Season123Model.instance:getBattleContext()
...
function Season123_3_5HeroGroupHeroItem:setHp(hpRate)
...
function Season123_3_5HeroGroupHeroItem:_equipIconAddDrag(go)
	if self._drag then
		return
	end
...
function Season123_3_5HeroGroupHeroItem:_onBeginDrag(equipTransform, pointerEventData)
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season123_3_5HeroGroupHeroItem:_onDrag(param, pointerEventData)
	if HeroGroupModel.instance:getCurGroupMO().isReplay then
		return
	end
...
function Season123_3_5HeroGroupHeroItem:_onEndDrag(equipTransform, pointerEventData)
	self:killEquipTweenId()
...
function Season123_3_5HeroGroupHeroItem:canMoveCardToPos(tarPos)
	for _, cardItem in pairs(self.cardList) do
		if not cardItem:canMoveToPos(tarPos) then
			return false
		end
	end
...
function Season123_3_5HeroGroupHeroItem:topDragTransformOrder()
	local dragTransform = self:getDragTransform()

	gohelper.addChildPosStay(self.currentParent, dragTransform.gameObject)
end

function Season123_3_5HeroGroupHeroItem:resetDragTransformOrder()
	local dragTransform = self:getDragTransform()

	gohelper.addChildPosStay(self._goequip, dragTransform.gameObject)
end

function Season123_3_5HeroGroupHeroItem:getDragTransform()
	return self._equip.moveContainerTrs
end

function Season123_3_5HeroGroupHeroItem:getOrignDragPos()
	return self._goequip.transform.position
end

function Season123_3_5HeroGroupHeroItem:onDragEndFail()
...
function Season123_3_5HeroGroupHeroItem:onDragEndSuccess(targetHeroItem)
...
function Season123_3_5HeroGroupHeroItem:_playDragEndAudio(targetHeroItem)
...
function Season123_3_5HeroGroupHeroItem:_tweenToPos(transform, anchorPos)
...
function Season123_3_5HeroGroupHeroItem:_setToPos(transform, anchorPos, tween, callback, callbackObj, param)
	local curAnchorX, curAnchorY = recthelper.getAnchor(transform)
...
function Season123_3_5HeroGroupHeroItem:_moveToTarget(position)
...
function Season123_3_5HeroGroupHeroItem:_setEquipDragEnabled(isEnabled)
...
function Season123_3_5HeroGroupHeroItem:playHeroGroupHeroEffect(state)
...
function Season123_3_5HeroGroupHeroItem:playRestrictAnimation(needRestrictHeroUidDict, deadHeroUidDict)
...
function Season123_3_5HeroGroupHeroItem:setGrayFactor(value)
...
function Season123_3_5HeroGroupHeroItem:showCounterSign()
	local career
	...
	if isRecommended or isCounter then
		local targetPos = self:isSeasonCardShow() and restrainTagPosUseSeasonCard or restrainTagPosUnUseSeasonCard
...
function Season123_3_5HeroGroupHeroItem:onItemBeginDrag(index)
...
function Season123_3_5HeroGroupHeroItem:onItemEndDrag(index, dragToIndex)
	ZProj.TweenHelper.DOScale(self.go.transform, 1, 1, 1, 0.2, nil, nil, nil, EaseType.Linear)
end

function Season123_3_5HeroGroupHeroItem:onItemCompleteDrag(index, dragToIndex, complete)
...
function Season123_3_5HeroGroupHeroItem:hideDragEffect()
	gohelper.setActive(self._goselectdebuff, false)
end

function Season123_3_5HeroGroupHeroItem:setHeroGroupEquipEffect(show)
	self._canPlayEffect = show
end

function Season123_3_5HeroGroupHeroItem:setParent(transform)
	self.currentParent = transform

	gohelper.addChildPosStay(transform.gameObject, self._subGO)
end

function Season123_3_5HeroGroupHeroItem:flowOriginParent()
	return
end

function Season123_3_5HeroGroupHeroItem:flowCurrentParent()
	return
end

function Season123_3_5HeroGroupHeroItem:getHeroItemList()
	return self._seasonHeroGroupListView:getHeroItemList()
end

function Season123_3_5HeroGroupHeroItem:killEquipTweenId()
...
function Season123_3_5HeroGroupHeroItem:_removeEvents()
	self._btnclickitem:RemoveClickListener()
	self._btnclickequip:RemoveClickListener()
end

function Season123_3_5HeroGroupHeroItem:onDestroy()
	self:_removeEvents()
	self:killEquipTweenId()
...
return Season123_3_5HeroGroupHeroItem
