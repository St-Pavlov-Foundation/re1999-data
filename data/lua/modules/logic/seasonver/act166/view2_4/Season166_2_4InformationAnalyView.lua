...
slot0 = class("Season166_2_4InformationAnalyView", BaseView)
...
function slot0.onInitView(slot0)
	...
	slot0.goDetail = gohelper.findChild(slot0.viewGO, "Detail")
...
function slot0._editableInitView(slot0)
	...
	slot0._animEventWrap:AddEventListener("switch", slot0.checkChangeInfo, slot0)
end
...
function slot0.onClickLeft(slot0)
	slot0.infoId = slot0.infoId - 1

	if slot0.infoId <= 0 then
		slot0.infoId = #Season166Config.instance:getSeasonInfos(slot0.actId)
	end

	slot0.anim:Play(UIAnimationName.Switch, 0, 0)
end

function slot0.onClickRight(slot0)
	slot0.infoId = slot0.infoId + 1

	if slot0.infoId > #Season166Config.instance:getSeasonInfos(slot0.actId) then
		slot0.infoId = 1
	end
...
function slot0.checkChangeInfo(slot0)
	Season166Controller.instance:dispatchEvent(Season166Event.ChangeAnalyInfo, slot0.infoId)
end
...
function slot0.refreshUI(slot0)
	...
	slot0.unlockState = Season166Model.instance:getActInfo(slot0.actId) and slot2:getInformationMO(slot0.infoId) and Season166Enum.UnlockState or Season166Enum.LockState
	...
	slot0.txtLockInfo.text = slot1.unlockDes

	gohelper.setActive(slot0.goDetail, slot4)
	gohelper.setActive(slot0.goLockInfo, not slot4)
	slot0:refreshBtn()
end
