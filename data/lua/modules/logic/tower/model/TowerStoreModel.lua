...
slot0 = class("TowerStoreModel", BaseModel)
...
function slot0.getCurrencyIcon(slot0, slot1)
	if CurrencyEnum.CurrencyType.TowerStore and CurrencyConfig.instance:getCurrencyCo(slot2) then
		return slot3.icon .. "_1"
	end
end

function slot0.getCurrencyCount(slot0, slot1)
	if CurrencyEnum.CurrencyType.TowerStore and CurrencyModel.instance:getCurrency(slot2) then
...
function slot0.getStoreGroupMO(slot0)
	for slot5, slot6 in pairs(StoreEnum.TowerStore) do
		-- Nothing
	end
...
function slot0.getUpdateStoreRemainTime(slot0, slot1)
	if slot0:getUpdateStoreActivityMo() then
		return slot2:getRemainTimeStr2ByEndTime(slot1)
	end
...
function slot0.checkUpdateStoreActivity(slot0)
	...
	if slot0:getStoreGroupMO() and slot1[StoreEnum.TowerStore.UpdateStore] then
		for slot7, slot8 in pairs(slot3:getGoodsList()) do
			...
			elseif slot2 ~= slot8.config.activityId then
				logError("回声港湾的红帆陈列馆绑定活动id不一致：" .. "  " .. slot8.config.id)

				return slot8.config.activityId
			end
		end
	end
...
function slot0.getStoreNewGoodsPrefKey(slot0)
	return "TowerStoreModel_StoreNewGoods_" .. (PlayerModel.instance:getPlayinfo() and slot2.userId or 1999) .. "|" .. (slot0:checkUpdateStoreActivity() or 0)
end
...
function slot0.saveAllStoreGroupNewData(slot0)
	slot1 = StoreEnum.TowerStore.NormalStore
...
function slot0.readAllStoreGroupNewData(slot0)
	slot0:readStoreGroupNewMo(StoreEnum.TowerStore.NormalStore)
end
...
function slot0.isHasNewGoods(slot0)
	if slot0._storeGroupGoodsMo and slot0._storeGroupGoodsMo[StoreEnum.TowerStore.NormalStore] then
		for slot6, slot7 in pairs(slot2) do
			if slot7.isNew then
				return true
			end
...
function slot0.getStoreGoodsNewDataPrefKey(slot0, slot1)
	return "TowerStoreModel_StoreGoodsNewData_" .. slot1 .. "_" .. (PlayerModel.instance:getPlayinfo() and slot2.userId or 1999)
end

function slot0.isUpdateStoreEmpty(slot0)
	slot2 = slot0:getStoreGroupMO() and slot1[StoreEnum.TowerStore.UpdateStore]

	return not slot2 or next(slot2:getGoodsList()) == nil
end
