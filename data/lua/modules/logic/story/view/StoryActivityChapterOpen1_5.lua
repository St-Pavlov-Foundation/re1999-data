...
slot0 = class("StoryActivityChapterOpen1_5", StoryActivityChapterBase)

function slot0.onCtor(slot0)
	slot0.assetPath = "ui/viewres/story/v1a5/storyactivitychapteropen.prefab"
end
...
function slot0.play(slot0)
	...
	slot0._simageName:LoadImage(slot0:getLangResBg(string.format("titleen_%s", slot4)), slot0.onNameLoaded, slot0)

	slot0._txtTitle.text = tonumber(StoryConfig.instance:getActivityOpenConfig(slot3, slot4) and slot8.labelRes or 1) == 0 and luaLang("prelude") or formatLuaLang("storyactivitysection", GameUtil.getNum2Chinese(slot11))
...
function slot0.getLangResBg(slot0, slot1)
	return string.format("singlebg_lang/txt_v1a5_storyactivityopenclose/%s.png", slot1)
end

function slot0.getVideoName(slot0, slot1)
	return slot1 == 1 and "1_5_kaimu"
end

function slot0.getAudioId(slot0, slot1)
	return slot1 == 1 and AudioEnum.Story.play_activitysfx_wulu_chapter_open or AudioEnum.Story.play_activitysfx_wulu_subsection_open
end
