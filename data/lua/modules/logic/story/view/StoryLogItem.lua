...
function slot0._onStopClick(slot0)
	...
	if StoryStepModel.instance:getStepListById(slot0._mo.info).conversation.type == StoryEnum.ConversationType.Player or slot1.nameShow and slot1.heroNames[LanguageEnum.LanguageStoryType.CN] == luaLang("mainrolename") and slot1.type ~= StoryEnum.ConversationType.None and slot1.type ~= StoryEnum.ConversationType.ScreenDialog then
		slot0:_setItemContentColor("#CCAD8F")
	else
		slot0:_setItemContentColor("#EEF1E8")
		SLFramework.UGUI.GuiHelper.SetColor(slot0._gonorole:GetComponent(gohelper.Type_Image), "#DFE2D9")
...
function slot0.onUpdateMO(slot0, slot1, slot2)
	...
	if type(slot1.info) == "number" then
		...
		if slot3.effType == StoryEnum.ConversationEffectType.Magic then
			slot4 = StoryConfig.instance:replaceStoryMagicText(string.find(StoryStepModel.instance:getStepListById(slot1.info).conversation.diaTexts[GameLanguageMgr.instance:getLanguageTypeStoryIndex()], "<voffset") and slot3.diaTexts[GameLanguageMgr.instance:getLanguageTypeStoryIndex()] or GameUtil.filterRichText(slot3.diaTexts[GameLanguageMgr.instance:getLanguageTypeStoryIndex()]))
		end
		...
		if slot3.type == StoryEnum.ConversationType.Player or slot3.nameShow and slot3.heroNames[LanguageEnum.LanguageStoryType.CN] == luaLang("mainrolename") and slot3.type ~= StoryEnum.ConversationType.None and slot3.type ~= StoryEnum.ConversationType.ScreenDialog then
			gohelper.setActive(slot0._goicon, true)
			slot0:_setItemContentColor("#CCAD8F")
		else
			gohelper.setActive(slot0._goicon, false)
		end
	elseif type(slot1.info) == "table" then
