...
function slot0.onInitView(slot0)
	...
	slot0._txtdeco = gohelper.findChildText(slot0._gorewarditem, "#go_birthdayrewarditem/#go_birthday/ScrollView/Viewport/#txt_deco")
...
function slot0._onTimesValueChanged(slot0, slot1)
	if not slot0:_getTextInputheros() or slot2 == "" then
		return
	end

	if not string.splitToNumber(slot2, "|") or #slot3 < 1 then
		return
	end
...
function slot0._setTitleInfo(slot0)
	...
	UISpriteSetMgr.instance:setSignInSprite(slot0._imageweek, "date_" .. tostring(os.date("%w", slot1)))
...
function slot0._setMonthViewRewardTips(slot0)
	slot0._txtdaycount.text = SignInModel.instance:getTotalSignDays()
	slot2 = SignInConfig.instance:getSignMonthRewards()

	for slot6 = 1, 3 do
		slot0:_showMonthRewardInfo(slot6)
	end

	slot3 = SignInModel.instance:isSignTotalRewardGet(1)
	slot4 = tonumber(SignInConfig.instance:getSignMonthReward(1).signinaddup) <= SignInModel.instance:getTotalSignDays()

	gohelper.setActive(slot0._gorewardmark1, not slot3 and slot4)
	gohelper.setActive(slot0._gogetmonthbg1, slot4)
	...
	slot5 = SignInModel.instance:isSignTotalRewardGet(2)
	slot6 = tonumber(SignInConfig.instance:getSignMonthReward(2).signinaddup) <= SignInModel.instance:getTotalSignDays()

	gohelper.setActive(slot0._gorewardmark2, not slot5 and slot6)
	gohelper.setActive(slot0._gogetmonthbg2, slot6)
	...
	slot7 = SignInModel.instance:isSignTotalRewardGet(3)
	slot8 = tonumber(SignInConfig.instance:getSignMonthReward(3).signinaddup) <= SignInModel.instance:getTotalSignDays()

	gohelper.setActive(slot0._gorewardmark3, not slot7 and slot8)
	gohelper.setActive(slot0._gogetmonthbg3, slot8)
	...
	slot9 = SignInModel.instance:getSignTargetDate()

	slot0:_setGoldRewards(slot9)

	slot0._curDayRewards = string.splitToNumber(SignInConfig.instance:getSignRewards(slot9[3]).signinBonus, "#")
	slot10, slot11 = ItemModel.instance:getItemConfigAndIcon(slot0._curDayRewards[1], slot0._curDayRewards[2], true)
	...
	slot0._simagenormaldayrewardicon:LoadImage(slot11)
	slot0._simagemonthcarddayrewardicon:LoadImage(slot11)

	if tonumber(slot0._curDayRewards[1]) == MaterialEnum.MaterialType.Equip then
		slot0._simagenormaldayrewardicon:LoadImage(ResUrl.getPropItemIcon(slot10.icon))
		slot0._simagemonthcarddayrewardicon:LoadImage(ResUrl.getPropItemIcon(slot10.icon))
	end

	slot12 = SignInModel.instance:getValidMonthCard(slot9[1], slot9[2], slot9[3])

	gohelper.setActive(slot0._gomonthcard, slot12)
	gohelper.setActive(slot0._gonormal, not slot12)

	if slot12 then
		slot13 = string.split(StoreConfig.instance:getMonthCardConfig(slot12).dailyBonus, "|")
		slot0._curmonthCardRewards = string.splitToNumber(slot13[1], "#")
		slot0._curmonthCardPower = string.splitToNumber(slot13[2], "#")
		...
		slot14, slot15 = ItemModel.instance:getItemConfigAndIcon(slot0._curmonthCardRewards[1], slot0._curmonthCardRewards[2], true)

		slot0._simagemonthcardicon:LoadImage(slot15)

		slot16, slot17 = ItemModel.instance:getItemConfigAndIcon(slot0._curmonthCardPower[1], slot0._curmonthCardPower[2], true)

		slot0._simagemonthcardpowericon:LoadImage(slot17)
		gohelper.setActive(slot0._gopowerlimittime, false)

		if slot16.expireTime then
			gohelper.setActive(slot0._gopowerlimittime, true)
		end

		if StoreModel.instance:getMonthCardInfo() then
			if slot18:getRemainDay() > 0 then
				gohelper.setActive(slot0._golimittime.gameObject, true)

				if slot19 <= StoreEnum.MonthCardStatus.NotEnoughThreeDay then
				...
				slot0._txtlimittime.text = formatLuaLang("remain_day", slot19)
			else
				gohelper.setActive(slot0._golimittime.gameObject, false)
			end
		end

		slot20 = slot0._curDate.year == slot0._targetDate[1] and slot0._curDate.month == slot0._targetDate[2] and slot0._curDate.day == slot0._targetDate[3] and not slot0._rewardGetState

		gohelper.setActive(slot0._gomonthcardget, slot20)
		gohelper.setActive(slot0._gomonthcarddayget, slot20)
		gohelper.setActive(slot0._gomonthcarddayget_gold, slot20)
		gohelper.setActive(slot0._gomonthcardnoget, not slot20)
		gohelper.setActive(slot0._gomonthcardpowernoget, not slot20)
		gohelper.setActive(slot0._gomonthcarddaynoget, not slot20)
		gohelper.setActive(slot0._gomonthcarddaynoget_gold, not slot20)
	end
end
...
function slot0._showMonthRewardInfo(slot0, slot1)
	...
	slot0["_txtrewardcount" .. slot1].text = string.format("%s%s", luaLang("multiple"), slot2.reward[3])
