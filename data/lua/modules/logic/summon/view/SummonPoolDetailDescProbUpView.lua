...
function slot0.addProbUpItemByRare(slot0, slot1)
	...
	for slot6, slot7 in ipairs(slot2) do
		...
		if slot0._poolType == SummonEnum.Type.MultiProbUp4 and slot1 == uv0 then
			slot8.txtProbability.text = tostring(SummonEnum.MultiProbUp4ShowRate) .. "%"
			slot8.txtProbabilityLabel.text = string.format(luaLang("summon_multiup4_rule_title"), slot1 + 1)
		else
			slot8.txtProbability.text = CommonConfig.instance:getConstNum(ConstEnum.SummonSSRUpProb) / 10 .. "%"
			slot8.txtProbabilityLabel.text = string.format(luaLang("p_summonpooldetail_up_probability"), slot1 + 1)
		end
	end
end
...
function slot0.getProbUpItem(slot0, slot1)
	if not slot0._probUpItemMap[slot1] then
		...
		slot2.equipIcons = {}
		slot8 = slot1
		slot3 = gohelper.clone(slot0._godesctitle, slot0._goheroitem, "prob_up_item_" .. tostring(slot8))
		slot2.go = slot3
