...
function slot0._refreshItem(slot0)
	if slot0._mo then
		...
		slot0._pooltype.text = slot0._mo.poolName
		...
		if slot0._colorStr ~= (SummonEnum.HistoryColor[slot2 or 1] or SummonEnum.HistoryColor[1]) then
			slot0._colorStr = slot3

			SLFramework.UGUI.GuiHelper.SetColor(slot0._name, slot3)

			if slot2 >= 4 then
				SLFramework.UGUI.GuiHelper.SetColor(slot0._imgstar, slot3)
			end
		end
	end
