...
function slot0._refresh(slot0)
	...
	if slot3[1] == MaterialEnum.MaterialType.Hero then
		...
		slot0._txtdesc.text = string.format("%s", ActivityConfig.instance:getActivityCo(slot0._actId).actDesc)
		slot0._txtreward.text = string.format("%s<color=#c66030>%s</color>。", luaLang("activitynovicesign_character"), slot4.name)

		if not slot0._hasCreateStar then
			for slot9 = 1, 4 do
				gohelper.cloneInPlace(slot0._gostaricon, "star" .. slot9)
			end
