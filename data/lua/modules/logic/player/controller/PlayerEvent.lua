module("modules.logic.player.controller.PlayerEvent", package.seeall)

slot0 = _M
slot0.PlayerbassinfoChange = 1
slot0.SetShowHero = 2
slot0.SetPortrait = 3
slot0.SelectPortrait = 4
slot0.ChangePlayerinfo = 5
slot0.PlayerLevelUp = 6
slot0.ChangePlayerName = 7
slot0.NickNameConfirmNo = 8
slot0.NickNameConfirmYes = 9
slot0.RenameReplyFail = 10
slot0.SelectCloth = 11
slot0.ShowClothSkillTips = 12
slot0.UpdateSimpleProperty = 13
slot0.UpdateAssistRewardCount = 14
slot0.OnDailyRefresh = 15
slot0.RenameFlagUpdate = 20
slot0.ChangeBgTab = 1000
slot0.ShowHideRoot = 1001

return slot0
