...
function slot0._initPlayerbassinfo(slot0, slot1)
	...
	slot0._txtentryday.text = TimeUtil.timestampToString3(ServerTime.timeInLocal(slot1.registerTime / 1000))
...
function slot0._initPlayerOtherinfo(slot0, slot1)
	...
	if slot1.lastEpisodeId and lua_episode.configDict[slot2] then
		...
		if slot3 then
			slot0._txtepisodeprogress.text = string.format("《%s %s》", DungeonController.getEpisodeName(slot3), slot3.name)
			slot0._txtepisodeprogressen.text = slot3.name_En
		end
	end

	slot4 = slot1.level
	slot0._txtlevel.text = slot4
	slot5 = slot1.exp
	slot6 = 0

	if slot4 < CommonConfig.instance:getConstNum(ConstEnum.PlayerMaxLev) then
		slot6 = PlayerConfig.instance:getPlayerLevelCO(slot4 + 1).exp
	else
		slot5 = PlayerConfig.instance:getPlayerLevelCO(slot4).exp
	end

	slot0._txtexp.text = slot5 .. "/" .. slot6

	slot0._imageexp:SetValue(slot5 / slot6)
end
