-- chunkname: @modules/logic/versionactivity3_3/enter/view/subview/VersionActivity3_3DungeonEnterView.lua

module("modules.logic.versionactivity3_3.enter.view.subview.VersionActivity3_3DungeonEnterView", package.seeall)

local VersionActivity3_3DungeonEnterView = class("VersionActivity3_3DungeonEnterView", BaseView)

function VersionActivity3_3DungeonEnterView:onInitView()
...
function VersionActivity3_3DungeonEnterView:addEvents()
...
function VersionActivity3_3DungeonEnterView:removeEvents()
...
function VersionActivity3_3DungeonEnterView:_onUpdateDungeonInfo()
...
function VersionActivity3_3DungeonEnterView:onRefreshActivity(actId)
	if self._hasPreviewFlag then
		self:refreshPreviewStatus()
	end
...
function VersionActivity3_3DungeonEnterView:_btnboardOnClick()
	CommandStationController.instance:openCommandStationPaperView()
end

function VersionActivity3_3DungeonEnterView:_btnstoreOnClick()
	VersionActivity3_3DungeonController.instance:openStoreView()
end

function VersionActivity3_3DungeonEnterView:_btnenterOnClick()
	...
	if self._chapterId and DungeonMainStoryModel.instance:showPreviewChapterFlag(self._chapterId) then
		...
		if not PlayerPrefsHelper.hasKey(key) then
			GameFacade.showMessageBox(MessageBoxIdDefine.PreviewChapterOpenTip, MsgBoxEnum.BoxType.Yes_No, function()
				PlayerPrefsHelper.setNumber(key, 1)
				VersionActivity3_3DungeonController.instance:openVersionActivityDungeonMapView()
			end, nil, nil)
...
function VersionActivity3_3DungeonEnterView:_btnFinishedOnClick()
	return
end

function VersionActivity3_3DungeonEnterView:_editableInitView()
	...
	self.actId = VersionActivity3_3Enum.ActivityId.Dungeon
	self.animComp = VersionActivity3_3SubAnimatorComp.get(self.viewGO, self)
...
function VersionActivity3_3DungeonEnterView:_updateBg()
	if DungeonModel.instance:hasPassLevelAndStory(11115) then
		self._simagebg:LoadImage("singlebg/v3a3_mainactivity_singlebg/v3a3_enterview_fullbg.png")
	else
		self._simagebg:LoadImage("singlebg/v3a3_mainactivity_singlebg/v3a3_enterview_fullbg.png")
	end
end

function VersionActivity3_3DungeonEnterView:refreshDot()
	RedDotController.instance:addRedDot(self._goreddot, RedDotEnum.DotNode.V3a3DungeonEnter)
end

function VersionActivity3_3DungeonEnterView:_setDesc()
...
function VersionActivity3_3DungeonEnterView:onUpdateParam()
	self:refreshUI()
end

function VersionActivity3_3DungeonEnterView:onOpen()
	self:refreshUI()
	self.animComp:playOpenAnim()
	TaskDispatcher.runRepeat(self.everyMinuteCall, self, TimeUtil.OneMinuteSecond)
end

function VersionActivity3_3DungeonEnterView:onOpenFinish()
	self._videoPath = langVideoUrl(VersionActivity3_3Enum.EnterLoopVideoName)
...
function VersionActivity3_3DungeonEnterView:onPlayVideoDone()
...
function VersionActivity3_3DungeonEnterView:everyMinuteCall()
	self:refreshUI()
end

function VersionActivity3_3DungeonEnterView:refreshUI()
...
function VersionActivity3_3DungeonEnterView:refreshPaperCount()
...
function VersionActivity3_3DungeonEnterView:refreshPreviewStatus()
	gohelper.setActive(self._gonewunlock, false)
end

function VersionActivity3_3DungeonEnterView:refreshRemainTime()
	...
	local storeActInfoMo = ActivityModel.instance:getActivityInfo()[VersionActivity3_3Enum.ActivityId.DungeonStore]

	self._txtstorename.text = storeActInfoMo.config.name
	self._txtStoreTime.text = storeActInfoMo:getRemainTimeStr2ByEndTime(true)
end

function VersionActivity3_3DungeonEnterView:refreshActivityState()
	...
	if not isNormal then
		local enterStatus = ActivityHelper.getActivityStatusAndToast(VersionActivity3_3Enum.ActivityId.EnterView)
	...
	local storeStatus = ActivityHelper.getActivityStatusAndToast(VersionActivity3_3Enum.ActivityId.DungeonStore)
	local isStoreNormal = storeStatus == ActivityEnum.ActivityStatus.Normal

	gohelper.setActive(self.goStore, isStoreNormal)
end

function VersionActivity3_3DungeonEnterView:refreshStoreCurrency()
...
function VersionActivity3_3DungeonEnterView:onClose()
	TaskDispatcher.cancelTask(self.everyMinuteCall, self)
end

function VersionActivity3_3DungeonEnterView:onDestroyView()
...
return VersionActivity3_3DungeonEnterView
