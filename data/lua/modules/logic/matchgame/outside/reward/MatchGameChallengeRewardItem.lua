-- chunkname: @modules/logic/matchgame/outside/reward/MatchGameChallengeRewardItem.lua

module("modules.logic.matchgame.outside.reward.MatchGameChallengeRewardItem", package.seeall)

local MatchGameChallengeRewardItem = class("MatchGameChallengeRewardItem", ListScrollCellExtend)

function MatchGameChallengeRewardItem:onInitView()
...
function MatchGameChallengeRewardItem:addEvents()
	return
end

function MatchGameChallengeRewardItem:removeEvents()
	return
end

function MatchGameChallengeRewardItem:refresh(data)
	if data then
...
function MatchGameChallengeRewardItem:onUpdateMO(rewardCo)
...
function MatchGameChallengeRewardItem:refreshReward()
...
function MatchGameChallengeRewardItem:createRewardItem(index)
...
function MatchGameChallengeRewardItem:refreshRewardItem(item, data)
	item.data = data
...
function MatchGameChallengeRewardItem:onClickItem(item)
	if not self.config then
		return
	end

	if self.status == MatchGameEnum.RewardItemStatus.CanGet then
		MatchGameRpc.instance:sendAct244ReceiveChallengeBonusRequest(self.config.activityId)
	elseif item.data then
		MaterialTipController.instance:showMaterialInfo(item.data[1], item.data[2])
	end
end

function MatchGameChallengeRewardItem:refreshChapter()
...
function MatchGameChallengeRewardItem:_editableInitView()
	return
end

function MatchGameChallengeRewardItem:onDestroyView()
...
return MatchGameChallengeRewardItem
