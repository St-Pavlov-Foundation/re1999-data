...
function slot0.checkRecommendIsInTime(slot0, slot1)
	...
	if not string.nilorempty(slot1.onlineTime) then
		slot4 = TimeUtil.stringToTimestamp(slot1.onlineTime) + ServerTime.clientToServerOffset()
	end

	if not string.nilorempty(slot1.offlineTime) then
		slot5 = TimeUtil.stringToTimestamp(slot1.offlineTime) + ServerTime.clientToServerOffset()
	end

	if not string.nilorempty(slot1.constTime) then
		slot3 = TimeUtil.stringToTimestamp(slot1.constTime) + ServerTime.clientToServerOffset()
	end
