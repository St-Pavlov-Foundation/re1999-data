...
slot0 = class("Season1_5MarketView", BaseView)
...
function slot0._btnstartOnClick(slot0)
	Activity104Model.instance:enterAct104Battle(SeasonConfig.instance:getSeasonEpisodeCo(Activity104Model.instance:getCurSeasonId(), slot0._layer).episodeId, slot0._layer)
end
...
function slot0._setLevels(slot0)
	...
	if slot0._layer <= 4 then
		for slot7 = 1, 6 do
			slot0._showLvItems[slot7] = Season1_5MarketShowLevelItem.New()
...
function slot0._setStages(slot0)
	...
	slot0._simagedecorate:LoadImage(SeasonViewHelper.getSeasonIcon(string.format("icon/b_chatu_%s.png", slot1.stagePicture)))
end
...
function slot0.refreshEquipCardItem(slot0)
	...
	if slot1.firstPassEquipId > 0 then
		if not slot0._rewardItems[1].itemIcon then
			slot0._rewardItems[1].itemIcon = Season1_5CelebrityCardItem.New()
