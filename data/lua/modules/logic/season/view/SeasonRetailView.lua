-- chunkname: @modules/logic/season/view/SeasonRetailView.lua

module("modules.logic.season.view.SeasonRetailView", package.seeall)

local SeasonRetailView = class("SeasonRetailView", BaseView)

function SeasonRetailView:onInitView()
...
function SeasonRetailView:addEvents()
...
function SeasonRetailView:removeEvents()
...
function SeasonRetailView:_btngo1OnClick()
	self:_enterLevelInfoView(1)
end

function SeasonRetailView:_btngo2OnClick()
	self:_enterLevelInfoView(2)
end

function SeasonRetailView:_btngo3OnClick()
	self:_enterLevelInfoView(3)
end

function SeasonRetailView:_btngo4OnClick()
	self:_enterLevelInfoView(4)
end

function SeasonRetailView:_btngo5OnClick()
	self:_enterLevelInfoView(5)
end

function SeasonRetailView:_btngo6OnClick()
	self:_enterLevelInfoView(6)
end

function SeasonRetailView:_btnruledetailOnClick()
	gohelper.setActive(self._goruletipdetail, true)
end

function SeasonRetailView:_btncloseruletipOnClick()
	gohelper.setActive(self._goruletipdetail, false)
end

function SeasonRetailView:_btncurrencyiconOnClick()
...
function SeasonRetailView:_enterLevelInfoView(index)
...
function SeasonRetailView:_btnsummonOnClick()
	if self._hasEnoughTicket then
		local function requestRefreshRetail()
		...
		if tabletool.len(retails) == 0 then
			requestRefreshRetail()
		else
			GameFacade.showMessageBox(MessageBoxIdDefine.SeasonRetailTicketLimited, MsgBoxEnum.BoxType.Yes_No, requestRefreshRetail)
		end
	else
...
function SeasonRetailView:_editableInitView()
	return
end

function SeasonRetailView:onUpdateParam()
	return
end

function SeasonRetailView:onOpen()
...
function SeasonRetailView:_onRefreshRetailSuccess()
...
function SeasonRetailView:onSwitchCard()
	self:_refreshLevel()
	self:_refreshTitle()
end

function SeasonRetailView:_refreshTitle()
	local actId = Activity104Model.instance:getCurSeasonId()
	local id = SeasonConfig.instance:getRetailTicket(actId)
...
function SeasonRetailView:_setStages()
...
function SeasonRetailView:_refreshLevel()
	local actId = Activity104Model.instance:getCurSeasonId()
	local id = SeasonConfig.instance:getRetailTicket(actId)
...
function SeasonRetailView:_showEntrance()
	...
	for _, v in pairs(retails) do
		...
		self["_txtlevelnum" .. v.position].text = SeasonConfig.instance:getSeasonTagDesc(actId, v.tag).name .. " " .. GameUtil.getRomanNums(showStage)
		...
		local equipCount = math.min(#v.showActivity104EquipIds, 3)
		...
		for i = 1, equipCount do
			...
			if not self._rewardCardItems[v.position].celebrityCards[i] then
				self._rewardCardItems[v.position].celebrityCards[i] = SeasonCelebrityCardItem.New()
...
function SeasonRetailView:onClose()
...
function SeasonRetailView:onDestroyView()
