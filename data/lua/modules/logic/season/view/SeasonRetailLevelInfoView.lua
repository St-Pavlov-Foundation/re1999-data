...
slot0 = class("SeasonRetailLevelInfoView", BaseView)
...
function slot0._btnstartOnClick(slot0)
	Activity104Rpc.instance:sendBeforeStartAct104BattleRequest(ActivityEnum.Activity.Season, 0, slot0.viewParam.episodeId)
end
...
function slot0._setInfo(slot0)
	...
	slot0._txtlevelname.text = SeasonConfig.instance:getSeasonTagDesc(Activity104Model.instance:getCurSeasonId(), slot0.viewParam.retail.tag).name .. " " .. GameUtil.getRomanNums(math.min(Activity104Model.instance:getAct104CurStage(), 6))
	slot0._txtenemylevelnum.text = HeroConfig.instance:getCommonLevelDisplay(SeasonConfig.instance:getSeasonRetailCo(ActivityEnum.Activity.Season, Activity104Model.instance:getRetailStage()).level)
	...
	for slot8 = 1, #slot0.viewParam.retail.showActivity104EquipIds do
		if not slot0._cardItems[slot8] then
			slot0._cardItems[slot8] = SeasonCelebrityCardItem.New()
