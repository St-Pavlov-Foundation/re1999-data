-- chunkname: @modules/logic/season/view/SeasonHeroGroupFightViewRule.lua

module("modules.logic.season.view.SeasonHeroGroupFightViewRule", package.seeall)

local SeasonHeroGroupFightViewRule = class("SeasonHeroGroupFightViewRule", BaseView)

function SeasonHeroGroupFightViewRule:onInitView()
...
function SeasonHeroGroupFightViewRule:addEvents()
...
function SeasonHeroGroupFightViewRule:removeEvents()
...
function SeasonHeroGroupFightViewRule:_btncloseruleOnClick()
...
function SeasonHeroGroupFightViewRule:_btnadditionRuleOnClick()
...
function SeasonHeroGroupFightViewRule:_btnenemyOnClick()
	EnemyInfoController.instance:openEnemyInfoViewByBattleId(HeroGroupModel.instance.battleId)
end

function SeasonHeroGroupFightViewRule:_editableInitView()
...
function SeasonHeroGroupFightViewRule:_refreshRules()
...
function SeasonHeroGroupFightViewRule:_refreshAddition()
	local episodeId = HeroGroupModel.instance.episodeId
...
function SeasonHeroGroupFightViewRule:_addRuleItem(ruleCo, targetId)
...
function SeasonHeroGroupFightViewRule:_setRuleDescItem(ruleCo, targetId)
	local tagColor = {
		"#6680bd",
		"#d05b4c",
		"#c7b376"
...
function SeasonHeroGroupFightViewRule:_refreshEnemy()
	...
	if episodeConfig.type == DungeonEnum.EpisodeType.Season then
		...
		level = SeasonConfig.instance:getSeasonEpisodeCo(ActivityEnum.Activity.Season, layer).level
	elseif episodeConfig.type == DungeonEnum.EpisodeType.SeasonRetail then
		local stage = Activity104Model.instance:getRetailStage()

		level = SeasonConfig.instance:getSeasonRetailCo(ActivityEnum.Activity.Season, stage).level
	elseif episodeConfig.type == DungeonEnum.EpisodeType.SeasonSpecial then
		local layer = Activity104Model.instance:getBattleFinishLayer()

		level = SeasonConfig.instance:getSeasonSpecialCo(ActivityEnum.Activity.Season, layer).level
	end
...
function SeasonHeroGroupFightViewRule:_recommendCareer()
...
function SeasonHeroGroupFightViewRule:_onRecommendCareerItemShow(obj, data, index)
	local icon = gohelper.findChildImage(obj, "icon")

	UISpriteSetMgr.instance:setHeroGroupSprite(icon, "career_" .. data)
end

function SeasonHeroGroupFightViewRule:_onEnemyItemShow(obj, data, index)
	local icon = gohelper.findChildImage(obj, "icon")
...
function SeasonHeroGroupFightViewRule:onClose()
	self:removeEventCb(HeroGroupController.instance, HeroGroupEvent.OnModifyHeroGroup, self._recommendCareer, self)
end

function SeasonHeroGroupFightViewRule:onDestroyView()
	return
end

return SeasonHeroGroupFightViewRule
