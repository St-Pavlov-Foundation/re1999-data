...
slot0 = class("Season1_2HeroGroupHeroItem", LuaCompBase)

function slot0.ctor(slot0, slot1)
	slot0._Season1_2HeroGroupListView = slot1
end
...
function slot0.init(slot0, slot1)
	...
	slot0.anim = slot0._goheroitem:GetComponent(typeof(slot5))
	slot0._subGO = gohelper.findChild(slot1, "heroitemani/aidtag")
	slot0._aidGO = gohelper.findChild(slot1, "heroitemani/storytag")
	...
	slot0._cardItem1 = Season1_2HeroGroupCardItem.New(slot0._gocarditem1, slot0, {
		slot = 1
	})
	slot0._gocarditem2 = gohelper.findChild(slot1, "heroitemani/equipcard/cardlist/carditem2")
	slot0._cardItem2 = Season1_2HeroGroupCardItem.New(slot0._gocarditem2, slot0, {
		slot = 2
	})
...
function slot0.onUpdateMO(slot0, slot1)
	...
	elseif slot0._monsterCo then
		...
		slot0._simagecharactericon:LoadImage(ResUrl.getHeadIconMiddle(slot3.retangleIcon))
	...
	if slot0.isRoleNumLock and slot0._heroMO ~= nil and slot0._monsterCo == nil then
		...
		slot8 = Activity104Model.instance:getSeasonCurSnapshotSubId(Activity104Model.instance:getCurSeasonId())
...
function slot0._updateAct104Equips(slot0)
	slot2 = HeroGroupModel.instance:getCurGroupMO():getAct104PosEquips(slot0.mo.id - 1).equipUid or {}
	...
	slot3 = slot1.isReplay and (slot8 and slot8[2] and slot8[2].equipId or 0) or nil
	...
	slot8 = not slot0.isAid and (slot0._cardItem1.slotUnlock or slot0._cardItem2.slotUnlock)
	...
	slot0._cardItem1:setActive(slot8)
	slot0._cardItem2:setActive(slot8)
...
function slot0.onDragEndSuccess(slot0, slot1)
	EquipTeamListModel.instance:openTeamEquip(slot0.mo.id - 1, slot0._heroMO, Activity104Model.instance:getRealHeroGroupBySubId())
	...
	if EquipModel.instance:getEquip(EquipTeamListModel.instance:getTeamEquip(slot0.mo.id - 1)[1]) and slot7 or nil then
		EquipTeamShowItem.removeEquip(slot5, true)
	end

	if EquipModel.instance:getEquip(EquipTeamListModel.instance:getTeamEquip(slot6)[1]) and slot8 or nil then
		EquipTeamShowItem.removeEquip(slot6, true)
	end

	if slot7 then
		EquipTeamShowItem.replaceEquip(slot6, slot7, true)
	end

	if slot8 then
		EquipTeamShowItem.replaceEquip(slot5, slot8, true)
	end
...
function slot0._moveToTarget(slot0, slot1)
	for slot5, slot6 in ipairs(slot0._Season1_2HeroGroupListView.heroPosTrList) do
		if slot0._Season1_2HeroGroupListView._heroItemList[slot5] ~= slot0 then
			slot7 = slot6.parent
			...
			if math.abs(recthelper.screenPosToAnchorPos(slot1, slot7).x) * 2 < recthelper.getWidth(slot7) and math.abs(slot8.y) * 2 < recthelper.getHeight(slot7) then
				return slot0._Season1_2HeroGroupListView._heroItemList[slot5]
			end
		end
	end
...
function slot0._setEquipDragEnabled(slot0, slot1)
	for slot5, slot6 in ipairs(slot0._Season1_2HeroGroupListView._heroItemList) do
		if slot6._drag then
...
function slot0.getHeroItemList(slot0)
	return slot0._Season1_2HeroGroupListView:getHeroItemList()
end
