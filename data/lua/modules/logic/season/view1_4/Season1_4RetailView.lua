...
slot0 = class("Season1_4RetailView", BaseView)
...
function slot0.onInitView(slot0)
	...
	slot0._txtsummon2 = gohelper.findChildText(slot0.viewGO, "right/#go_summon/circle/#txt_summon2")
	...
	MainCameraMgr.instance:addView(ViewName.Season1_4RetailView, slot0.autoInitRetailViewCamera, nil, slot0)
...
function slot0._btnsummonOnClick(slot0)
	if slot0._waitRefreshingRetailReply then
		return
	end
	...
	if slot0._hasEnoughTicket then
		if tabletool.len(Activity104Model.instance:getAct104Retails()) == 0 then
			function (slot0)
				Activity104Rpc.instance:sendRefreshRetailRequest(Activity104Model.instance:getCurSeasonId())

				slot0._waitRefreshingRetailReply = false
			end(slot0)
		else
			GameFacade.showMessageBox(MessageBoxIdDefine.SeasonRetailTicketLimited, MsgBoxEnum.BoxType.Yes_No, slot1, nil, , slot0)
		end
	else
...
function slot0._refreshConstTips(slot0)
	...
	if slot3 ~= 0 then
		if not slot0._rareCard then
			slot0._rareCard = Season1_4CelebrityCardItem.New()
...
function slot0._showEntrance(slot0)
	...
	for slot7, slot8 in pairs(Activity104Model.instance:getAct104Retails()) do
		...
		slot0["_txtlevelnum" .. slot8.position].text = string.format("%s %s", SeasonConfig.instance:getSeasonTagDesc(Activity104Model.instance:getCurSeasonId(), slot8.tag).name, GameUtil.getRomanNums(math.min(Activity104Model.instance:getAct104CurStage(), 6)))
		...
		if slot8.advancedId ~= 0 and slot8.advancedRare == 2 then
			...
			for slot15, slot16 in pairs(slot8.showActivity104EquipIds) do
				if SeasonConfig.instance:getSeasonEquipCo(slot16).isOptional == 1 then
					slot11 = slot17.name
			...
			gohelper.findChildText(slot0["_gospecialtips" .. slot8.position], "tips").text = string.format(luaLang("season_retail_specialtips"), slot11)
		end

		slot11 = gohelper.findChild(slot0["_scrollcelebritycard" .. slot8.position].gameObject, "scrollcontent_seasoncelebritycarditem")

		if #slot0._rewardCardItems[slot8.position].celebrityCards > 0 then
			slot15 = slot8.position

			for slot15, slot16 in pairs(slot0._rewardCardItems[slot15].celebrityCards) do
				gohelper.setActive(slot16.go, false)
			end
		end

		for slot15 = 1, 3 do
			if not slot0._rewardCardItems[slot8.position].celebrityCards[slot15] then
				slot0._rewardCardItems[slot8.position].celebrityCards[slot15] = Season1_4CelebrityCardItem.New()
				uv0.showNewFlag2 = Activity104Model.instance:isNew104Equip(slot8.showActivity104EquipIds[slot15])

				slot0._rewardCardItems[slot8.position].celebrityCards[slot15]:init(slot11, slot16, uv0)
...
function slot0.autoInitRetailViewCamera(slot0)
