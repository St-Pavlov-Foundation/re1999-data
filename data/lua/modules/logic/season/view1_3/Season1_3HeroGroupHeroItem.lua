...
slot0 = class("Season1_3HeroGroupHeroItem", LuaCompBase)

function slot0.ctor(slot0, slot1)
	slot0._Season1_3HeroGroupListView = slot1
end
...
function slot0.init(slot0, slot1)
	...
	slot0._cardItem1 = Season1_3HeroGroupCardItem.New(slot0._gocarditem1, slot0, {
		slot = 1
	})
	slot0._gocarditem2 = gohelper.findChild(slot1, "heroitemani/equipcard/cardlist/carditem2")
	slot0._cardItem2 = Season1_3HeroGroupCardItem.New(slot0._gocarditem2, slot0, {
		slot = 2
	})
...
function slot0.onUpdateMO(slot0, slot1)
	...
	elseif slot0.trialCO then
		...
		slot0._simagecharactericon:LoadImage(ResUrl.getHeadIconMiddle(((slot0.trialCO.skin <= 0 or SkinConfig.instance:getSkinCo(slot0.trialCO.skin)) and SkinConfig.instance:getSkinCo(slot3.skinId)).retangleIcon))
...
function slot0._updateAct104Equips(slot0)
	...
	slot3 = not slot1.isReplay and slot2[1]
	slot4 = not slot1.isReplay and slot2[2]
	slot5 = Activity104Model.instance:getItemIdByUid(slot2[1])
	slot6 = Activity104Model.instance:getItemIdByUid(slot2[2])

	if slot1.isReplay then
		slot5 = slot1.replay_activity104Equip_data[slot0.mo.heroUid] and slot7[1] and slot7[1].equipId or 0
		slot6 = slot7 and slot7[2] and slot7[2].equipId or 0
	end

	slot0._cardItem1:setData(slot0.mo, slot5, slot3)
	slot0._cardItem2:setData(slot0.mo, slot6, slot4)
...
function slot0._moveToTarget(slot0, slot1)
	for slot5, slot6 in ipairs(slot0._Season1_3HeroGroupListView.heroPosTrList) do
		if slot0._Season1_3HeroGroupListView._heroItemList[slot5] ~= slot0 then
			slot7 = slot6.parent
			...
			if math.abs(recthelper.screenPosToAnchorPos(slot1, slot7).x) * 2 < recthelper.getWidth(slot7) and math.abs(slot8.y) * 2 < recthelper.getHeight(slot7) then
				return slot0._Season1_3HeroGroupListView._heroItemList[slot5]
			end
		end
	end
...
function slot0._setEquipDragEnabled(slot0, slot1)
	for slot5, slot6 in ipairs(slot0._Season1_3HeroGroupListView._heroItemList) do
		if slot6._drag then
...
function slot0.getHeroItemList(slot0)
	return slot0._Season1_3HeroGroupListView:getHeroItemList()
end
