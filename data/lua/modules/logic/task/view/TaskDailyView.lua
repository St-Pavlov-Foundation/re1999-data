...
slot0 = class("TaskDailyView", BaseView)
...
function slot0.onInitView(slot0)
	...
	slot0._txtLeftTime = gohelper.findChildText(slot0.viewGO, "#go_right/#txtLeftTime")
...
function slot0.onOpen(slot0)
	slot0:addEventCb(TaskController.instance, TaskEvent.UpdateTaskList, slot0._updateDaily, slot0)
	slot0:addEventCb(TaskController.instance, TaskEvent.SetTaskList, slot0._updateDaily, slot0)
	slot0:addEventCb(TaskController.instance, TaskEvent.SuccessGetBonus, slot0._updateDaily, slot0)
	slot0:addEventCb(TaskController.instance, TaskEvent.OnShowTaskFinished, slot0._onShowTaskFinished, slot0)
	transformhelper.setLocalPosXY(slot0._gonormaltask.transform, 0, TaskModel.instance:getMaxStage(TaskEnum.TaskType.Daily) - (TaskModel.instance:getTaskActivityMO(TaskEnum.TaskType.Daily).defineId + 1) >= 5 and 160 * (slot2 - 1) or 160 * (slot1 - 5))

	if TaskModel.instance:getRefreshCount() == 0 and TaskView.getInitTaskType() ~= TaskEnum.TaskType.Daily then
		return
	end
...
function slot0._onShowTaskFinished(slot0, slot1)
	if slot1 == TaskEnum.TaskType.Daily then
		return
	end
...
function slot0._refreshDaily(slot0)
...
function slot0._updateDaily(slot0)
	transformhelper.setLocalPosXY(slot0._gonormaltask.transform, 0, TaskModel.instance:getMaxStage(TaskEnum.TaskType.Daily) - (TaskModel.instance:getTaskActivityMO(TaskEnum.TaskType.Daily).defineId + 1) >= 5 and 160 * (slot2 - 1) or 160 * (slot1 - 5))
...
function slot0._showAllComplete(slot0)
	slot1 = TaskModel.instance:isAllRewardGet(TaskEnum.TaskType.Daily)
...
function slot0._refreshLeftTime(slot0)
	if TaskModel.instance:getTaskTypeExpireTime(TaskEnum.TaskType.Daily) - ServerTime.now() > 0 then
		slot0._txtLeftTime.text = TimeUtil.secondToRoughTime3(slot1, false) and luaLang("task_remaintime") .. slot2 or ""
	else
...
function slot0._refreshTaskLevelItem(slot0, slot1)
	for slot6 = 1, #TaskConfig.instance:getTaskActivityBonusConfig(TaskEnum.TaskType.Daily) do
		if not slot0._tasklevels[slot6] then
			slot7 = gohelper.cloneInPlace(slot0._gotasklevel.gameObject)

			gohelper.setActive(slot7, true)

			slot0._tasklevels[slot6] = TaskListCommonLevelItem.New()

			slot0._tasklevels[slot6]:init(slot7, slot0._goleft)
		end

		slot0._tasklevels[slot6]:setItem(slot6, slot2[slot6], TaskEnum.TaskType.Daily, slot1)
	end
...
function slot0._setCommonTaskItem(slot0, slot1)
	...
	slot4 = TaskListModel.instance:getTaskList(TaskEnum.TaskType.Daily)
	...
	if slot1 then
		...
		TaskDispatcher.runRepeat(slot0.showByLine, slot0, 0.04, #(TaskModel.instance:isAllRewardGet(TaskEnum.TaskType.Daily) and {} or TaskModel.instance:getAllRewardUnreceivedTasks(TaskEnum.TaskType.Daily)) >= 2 and #slot4 + 1 or #slot4)
	else
...
function slot0.showByLine(slot0)
	...
	slot2 = TaskModel.instance:isAllRewardGet(TaskEnum.TaskType.Daily) and {} or TaskModel.instance:getAllRewardUnreceivedTasks(TaskEnum.TaskType.Daily)
	slot3 = TaskListModel.instance:getTaskList(TaskEnum.TaskType.Daily)
...
function slot0.setItem(slot0, slot1, slot2, slot3)
	...
	if slot0._taskItems[slot2] then
		...
		slot0._taskItems[slot2]:reset(TaskEnum.TaskType.Daily, slot2, slot1)
	else
		slot0._taskItems[slot2] = TaskListCommonItem.New()

		slot0._taskItems[slot2]:init(slot0:getResInst(slot0.viewContainer:getSetting().otherRes[1], slot0._gotaskitemcontent, "item" .. slot2), TaskEnum.TaskType.Daily, slot2, slot1, slot3)
	end
...
function slot0.onDestroyView(slot0)
	...
	TaskDispatcher.cancelTask(slot0._updateItems, slot0)
	TaskDispatcher.cancelTask(slot0._refreshLeftTime, slot0)
	slot0:removeEventCb(TaskController.instance, TaskEvent.SetTaskList, slot0._updateDaily, slot0)
	slot0:removeEventCb(TaskController.instance, TaskEvent.UpdateTaskList, slot0._updateDaily, slot0)
	slot0:removeEventCb(TaskController.instance, TaskEvent.SuccessGetBonus, slot0._updateDaily, slot0)
