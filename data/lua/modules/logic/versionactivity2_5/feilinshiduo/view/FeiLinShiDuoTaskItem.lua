...
slot0 = class("FeiLinShiDuoTaskItem", ListScrollCellExtend)
...
function slot0._btnnotfinishbgOnClick(slot0)
	if slot0._act185TaskMO.config.jumpId > 0 then
		GameFacade.jump(slot0._act185TaskMO.config.jumpId)
	end
end

function slot0._btnfinishbgOnClick(slot0)
	slot0:_onOneClickClaimReward(slot0._act185TaskMO.activityId)
...
function slot0._btngetallOnClick(slot0)
	FeiLinShiDuoGameController.instance:dispatchEvent(FeiLinShiDuoEvent.OneClickClaimReward, slot0._act185TaskMO.activityId)
	UIBlockHelper.instance:startBlock("FeiLinShiDuoTaskItem_finishAnim", 0.5)
	TaskDispatcher.runDelay(slot0._delayFinishAll, slot0, 0.5)
end

function slot0._delayFinish(slot0)
	TaskRpc.instance:sendFinishTaskRequest(slot0._act185TaskMO.config.id)
end
...
function slot0._editableAddEvents(slot0)
	FeiLinShiDuoGameController.instance:registerCallback(FeiLinShiDuoEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._editableRemoveEvents(slot0)
	FeiLinShiDuoGameController.instance:unregisterCallback(FeiLinShiDuoEvent.OneClickClaimReward, slot0._onOneClickClaimReward, slot0)
end

function slot0._onOneClickClaimReward(slot0, slot1)
	if slot0._act185TaskMO and slot0._act185TaskMO.activityId == slot1 and slot0._act185TaskMO:alreadyGotReward() then
...
function slot0.onUpdateMO(slot0, slot1)
	slot0._act185TaskMO = slot1
	slot0._scrollRewards.parentGameObject = slot0._view._csListScroll.gameObject

	slot0:_refreshUI()
	slot0:_moveByRankDiff(FeiLinShiDuoTaskListModel.instance:getRankDiff(slot1))
end
...
function slot0._refreshUI(slot0)
	if not slot0._act185TaskMO then
		return
	end
...
slot0.prefabPath = "ui/viewres/versionactivity_2_5/feilinshiduo/view/v2a5_feilinshiduo_taskitem.prefab"
