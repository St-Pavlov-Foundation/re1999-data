...
function slot0._editableInitView(slot0)
	...
	slot0._roleNum = Act183Helper.getEpisodeBattleNum(slot0._episodeId)
	slot0._snapshotType = Act183Helper.getEpisodeSnapShotType(slot0._episodeId)
...
function slot0.initHeroList(slot0)
	...
	if not slot0._cardScale then
		logError(string.format("卡牌缩放配置(BgAndCardScaleMap)不存在 snapshotType = %s", slot0._snapshotType))
	end

	for slot4 = 1, slot0._roleNum do
		slot8 = MonoHelper.addNoUpdateLuaComOnceToGo(gohelper.cloneInPlace(slot0._goheroitem, "item" .. slot4), slot0:_getHeroItemCls(), slot0)

		slot8:setIndex(slot4)
		slot8:setScale(slot0._cardScale[1] or 1, slot0._cardScale[2] or 1, slot0._cardScale[3] or 1)
