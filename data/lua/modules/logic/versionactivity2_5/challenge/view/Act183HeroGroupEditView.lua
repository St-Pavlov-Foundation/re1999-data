module("modules.logic.versionactivity2_5.challenge.view.Act183HeroGroupEditView", package.seeall)

slot0 = class("Act183HeroGroupEditView", BaseView)
...
function slot0._btnconfirmOnClick(slot0)
	if slot0._isShowQuickEdit then
		if Act183HeroGroupQuickEditListModel.instance:getHeroUids() and #slot1 > 0 then
	...
	if slot0._heroMO then
		...
		if slot0._heroMO:isTrial() and not HeroSingleGroupModel.instance:isInGroup(slot0._heroMO.uid) and (slot1:isEmpty() or not slot1.trial) and Act183HeroGroupEditListModel.instance:isTrialLimit() then
...
function slot0._btncharacterOnClick(slot0)
	if slot0._heroMO then
		...
		for slot6, slot7 in ipairs((not slot0._isShowQuickEdit or Act183HeroGroupQuickEditListModel.instance:getList()) and Act183HeroGroupEditListModel.instance:getList()) do
...
function slot0._btnattributeOnClick(slot0)
	if slot0._heroMO then
		...
		CharacterController.instance:openCharacterTipView({
			...
			showExtraAttr = true,
			fromAct183HeroGroupEditView = true,
			heroMo = slot0._heroMO,
			trialEquipMo = slot2,
			isBalance = HeroGroupBalanceHelper.getIsBalanceMode() and not slot0._heroMO:isTrial()
		})
...
function slot0._btnquickeditOnClick(slot0)
	...
	if slot0._isShowQuickEdit then
		...
		if Act183HeroGroupQuickEditListModel.instance:getById(slot0._originalHeroUid) then
			Act183HeroGroupQuickEditListModel.instance:selectCell(Act183HeroGroupQuickEditListModel.instance:getIndex(slot1), true)
		end
	else
		...
		if HeroSingleGroupModel.instance:getHeroUid(slot0._singleGroupMOId) ~= "0" then
			Act183HeroGroupEditListModel.instance:selectCell(Act183HeroGroupEditListModel.instance:getIndex(Act183HeroGroupEditListModel.instance:getById(slot1)), true)
		end
...
function slot0._saveQuickGroupInfo(slot0)
	if Act183HeroGroupQuickEditListModel.instance:getIsDirty() then
		...
		slot0:replaceQuickGroupHeroDefaultEquip(Act183HeroGroupQuickEditListModel.instance:getHeroUids())
...
function slot0._onGroupModify(slot0)
	if slot0._isShowQuickEdit then
		...
		if slot0._originalHeroUid ~= HeroSingleGroupModel.instance:getHeroUid(slot0._singleGroupMOId) then
			...
			Act183HeroGroupEditListModel.instance:setParam(slot1, slot0._adventure)
			slot0:_onHeroItemClick(nil)
			Act183HeroGroupEditListModel.instance:cancelAllSelected()
			Act183HeroGroupEditListModel.instance:selectCell(Act183HeroGroupEditListModel.instance:getIndex(Act183HeroGroupEditListModel.instance:getById(slot1)), true)
		end
...
function slot0.onOpen(slot0)
	...
	slot0._episodeId = slot0.viewParam and slot0.viewParam.episodeId

	CharacterModel.instance:setCharacterList(false, CharacterEnum.FilterType.HeroGroup)
	Act183HeroGroupEditListModel.instance:init(slot0._activityId, slot0._episodeId)
	Act183HeroGroupEditListModel.instance:setParam(slot0._originalHeroUid, slot0._adventure, slot0._isTowerBattle)
	Act183HeroGroupQuickEditListModel.instance:init(slot0._activityId, slot0._episodeId)
	Act183HeroGroupQuickEditListModel.instance:setParam(slot0._adventure, slot0._isTowerBattle)

	slot0._heroMO = Act183HeroGroupEditListModel.instance:copyCharacterCardList(true)
