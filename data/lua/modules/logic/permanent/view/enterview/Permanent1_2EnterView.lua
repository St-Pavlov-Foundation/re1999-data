...
slot0 = class("Permanent1_2EnterView", BaseView)
...
function slot0._btnEntranceRole1OnClick(slot0)
	if string.nilorempty(ActivityConfig.instance:getActivityCo(VersionActivity1_2Enum.ActivityId.YaXian).confirmCondition) then
		YaXianController.instance:openYaXianMapView()
	elseif OpenModel.instance:isFunctionUnlock(tonumber(string.split(slot2, "=")[2])) or PlayerPrefsHelper.getNumber(PlayerPrefsKey.EnterRoleActivity .. "#" .. tostring(VersionActivity1_2Enum.ActivityId.YaXian) .. "#" .. tostring(PlayerModel.instance:getPlayinfo().userId), 0) == 1 then
		YaXianController.instance:openYaXianMapView()
	else
...
function slot0._editableInitView(slot0)
	slot0.actCfg = ActivityConfig.instance:getActivityCo(VersionActivity1_2Enum.ActivityId.EnterView)
end

function slot0.onOpen(slot0)
	slot2 = ActivityConfig.instance:getActivityCo(VersionActivity1_2Enum.ActivityId.JieXiKa)
	slot3 = ActivityConfig.instance:getActivityCo(VersionActivity1_2Enum.ActivityId.Dungeon)

	if ActivityConfig.instance:getActivityCo(VersionActivity1_2Enum.ActivityId.YaXian).redDotId ~= 0 then
