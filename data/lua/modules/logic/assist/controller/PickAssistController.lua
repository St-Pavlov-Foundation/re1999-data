-- chunkname: @modules/logic/assist/controller/PickAssistController.lua

module("modules.logic.assist.controller.PickAssistController", package.seeall)

local PickAssistController = class("PickAssistController", BaseController)

function PickAssistController:openPickAssistView(assistType, actId, selectedHeroUid, finishCall, finishCallObj, isAutoRefresh, extraStr, notNeedCareer, viewParamData)
	...
	self._viewParamData = viewParamData
...
function PickAssistController:_openPickAssistViewAfterRpc(cmd, resultCode, msg)
	...
	if viewName then
		ViewMgr.instance:openView(viewName, self._viewParamData)
	end
end
