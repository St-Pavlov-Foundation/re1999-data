-- chunkname: @modules/logic/versionactivity2_2/dungeon/view/store/VersionActivity2_2StoreTalk.lua

module("modules.logic.versionactivity2_2.dungeon.view.store.VersionActivity2_2StoreTalk", package.seeall)

local VersionActivity2_2StoreTalk = class("VersionActivity2_2StoreTalk", BaseView)

function VersionActivity2_2StoreTalk:onInitView()
...
function VersionActivity2_2StoreTalk:addEvents()
	return
end

function VersionActivity2_2StoreTalk:removeEvents()
	return
end

VersionActivity2_2StoreTalk.TalkMinTriggerDuration = 2
VersionActivity2_2StoreTalk.TalkStepShowDuration = 2
VersionActivity2_2StoreTalk.TalkTxtShowDuration = 6
VersionActivity2_2StoreTalk.TextSpawnInterval = 0.06
VersionActivity2_2StoreTalk.ChessJumpAnimDuration = 1.167
VersionActivity2_2StoreTalk.ScrollTalkMargin = 40
VersionActivity2_2StoreTalk.TextMaxHeight = 99999
VersionActivity2_2StoreTalk.TipHeight = 10
VersionActivity2_2StoreTalk.SplitChar = "|"

function VersionActivity2_2StoreTalk:onClickStage()
	AudioMgr.instance:trigger(AudioEnum.VersionActivity2_2Store.play_ui_jinye_click_stage)
	self:triggerTalkByType(ActivityStoreConfig.BubbleTalkTriggerType.ClickStageArea)
end

function VersionActivity2_2StoreTalk:onClickText()
...
function VersionActivity2_2StoreTalk:_editableInitView()
	self._scrollTalk.verticalNormalizedPosition = 1
	self.lastTriggerTime = -VersionActivity2_2StoreTalk.TalkMinTriggerDuration
	self.canTriggerList = {}
	self.tempTypeList = {}
	self.triggerStepList = {}
	self.actId = VersionActivity2_2Enum.ActivityId.DungeonStore
...
function VersionActivity2_2StoreTalk:initUpdateBeat()
...
function VersionActivity2_2StoreTalk:_onFrameLateUpdate()
	if self.closed then
		return
	end
	...
	local margin = VersionActivity2_2StoreTalk.ScrollTalkMargin
	local talkHeight

	if preferHeight > VersionActivity2_2StoreTalk.TextMaxHeight then
		talkHeight = VersionActivity2_2StoreTalk.TextMaxHeight + margin
	else
		talkHeight = preferHeight + margin
	end

	if self.waitPlayNextStep then
		talkHeight = talkHeight + VersionActivity2_2StoreTalk.TipHeight
	end

	recthelper.setHeight(self.rectTrTalk, talkHeight)
end

function VersionActivity2_2StoreTalk:_onFrame()
	if self.closed then
		return
	end
	...
	if currentTime - self.lastSpawnTime < VersionActivity2_2StoreTalk.TextSpawnInterval then
		return
	end
...
function VersionActivity2_2StoreTalk:_tickContent()
	if self.closed then
		return
	end
	...
	if self.currentCharIndex == self.contentLen + 1 then
		...
		if nextStepCo then
			...
			TaskDispatcher.runDelay(self.playNextStep, self, VersionActivity2_2StoreTalk.TalkStepShowDuration)
		else
			self:startWaitFinishTalk()
			TaskDispatcher.runDelay(self.onFinishTalk, self, VersionActivity2_2StoreTalk.TalkTxtShowDuration)
		end
	end
end

function VersionActivity2_2StoreTalk:startWaitFinishTalk()
	self.waitFinishTalk = true
end

function VersionActivity2_2StoreTalk:startWaitPlayNextStep()
	self.waitPlayNextStep = true

	gohelper.setActive(self._goArrowTip, true)
end

function VersionActivity2_2StoreTalk:onFinishTalk()
...
function VersionActivity2_2StoreTalk:playNextStep()
...
function VersionActivity2_2StoreTalk:onCloseViewFinish(viewName)
...
function VersionActivity2_2StoreTalk:onBuyGoodsSuccess(actId, goodsId)
...
function VersionActivity2_2StoreTalk:onOpen()
	self:initGroupCo()
	TaskDispatcher.runDelay(self._triggerEnterAudio, self, 0.5)
end

function VersionActivity2_2StoreTalk:_triggerEnterAudio()
	AudioMgr.instance:trigger(AudioEnum.VersionActivity2_2Store.play_ui_jinye_chess_enter)
end

function VersionActivity2_2StoreTalk:onOpenFinish()
	if self.isFirstEnter then
		self:triggerTalkByType(ActivityStoreConfig.BubbleTalkTriggerType.FirstEnterActivityStore)

		local key = PlayerModel.instance:getPlayerPrefsKey(PlayerPrefsKey.Version2_2PlayedStoreGroupIdKey)
		local preValue = PlayerPrefsHelper.getString(key, "")
		local idList = string.splitToNumber(preValue, VersionActivity2_2StoreTalk.SplitChar)

		table.insert(idList, self.groupCo.groupId)
		PlayerPrefsHelper.setString(key, table.concat(idList, VersionActivity2_2StoreTalk.SplitChar))
	else
		self:triggerTalkByType(ActivityStoreConfig.BubbleTalkTriggerType.EnterActivityStore)
	end
end

function VersionActivity2_2StoreTalk:initGroupCo()
	local groupList = ActivityStoreConfig.instance:getUnlockGroupList(self.actId)
	local key = PlayerModel.instance:getPlayerPrefsKey(PlayerPrefsKey.Version2_2PlayedStoreGroupIdKey)
	local groupIdList = {}

	for _, v in ipairs(string.split(PlayerPrefsHelper.getString(key, ""), VersionActivity2_2StoreTalk.SplitChar)) do
...
function VersionActivity2_2StoreTalk:triggerTalkByTypeList(typeList, typeParam)
...
function VersionActivity2_2StoreTalk:triggerTalkByType(type, typeParam)
	tabletool.clear(self.canTriggerList)
...
function VersionActivity2_2StoreTalk:getRandomTalkCo(coList)
	local count = #coList
...
function VersionActivity2_2StoreTalk:_triggerTalk(talkCo)
	local currentTime = Time.time

	if currentTime - self.lastTriggerTime < VersionActivity2_2StoreTalk.TalkMinTriggerDuration then
		return
	end
	...
	self.lastSpawnTime = currentTime - VersionActivity2_2StoreTalk.TextSpawnInterval
...
function VersionActivity2_2StoreTalk:playStep(stepIndex)
	...
	TaskDispatcher.runRepeat(self.stopChessAnim, self, VersionActivity2_2StoreTalk.ChessJumpAnimDuration)
	...
	AudioMgr.instance:trigger(AudioEnum.VersionActivity2_2Store.play_ui_jinye_chess_talk)
end

function VersionActivity2_2StoreTalk:buildStepCoList(talkCo)
	...
	if #self.triggerStepList > 1 then
		table.sort(self.triggerStepList, VersionActivity2_2StoreTalk.sortStep)
	end
end

function VersionActivity2_2StoreTalk:stopChessAnim()
	if self.waitPlayNextStep or self.waitFinishTalk then
...
function VersionActivity2_2StoreTalk:onClose()
	TaskDispatcher.cancelTask(self.stopChessAnim, self)
	TaskDispatcher.cancelTask(self.playNextStep, self)
	TaskDispatcher.cancelTask(self.onFinishTalk, self)
	AudioMgr.instance:trigger(AudioEnum.VersionActivity2_2Store.stop_ui_jinye_chess_talk)
...
function VersionActivity2_2StoreTalk:onDestroyView()
	self.stageClick:RemoveClickListener()
	self.talkTextClick:RemoveClickListener()
end

function VersionActivity2_2StoreTalk.sortStep(co1, co2)
	return co1.step < co2.step
end

return VersionActivity2_2StoreTalk
