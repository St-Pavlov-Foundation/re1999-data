...
function slot0.onOpen(slot0)
	...
	gohelper.setActive(slot0.goAccountContainer, slot0.isShowUserCenter and (GameChannelConfig.isXfsdk() and not SettingsModel.isBilibili()))

	slot3 = SDKMgr.instance:isShowUnregisterButton()

	logNormal(string.format("SettingsAccountView isShowUserCenter:%s,isBilibili:%s,isShowUnregisterButton:%s", tostring(slot0.isShowUserCenter), tostring(SettingsModel.isBilibili()), tostring(slot3)))

	if slot3 and not BootNativeUtil.isIOS() then
		gohelper.setActive(slot0.goAccountLogoutContainer, true)
	end
end
