...
function slot0.onInitView(slot0)
	...
	slot0._txtcollectProcess = gohelper.findChildText(slot0.viewGO, "root/right/collect/#go_collectProcess/#txt_collectProcess")
						...
						if GameUtil.utf8len(string.sub(slot6, 1, slot9 - 1)) <= slot13.textInfo.characterCount then
							...
							if slot13.textInfo.characterInfo[GameUtil.utf8len("") + slot21 - 1].bottomRight.x < slot25.x then
								...
								for slot34 = slot21, slot28 do
									...
									if slot13.textInfo.characterInfo[slot34] and slot36 and slot35.baseLine - slot36.baseLine > 1 then
										slot37 = gohelper.cloneInPlace(slot17)

										table.insert(slot18, slot37)

										slot38 = slot37.transform

										recthelper.setAnchorX(slot38, slot36.bottomLeft.x + slot22 / 2)
										recthelper.setAnchorY(slot38, slot36.baseLine - slot23 / 2 - 10)
										recthelper.setWidth(slot38, slot30.x - slot36.bottomLeft.x)
									end
								end
							else
