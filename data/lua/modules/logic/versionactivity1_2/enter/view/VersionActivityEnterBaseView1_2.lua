...
function slot0.refreshCenterActUI(slot0)
	...
	if slot0._txttime then
		slot0._txttime.text = string.format("<color=#578425>%s ~ %s(%s)</color>", slot1:getStartTimeStr(), slot1:getEndTimeStr(), ServerTime.GetUTCOffsetStr())
	end

	if slot0._txtremaintime then
		slot2 = slot1:getRealEndTimeStamp() - ServerTime.now()
		slot0._txtremaintime.text = string.format(luaLang("remain"), Mathf.Floor(slot2 / TimeUtil.OneDaySecond) .. luaLang("time_day") .. Mathf.Floor(slot2 % TimeUtil.OneDaySecond / TimeUtil.OneHourSecond) .. luaLang("time_hour2"))
	end
end
