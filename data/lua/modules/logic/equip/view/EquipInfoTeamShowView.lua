...
slot0 = class("EquipInfoTeamShowView", BaseView)
...
function slot0._btnconfirmOnClick(slot0)
	slot0.handleFuncDict[slot0.viewParam.confirmViewType or slot0.viewParam.fromView](slot0)
end
...
function slot0._onClickConfirmBtnFromRougeHeroGroupFightView(slot0)
	slot3 = false
	...
	RougeHeroGroupModel.instance:replaceEquips({
		index = slot0.posIndex,
		equipUid = {
			slot0.selectedEquipMo and slot0.selectedEquipMo.uid or "0"
		}
	...
	HeroGroupController.instance:dispatchEvent(HeroGroupEvent.ChangeEquip, slot0.posIndex)
	RougeHeroGroupModel.instance:rougeSaveCurGroup(slot0.closeThis, slot0, slot4)
end
...
function slot0._onClickConfirmBtnFromSeason166HeroGroupFightView(slot0)
	slot3 = false
	...
	Season166HeroGroupModel.instance:replaceEquips({
		index = slot0.posIndex,
		equipUid = {
			slot0.selectedEquipMo and slot0.selectedEquipMo.uid or "0"
		}
	...
	HeroGroupController.instance:dispatchEvent(HeroGroupEvent.ChangeEquip, slot0.posIndex)
	Season166HeroGroupModel.instance:saveCurGroupData(slot0.closeThis, slot0, slot4)
end
...
function slot0.onEquipTypeHasChange(slot0, slot1)
	...
	if slot0.heroMo and slot0.heroMo:getTrialEquipMo() then
...
function slot0._editableInitView(slot0)
	...
	slot0.handleFuncDict = {
		...
		[EquipEnum.FromViewEnum.FromRougeHeroGroupFightView] = slot0._onClickConfirmBtnFromRougeHeroGroupFightView,
		...
		[EquipEnum.FromViewEnum.FromSeason123HeroGroupFightView] = slot0._onClickConfirmBtnFromHeroGroupFightView,
		[EquipEnum.FromViewEnum.FromSeason166HeroGroupFightView] = slot0._onClickConfirmBtnFromSeason166HeroGroupFightView
	}
end
...
function slot0.onOpen(slot0)
	...
	slot0._isBalance = slot0.viewParam.isBalance
...
function slot0.initOriginEquipMo(slot0)
	...
	elseif slot0.viewParam.fromView == EquipEnum.FromViewEnum.FromCachotHeroGroupFightView then
		slot0.originEquipMo = slot0.viewParam.equipMo
	elseif slot0.viewParam.fromView == EquipEnum.FromViewEnum.FromSeason123HeroGroupFightView or slot0.viewParam.fromView == EquipEnum.FromViewEnum.FromSeason166HeroGroupFightView then
		slot0.originEquipMo = slot0.viewParam.equipMo
	else
...
function slot0.refreshCompareContainerUI(slot0)
	...
	if tonumber(slot1.uid) > 0 and slot0.viewParam.fromView == EquipEnum.FromViewEnum.FromHeroGroupFightView and HeroGroupBalanceHelper.getIsBalanceMode() or slot0._isBalance then
		if slot1.level < slot0.viewContainer:getBalanceEquipLv() then
			slot4 = EquipMO.New()

			slot4:initByConfig(nil, slot1.equipId, slot3, slot1.refineLv)
	...
	if #EquipHelper.getEquipSkillBaseDes(slot1.config.id, slot1.refineLv, "#D9A06F") == 0 then
		...
		for slot19, slot20 in ipairs(slot14) do
			if not slot0.container1_skillDescItems[slot19] then
				...
				SkillHelper.addHyperLinkClick(slot15.txt)
				table.insert(slot0.container1_skillDescItems, slot15)
			end

			slot15.txt.text = EquipHelper.getEquipSkillDesc(slot20)
...
function slot0.refreshHeroInfo(slot0)
	...
	slot0._txtheroname.text = slot0.heroMo:getHeroName()
...
function slot0.refreshLeftUI(slot0)
	...
	if (slot0.selectedEquipMo and not (slot0.heroMo and slot0.heroMo.trialEquipMo and true or false) and tonumber(slot0.selectedEquipMo.uid) > 0 and slot0.viewParam.fromView == EquipEnum.FromViewEnum.FromHeroGroupFightView and HeroGroupBalanceHelper.getIsBalanceMode() or slot0._isBalance) and slot0.selectedEquipMo then
		if slot0.viewContainer:getBalanceEquipLv() <= slot0.selectedEquipMo.level then
			slot2 = nil
		else
			slot4 = EquipMO.New()

			slot4:initByConfig(nil, slot0.selectedEquipMo.equipId, slot3, slot0.selectedEquipMo.refineLv)

			slot0._balanceEquipMo = slot4
		end
	end
	...
	slot0.layoutElement.minHeight = slot2 and 142 or 187
	...
	gohelper.setActive(slot0._btnjump.gameObject, slot0.selectedEquipMo ~= nil and tonumber(slot0.selectedEquipMo.uid) > 0)
...
function slot0.refreshEquipLevel(slot0)
	if slot0._balanceEquipMo then
		...
		slot0._txtlevel.text = string.format("LV.<color=#d9a06f>%d</color>/<color=#777676>%d</color>", slot0.selectedEquipMo.level, EquipConfig.instance:getCurrentBreakLevelMaxLevel(slot0.selectedEquipMo))
	end
end

function slot0.refreshEquipNormalAttr(slot0)
	slot1 = slot0._balanceEquipMo or slot0.selectedEquipMo
	slot2, slot3 = EquipConfig.instance:getEquipNormalAttr(slot1.config.id, slot1.level, HeroConfig.sortAttrForEquipView)
	slot4, slot5 = nil

	for slot9, slot10 in ipairs(slot3) do
		if not slot0.strengthenAttrItems[slot9] then
			slot4 = {
				go = gohelper.cloneInPlace(slot0._gostrengthenattr, "item" .. slot9)
			}
			slot4.icon = gohelper.findChildImage(slot4.go, "image_icon")
			slot4.name = gohelper.findChildText(slot4.go, "txt_name")
			slot4.attr_value = gohelper.findChildText(slot4.go, "txt_value")
			slot4.bg = gohelper.findChild(slot4.go, "bg")

			table.insert(slot0.strengthenAttrItems, slot4)
		end

		slot5 = HeroConfig.instance:getHeroAttributeCO(HeroConfig.instance:getIDByAttrType(slot10.attrType))

		UISpriteSetMgr.instance:setCommonSprite(slot4.icon, "icon_att_" .. slot5.id)

		slot4.name.text = slot5.name
		slot4.attr_value.text = slot10.value

		gohelper.setActive(slot4.bg, slot9 % 2 == 0)
		gohelper.setActive(slot4.go, true)
	end

	for slot9 = #slot3 + 1, #slot0.strengthenAttrItems do
		gohelper.setActive(slot0.strengthenAttrItems[slot9].go, false)
	end

	slot6, slot7 = EquipConfig.instance:getEquipCurrentBreakLvAttrEffect(slot1.config, slot1.breakLv)

	if slot6 then
		gohelper.setActive(slot0._gobreakeffect, true)
		UISpriteSetMgr.instance:setCommonSprite(slot0.imageBreakIcon, "icon_att_" .. slot6)

		slot0.txtBreakAttrName.text = EquipHelper.getAttrBreakText(slot6)
		slot0.txtBreakValue.text = EquipHelper.getAttrPercentValueStr(slot7)
	...
	if #EquipHelper.getEquipSkillBaseDes(slot0.selectedEquipMo.config.id, slot0.selectedEquipMo.refineLv, "#D9A06F") == 0 then
		...
		for slot8, slot9 in ipairs(slot1) do
			if not slot0.skillDescItems[slot8] then
				...
				SkillHelper.addHyperLinkClick(slot4.txt)
				table.insert(slot0.skillDescItems, slot4)
			end

			slot2.txt.text = EquipHelper.getEquipSkillDesc(slot9)
